...
Then there exists $0 < \theta < 1$, which only depends on $V$, such that for any $i \geq 10$, $\vol(Z_i) \leq \theta^i$.
\end{lemma}
\begin{proof}
...
The result follows when we take $\theta < 2^{-s}$ so small that $\theta^{10}$ can absorb the implied constants.
\end{proof}
...
$$\vol\left(\bigcup_{j \geq i} Z_j\right) \leq \sum_{j = i}^\infty \theta^j = \frac{\theta^i}{1 - \theta},$$
