...
To state it we recall that for $s \geq 0$, $1 < p, q < \infty$ such that $\frac{1}{p} + \frac{1}{q} = 1$, and $V$ a Riemannian vector bundle, the dual space of $W^{s, q}_0(M, V)$ (the traceless part of $W^{s, q}$) is denoted by $W^{-s, p}(M, V')$ where $V'$ denotes the dual bundle.

\begin{proposition}[trace theorem]
Let $N$ be an $\ell$-fold, and let $\iota: N \to M$ be a smooth embedding.
...
\end{proposition}
\begin{proof}
It suffices to prove that $\iota^*$ is bounded on smooth $\ell$-forms.
Let $\alpha \in C^\infty(M, \Omega^\ell)$ and $\beta \in C^\infty(N, \Omega^{d - 1 - \ell})$.
Then by Stokes' theorem, (\ref{Stokes trace}) holds.
By the inverse trace theorem \todo{Cite this} and the fact that $\frac{1}{p} = 1 - \frac{1}{q}$,
% https://mathoverflow.net/questions/96532/the-inverse-for-the-trace-theorem
we may extend $\beta$ to a smooth $d - \ell$-form on $M$ such that
$$\|\beta\|_{W^{1, q}(M, \Omega^{d - 1 - \ell})} \lesssim \|\beta\|_{W^{\frac{1}{p}, q}(N, \Omega^{d - 1 - \ell})}.$$
We then estimate using H\"older's inequality
\begin{align*}
\left|\int_N \iota^* \alpha \wedge \iota^* \beta\right|
&\leq \|\dif \alpha\|_{L^p} \|\beta\|_{L^q} + \|\alpha\|_{L^p} \|\dif \beta\|_{L^q} \\
&\leq \|\alpha\|_{W^{1, p} \Omega^\ell} \|\beta\|_{W^{1, q}(M, \Omega^{d - 1 - \ell})} \\
&\lesssim \|\alpha\|_{W^{1, p} \Omega^\ell} \|\beta\|_{W^{\frac{1}{p}, q}(N, \Omega^{d - 1 - \ell})}. \qedhere
\end{align*}
\end{proof}

As $W^{\frac{1}{p}, q}(N, \Omega^{d - 1 - \ell})$ is somewhat intractible, we shall only use the following consequence on closed submanifolds.

\begin{corollary}\label{trace on cycles}
Let $N$ be a closed $\ell$-fold, and let $\iota: N \to M$ a smooth embedding.
Then for $1 < p < \infty$ and $\alpha \in W^{1, p} \Omega^\ell(M)$, the pullback $\iota^* \alpha$ is a signed Radon measure of finite total variation.
In particular, convergence in $W^{1, p} \Omega^\ell$ implies convergence in the weak topology of measures of the pullbacks.
\end{corollary}
\begin{proof}
Let $\frac{1}{p} + \frac{1}{q} = 1$, so that the dual space of $W^{-\frac{1}{p}, p}(N)$ is $W^{\frac{1}{p}, q}_0(N) = W^{\frac{1}{p}, q}(N)$ (since $N$ is closed).
Moreover, $W^{\frac{1}{p}, q}$ is an interpolation space of $W^{1, q}$ and $L^q$, hence 
$$\|1\|_{W^{\frac{1}{p}, q}(N)} = \|1\|_{L^q(N)} = |N|^{\frac{1}{q}} < \infty.$$
So for $\alpha \in W^{1, p} \Omega^\ell(M)$, we obtain from the trace theorem
\begin{align*}
\|\alpha\|_{L^1(N)} 
&= \int_N \star |\alpha| \leq \|\alpha\|_{W^{-\frac{1}{p}, p}(N)} |N|^{\frac{1}{q}} \lesssim \|\alpha\|_{W^{1, p} \Omega^\ell(M)}. \qedhere 
\end{align*}
\end{proof}
...
The \dfn{maximum curl locus} is the set $L(A, x) = L(A)$.
\end{definition}
...
\item $A \mapsto \lambda A$. This changes both $\|\dif A\|_{L^\infty}$ and $c_1(\mathscr F)$.
\end{enumerate}
In particular, by applying both scalings and using Chern-Weil theory, we may impose the following:
