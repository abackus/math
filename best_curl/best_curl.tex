...
In Thurston's the main point is to associate a geodesic lamination in $M$ to each homotopy class of maps $\rho \in [M, N]$, by considering the set on which a best Lipschitz representative of $\rho$ attains its Lipschitz constant.
...
%%%%%%%%%%%%%%%%
\subsection{An analytic conjecture}\label{analytic goals}
...
\begin{enumerate}
	...
	\item We say that $F$ has \dfn{absolutely best comass} if it has best comass, and for every small open ball $\Omega$, $F$ is a minimizer of $L_\Omega$ among all closed forms $B$ with $B|_{\partial \Omega} = F|_{\partial \Omega}$.
\end{enumerate}
\end{definition}
...
Let $\tau$ be a $d-1$-rectifiable set, and $\psi \in C^1(M)$.
...
By \cite[Remark 11.7]{Simon84}, we can write $\tau$ as the disjoint union of a $\mathcal H^{d - 1}$-null set (which we may then assume is empty) and countably many $C^1$ hypersurfaces $(\tau_i)$.
By subdividing further we may assume that $\tau_i$ is contractible.
Each hypersurface $\tau_i$ may not have a well-defined boundary, but it does admit a $C^1$ compact exhaustion $(\chi_i^j)_{j \geq 1}$ with $\supp \chi_i^j$ contractible.
Let $\chi_i^0 = 0$.

Let $F, G \in C^\infty(\Sigma, Z^{d - 1})$ and $\Sigma_i^j$ a contractible neighborhood of $\supp \chi_i^j$ in $M$.
After shrinking $\Sigma_i^j$ suitably we may assume that for each $x \in M$, the number of $\Sigma_i^j$ containing $x$ is at most a constant which only depends on $\tau$.
Since $\Sigma_i^j$ is contractible, there exist $d-2$-forms $A_{ij}, B_{ij}$ in Coulomb gauge on $\Sigma_i^j$ such that $\dif A_{ij} = F$ and $\dif B_{ij} = G$.
\todo{Reduce to the case that there only finitely many active $i,j$. We don't want the constant to depend on $i,j$.}
then by integration by parts and (\ref{Sobolev}),
\begin{align*}
	\left|\int_\tau \psi(F - G)\right| 
	&\leq \sum_{i,j=1}^\infty \left|\int_{\tau_i} (\chi_i^j - \chi_i^{j - 1}) \psi_i (F - G)\right| \\
	&\leq \sum_{i,j=1}^\infty \left|\int_{\tau_i} (\chi_i^j - \chi_i^{j - 1}) \dif \psi_i \wedge (A_{ij} - B_{ij})\right| + \sum_{i,j=1}^\infty \left|\int_{\tau_i} \psi_i \dif (\chi_i^j - \chi_i^{j - 1}) \wedge (A_{ij} - B_{ij})\right| \\
	&\lesssim_{\tau, \psi} \sum_{i,j=1}^\infty \|A_{ij} - B_{ij}\|_{C^0(\Sigma_i^j)} \lesssim_\tau \|F - G\|_{L^p}.
\end{align*}
...
Taking $\psi := 1$, and recalling that chains are rectifiable, it follows the integral in the definition of comass is well-defined as long as $F \in L^p$, $p > d$, and each such integral depends continuously on $F$ in $L^p$.
...
\end{proposition}
\begin{proof}
The reduced boundary $\partial^* \{u > y\}$ is $d-1$-rectifiable, so by the trace theorem, the right-hand side of (\ref{coarea formula}) depends continuously on $F$ in $L^p$.
The left-hand side of (\ref{coarea formula}) depends continuously on $F$ because it only depends on the cohomology class of $F$, which by the trace theorem depends continuously on $F$ in $L^p$.
Therefore we may assume that $F$ is continuous.

Let $\mu := \star |\dif u|$ be the total variation measure and $\normal$ its polar part.
\todo{Why can we assume that $u \in C^1$?}
...
By analogy with $\infty$-harmonic maps, one expects $\infty$-tight forms to be unique, have absolutely best comass, and solve the Euler-Lagrange equations (\ref{infinity Maxwell}).
However, we shall not prove this here; it should follow from the various conjectures of \S\ref{analytic goals}, if they are indeed true.
Thus we conjecture:

\begin{conjecture}
For every $\rho \in H^{d - 1}(M, \RR)$ there exists a unique $\infty$-tight representative $F$ of $\rho$.
Moreover, $F$ has absolutely best comass, and is a weak solution of (\ref{infinity Maxwell}) in a suitable tug-of-war sense.
\end{conjecture}
...
% Of course, the notions of viscosity solution and maximum principle are of no use in \emph{systems} of PDE, and in particular are useless for studying $\infty$-tight forms.
