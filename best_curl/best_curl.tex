...
\begin{corollary}
Let $u: M \to \RR$ be a $q$-harmonic function, and let $A$ be a solution of the dual problem (\ref{dual problem}).
Then $\dif A$ is a Noetherian conserved current for $A$, induced by the translation symmetry of the target $\RR$.
\end{corollary}

Thus we call $A$ a \dfn{Noetherian potential} for $u$.
