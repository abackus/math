...
Let $\mu(U) := \Mass_U(\dif u)$ be the total variation measure of a function $u \in BV_\loc(M)$.
Then 
...
Let $u \in BV(M)$.
Then for almost every $y \in \RR$, integration along the reduced level set $\partial^* \{u > y\}$ defines an integral $d - 1$-current.
\end{lemma}
\begin{proof}
By Proposition \ref{coarea}, for almost every $y \in \RR$, $\{u > y\}$ has locally finite perimeter.
So by the de Giorgi structure theorem \cite[Theorem 14.3]{Simon84}, $\partial^* \{u > y\}$ is an oriented rectifiable $d - 1$-cycle.
So integration along $\partial^* \{u > y\}$ defines a closed rectifiable, and hence integral, $d - 1$-current.
\end{proof}
...
\begin{definition}
Let $\lambda$ be a lamination and $F$ a calibration.
We say that $\lambda$ is \dfn{$F$-calibrated} if every leaf of $\lambda$ is $F$-calibrated.
\end{definition}

The fundamental theorem of calibrated geometry \cite{Harvey82} implies that a calibrated integral current is minimal, so every calibrated lamination is minimal.
...
$$\frac{1}{\Mass(\sigma)} \int_\sigma F \leq \sup_{y \in B_\varepsilon(x)} L(F, y) < L = 1.$$
But then 
...
We remind the reader of the definition (\ref{coarea formula}) of the Radon measure $\dif u \wedge F$.
...
\item Let $F$ be the $\infty$-tight representative of $\rho$, with best comass $L$. We have the \dfn{max flow min cut principle} that
