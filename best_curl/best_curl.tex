...
Since the space $W^{-\frac{1}{p}, p}$ can be intractible, we use the following consequence on closed submanifolds, or equivalently cycles.

\begin{corollary}\label{trace on cycles}
Let $N$ be a closed $\ell$-fold, and let $\iota: N \to M$ a smooth immersion.
...
\end{corollary}
\begin{proof}
Let $\frac{1}{p} + \frac{1}{q} = 1$, so that the dual space of $W^{-\frac{1}{p}, p}$ is $W^{\frac{1}{p}, q}_0$.
Since $N$ is closed, $W^{\frac{1}{p}, q}_0(N) = W^{\frac{1}{p}, q}(N)$.
...
\begin{align*}
\|\alpha\|_{L^1(N)} 
&= \int \star |\alpha| \leq \|\alpha\|_{W^{-\frac{1}{p}, p}(N)} |N|^{\frac{1}{q}} \lesssim \|\alpha\|_{W^{1, p} \Omega^\ell(M)}. \qedhere 
\end{align*}
\end{proof}
