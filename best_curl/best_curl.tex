...
Let $\rho$ be a point on the unit sphere of $H^{d - 1}(M, \RR)$, and let $F$ be a best comass representative of $\rho$.
Then there exists a unique $F$-calibrated lamination $\lambda_\rho$ such that every $F$-calibrated lamination is a sublamination of $\lambda_\rho$.
Moreover, $\lambda_\rho$ is determined by $\rho$.
\end{proposition}
\begin{proof}
Let $\mathscr L$ be the set of connected complete $F$-calibrated hypersurfaces.
By Lemma \ref{calibrated implies disjoint}, $\mathscr L$ consists of pairwise disjoint minimal hypersurfaces.
By Proposition \ref{MCL contains Thurston}, there is an $F$-calibrated lamination $\lambda$, and then by Proposition \ref{properties of calibrated laminations}, there is an $F$-calibrated leaf of $\lambda$, which implies that $\mathscr L$ is nonempty.

Let $B \subset M$ be a small ball, $L \in \mathscr L$, and $N$ a component of $L \cap B$ (so $\partial N \subset \partial B$).
...
$$\|\Two_N\|_{C^0(B')} \lesssim_{B'} 1.$$
Since this estimate is independent of $N$, it follows by Theorem \ref{disjoint surfaces are lamination} that $\mathscr L$ is the set of leaves of some lamination $\lambda_\rho$.
By Proposition \ref{properties of calibrated laminations}, $\lambda_\rho$ only depends on $\rho$ and not $F$.
By definition, $\lambda_\rho$ is an $F$-calibrated lamination; if $\lambda$ is another $F$-calibrated lamination, then the leaves of $\lambda$ are $F$-calibrated, hence are leaves of $\lambda_\rho$, so $\lambda_\rho$ is maximal among $F$-calibrated laminations.
\end{proof}
...
\begin{lemma}\label{minimal sets are measured stretch}
Let $\rho$ be a point on the unit sphere of $H^{d - 1}(M, \RR)$, with canonical lamination $\lambda_\rho$.
Then every sublamination of $\lambda_\rho$ which is minimal with respect to inclusion is a measured stretch lamination associated to $\rho$.
\end{lemma}
\begin{proof}
Let $\kappa$ be a sublamination of $\lambda_\rho$ which is minimal with respect to inclusion.
\todo{Then every leaf of $\kappa$ is dense in $\kappa$.}

If $\kappa$ consists of a single closed leaf, we can define a $1$-harmonic map $u: M \to \Sph^1$ which maps all of $M$ to the north pole of $\Sph^1$, but which jumps around all of $\Sph^1$ along the leaf of $\kappa$.
We can then lift $u$ to a $\pi_1(M)$-equivariant $1$-harmonic function on $\tilde M$ to see that $\kappa$ is a measured stretch lamination.

Otherwise \todo{take the equidistributed measure on the universal cover}.
\end{proof}

\begin{proposition}
Let $\rho$ be a point on the unit sphere of $H^{d - 1}(M, \RR)$, with canonical lamination $\lambda_\rho$.
Then the set of leaves of $\lambda_\rho$ is the union of the sets of leaves of the measured stretch laminations associated to $\rho$.
\end{proposition}
\begin{proof}
By Zorn's lemma, every leaf of $\lambda_\rho$ is contained in a sublamination $\kappa$ of $\lambda_\rho$ which is minimal with respect to inclusion.
By Lemma \ref{minimal sets are measured stretch}, $\kappa$ is a measured stretch lamination for $\rho$, and every leaf of $\kappa$ is a leaf of $\lambda_\rho$.
So every leaf of $\lambda_\rho$ is contained in a measured stretch lamination.

In the other direction, let $F$ be a best comass representative of $\rho$.
Every measured stretch lamination $\kappa$ is $F$-calibrated by Proposition \ref{MCL contains Thurston}, so by definition of the canonical lamination, $\kappa$ is a sublamination of $\lambda_\rho$.
\end{proof}
...
Still, one expects to be able to recover the theory:

\begin{problem}
Formulate the results in this paper for manifolds with boundary and infinite ends.
\end{problem}
