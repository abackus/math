...
$$|\dif u| = (\dif u, X).$$
A function is $1$-harmonic if and only if it has least gradient, in the sense that it is a critical point of $\|\dif u\|_{TV}$ (which is just $\|\dif u\|_{L^1}$ if $u$ is absolutely continuous) \cite{Mazon14}.
...
Then for any $d < p \leq \infty$, $F \mapsto \int_\tau \psi F$ extends to a continuous linear functional on $L^p(M, Z^{d - 1})$.
Moreover,
...
The notions of ``locally finite perimeter'' and ``reduced boundary'' are independent of the choice of Riemannian metric \cite[\S2.2]{BackusFLG}.

Let $\mu$ be the total variation measure of $\dif u$, and let $\normal$ be the conormal $1$-form to the level sets of $u$, hence $\dif u = \normal \mu$.
Then for any open set $U$ \cite[Proposition 2.5]{BackusFLG}
...
Throughout this section, let $M$ be a closed Riemannian of dimension $2 \leq d \leq 4$ equipped with a cohomology class $\rho \in H^{d - 1}(M, \RR)$.
We study the set on which a best comass representative of $\rho$ attains its comass.
...
\begin{definition}\label{lamination dfn}
...
For a more careful exposition of the definitions, see \cite[\S1]{BackusCML} and \cite[\S\S7--8]{daskalopoulos2020transverse}.
