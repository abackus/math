...
\subsection{Preliminaries on convex optimization}
We follow \cite{Ekeland99}.
For a reflexive Banach space $X$, we denote by $\hat X$ its dual.
If $J: X \to \RR \cup \{+\infty\}$ is a convex function, we introduce its \dfn{Legendre transform}, the convex function
\begin{align*}
	\hat J: \hat X &\to \RR \cup \{+\infty\}\\
	\xi &\mapsto \sup_{x \in X} \langle \xi, x\rangle - J(x).
\end{align*}

\begin{proposition}[convex duality]\label{abstract convex analysis}
...
One of the most famous corollaries of Proposition \ref{abstract convex analysis} is the \dfn{max flow min cut theorem} of discrete convex optimization \cite[Chapter 7]{umesh2006algorithms}.
It is profitable to view Proposition \ref{abstract convex analysis} as a vast generalization of the max flow min cut theorem.

We now turn to the problem at hand.
Let $M$ be a closed oriented manifold with fundamental group $\Gamma$ and universal covering $\tilde M \to M$, $M_{\rm fun} \subset \tilde M$ a fundamental domain of $\Gamma$, $1 < q < \infty$, and $Y := L^q(M, Z^1)$, where $Z^\ell \subseteq \Omega^\ell$ denotes the sheaf of \emph{closed} $\ell$-forms.
By Poincar\'e duality and the Hurcewiz theorem, we have canonical isomorphisms
$$H_{d - 1}(M, \RR) = H^1(M, \RR) = \Hom(\Gamma, \RR).$$
Sometimes we are interested specifically in integral representations; in that case, we can use the fact that $\Sph^1$ is a $K(\ZZ, 1)$-space to further identify 
$$H^1(M, \ZZ) = \Hom(\Gamma, \ZZ) = [M, \Sph^1].$$

Given a representation
$$\alpha: \Gamma \to \RR,$$
let $X$ be the space of $\alpha$-equivariant functions on $\tilde M$, with the $W^{1, q}(M_{\rm fun})$ norm, modulo constants.
By subtracting off a primitive $\tilde u$ of a representative $1$-form $\dif \tilde u$ of $\alpha$, we may identify $X$ with $W^{1, q}_0(M_{\rm fun})$.
Then $\Lambda := \dif$ carries $X$ into $Y$.
We are interested in solving the $q$-Laplace equation
$$\dif^* (|\dif u|^{q - 2} \dif u) = 0$$
for $u \in X$, which is the Euler-Lagrange equation of the function
$$J_q(\varphi) := \frac{1}{q} \int_M \star |\varphi|^q,$$
where $\varphi := \dif u \in Y$.
Using the methods of \cite[Chapter I, \S4]{Ekeland99} one can easily show that $J_q$ is strictly convex and (with $1/p + 1/q = 1$) its Legendre transform is the strictly convex function
$$\hat J_q(F) = \frac{1}{p} \int_M \star |F|^p + \int_M \dif \tilde u \wedge F$$
where $F \in \hat Y$.

\todo{Rewrite this duality to make it so $\hat Y = L^pZ^{d - 1}$}

The convex dual problem to the $q$-Laplacian is the problem of maximizing
$$-\hat J_q(-F) = -\frac{1}{p} \int_M \star |F|^p + \int_M \dif \tilde u \wedge F$$
among \emph{closed} forms $F$.

$$\frac{1}{q} \int_M \star |\dif u|^q = -\int_M \star |F|^p + \int_M \dif u \wedge F.$$
Thus by Proposition \ref{abstract convex analysis}:


% \begin{definition}
% \begin{enumerate}
% \item A \dfn{flow network} is a finite directed graph $(V, E)$, distinct \dfn{source} and \dfn{sink} vertices $s, t \in V$, and a \dfn{capacity} function $c: E \to \RR_+$.
% \item A \dfn{flow} through a flow network $(V, E, s, t, c)$ is a function $f: E \to \RR_+$ with $f \leq c$, which is \dfn{divergence-free} in the sense that for every vertex $v \in V \setminus \{s, t\}$, if $e_i$ are the edges into $v$ and $f_j$ are the edges out of $v$, then $\sum_i c(e_i) = \sum_j c(f_j)$.
% \item A \dfn{cut} of a flow network $(V, E, s, t, c)$ is a partition $V = S \sqcup T$, where $s \in S$ and $t \in T$.
% \end{enumerate}
% \end{definition}

% \begin{theorem}[max flow min cut]
% Let $(V, E, s, t, c)$ be a flow network.
...
To obtain the convexity, let $F, G$ be representatives of a class $\rho \in H^{d - 1}(M, \RR)$ and $t \in [0, 1]$. Then
\begin{align*}
J_p(tF + (1 - t)G)
...
Moreover,
$$[tF + (1 - t)G] = t[F] + (1 - t)[G] = (t + 1 - t)\rho = \rho,$$
so that $J_p$ is specifically convex when restricted to the class $\rho$.
\end{proof}
...
\todo{Merge this with the appendix, lots of redundancy here}
...
\end{equation}
Indeed, one can write the $p$-tight equation in nondivergence form
$$(p - 2) |\dif A|^{p - 4} \nabla_i (\dif A)_{jk} {(\dif A)^i}_\ell (\dif A)^{jk} + |\dif A|^{p - 2} (\dif^* \dif A)_\ell = 0,$$
renormalize by dividing by $(p - 2) |\dif A|^{p - 4}$, and take the limit as $p \to \infty$ to obtain (\ref{infinity Maxwell}).
