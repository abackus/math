...
\nabla_i F_{jk} {F^i}_\ell F^{jk} = 0.
\end{equation}
Indeed, one can write the $p$-Maxwell equation in nondivergence form
$$(p - 2) |F|^{p - 4} \nabla_i F_{jk} {F^i}_\ell F^{jk} + |F|^{p - 2} (\dif^* F)_\ell = 0,$$
renormalize by dividing by $(p - 2) |F|^{p - 4}$, and take the limit as $p \to \infty$ to obtain (\ref{infinity Maxwell}).
