...
\todo{Prove all this. We might need to assume that $H^{d - 1}(\tilde M, \RR) = 0$ but it might also follow from Poincar\'e duality with compact support.}

\begin{corollary}
Let $A$ be a solution of (\ref{dual problem}) and let $u$ be a solution of the $q$-Laplacian. 
Then $\dif A$ is the Noetherian flux for the invariance of the $q$-Laplacian under addition of constants.
\end{corollary}
