...
Since (for almost every $y$) $\{u > y\}$ has locally finite perimeter, $\tau_y := \partial^* \{u > y\}$ is an exact current of integer multiplicity \cite[Theorem 14.3]{Simon84}, and in particular is integral.
...
By analogy with $\infty$-harmonic maps, one expects $\infty$-tight forms to be unique, solve the Euler-Lagrange equations (\ref{infinity Maxwell}), and have \dfn{absolutely best comass} in the sense that they have best comass in \emph{every} open subset of $M$ with smooth boundary.
