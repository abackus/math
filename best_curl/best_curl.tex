...
From our perspective, this is because $\rho^*$ is a flat subset of $\partial B$.
...
\end{align*}
Also recall that $B$ is strictly convex iff $\partial B$ has no flat subsets except singletons.
Then we have the following theorem, which summarizes \S\ref{convexity sec}:

\begin{mainthm}\label{convexity summary}
Let $B$ be the stable unit ball of a closed Riemannian manifold $M$ of dimension $\leq 7$.
Then:
\begin{enumerate}
\item Every flat subset of $\partial B$ is a polytope. \todo{Every flat is, but not every flat subset is a flat}
\item If $S$ is a flat subset of $\partial B$, and $\alpha, \beta \in S$, then $\alpha \cdot \beta = 0$.
\item If $M$ is homeomorphic to a torus, then $B$ is strictly convex.
\end{enumerate}
\end{mainthm}
...
Let $\rho \in H^{d - 1}(M, \RR)$ be a nonzero class, and let $F$ be its tight representative.
...
In order to construct certain laminations associated to calibrations of best comass, we must establish that if $N, N'$ are minimal hypersurfaces, then at the generic point of $N \cap N'$, $N$ is transverse to $N'$.
If the dimension of the underlying manifold $M$ is $d = 2$, then this is trivial, and if $d = 3$, then the structure of $N \cap N'$ is completely described by complex-analytic means \cite[Theorem 7.3]{colding2011course}, so the results of this section will mainly be interesting when $d \geq 4$.
...
However, we shall need this fact when $P$ is the set of points of tangency of $N, N'$.
Then $P$ turns out to not be a manifold, but the proof still works as long as we apply Alexander duality for sheaf cohomology.
...
It follows that the sheaf cohomology $\hat H^{d - 2}(P, \RR) = 0$.
...
So our strategy is to first construct the largest lamination $\lambda_F$ which $F$ calibrates, and take an intersection over all the $\lambda_F$s.
...
Then either $\lambda$ is a foliation, or $\lambda$ separates into finite number of clopen sublaminations, each of which is a parallel family of closed leaves or an exceptional indecomposable lamination.
\end{theorem}
\begin{proof}
This follows from \cite[Theorem I.3.2]{Morgan88}.
...
There exists a finite set $\rho^*_{\rm cl} \subseteq \rho^*$ such that for each leaf $N$ of $\lambda_\rho$, exactly one of the following holds:
\begin{enumerate}
\item $N$ is closed and $[N] \in \rho^*_{\rm cl}$.
\item $N$ is noncompact and $N$ is a leaf of an exceptional indecomposable measured stretch lamination $\kappa$ associated to $\rho$.
...
\end{proposition}
\begin{proof}
First suppose that there are infinitely many distinct homology classes $(\alpha_j)$ which are represented by a closed leaf $N_j$ of $\lambda_\rho$
Let $\kappa_j$ be the measured lamination with leaves $N_1, \dots, N_j$, which gives the leaf $N_i$ the weight $1/(2^j \Mass(N_i))$.
Then 
$$\Mass(\kappa_j) = \sum_{i \leq j} \Mass(N_i) \cdot \frac{1}{2^j \Mass(N_i)} = \sum_{i \leq j} 2^{-i} = 1 - 2^{-j}$$
and it follows by \todo{compactness of the weak topology} that $\kappa_j \to \kappa$ for some $\kappa$ in the weak topology of measures.
Moreover, $\kappa$ is a sublamination of $\lambda_\rho$ by \todo{limsup of the supports}.
By Theorem \ref{MorganShelan}, either $\kappa$ is a foliation or there exist finitely many parallel families $\zeta_1, \dots, \zeta_n$ of closed leaves such that for infinitely many $j$, $N_j$ is a leaf of some $\zeta_{i(j)}$.

If $\kappa$ is a foliation, then there exist infinitely many nohomologous closed leaves of $\kappa$, which by compactness of $M$ must all be sections of the normal bundle of some leaf $N$.
...
Therefore $\kappa$ is not a foliation, and there are two distinct closed leaves $N_j, N_{j'}$ which are part of the same parallel family, and hence are homologous.
But this is a contradiction, since $\alpha_j, \alpha_{j'}$ are linearly independent.

So we may let $\rho^*_{\rm cl}$ be the set of homology classes in $\rho^*$ which contain a closed leaf of $\lambda_\rho$.
If $N$ is a closed leaf of $\lambda_\rho$, then $N$ equipped with its Dirac measure is a measured lamination, calibrated by any tight representative of $\rho$; hence it is measured stretch for $\rho$, and $[N] \in \rho^*_{\rm cl}$.
...
By Theorem \ref{MorganShelan} and the fact that the leaves of a parallel family of closed leaves are all homologous, we may assume that $\zeta$ is a clopen parallel family of closed leaves, or is an exceptional indecomposable sublamination of $\kappa$.
...
Then there exists an indecomposable measured stretch sublamination of $\lambda_\rho$ representing $\alpha$.
\end{proposition}
\begin{proof}
...
\todo{Extend Loisel's algorithm, application to string theory}

%%%%%%%%%%%%%

\subsection{More general Riemannian manifolds}
For simplicity in this paper we have only dealt with closed manifolds of dimension $\leq 7$.
...
%%%%%%%%%%
\subsection{The canonical lamination}\label{canonical conjectures}
