...
The restriction to a cohomology class remains strictly convex, since cohomology classes are closed affine subspaces of $L^p$ and hence are themselves closed and convex. \todo{Orthogonality to the tight forms -- it's not just that the classes are convex, but that they're orthogonal}
\end{proof}
...
\todo{Use the homologically minimizing property and $L = K$ to show that the stable and costable norms are equivalent.
In fact, the suprema in the duality is witnessed by the best comass form and its minimal current.}
\cite[Theorem 3.8]{AUER20011095}
