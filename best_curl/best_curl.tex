...
We now choose $G$ so that $\|F - G\|_{L^1} \leq \delta \alpha^3$ for any $\delta > 0$, so that (after applying the claim with $\alpha = \frac{1}{m}$)
...
Taking $\delta \to 0$, we conclude Lemma \ref{PLDT}.
\end{proof}
