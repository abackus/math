...
&\lesssim (q - 1) t^{-d/2} \|\dif u_q\|_{L^q(U_\alpha)}^2.
\end{align*}
We now set $t_q := (q - 1)^{1/d}$, so that $(q - 1) t_q^{-d/2} = \sqrt{q - 1}$.
By the above considerations, there exists $C > 0$ which only depends on $M$ such that for any $1 < q \leq 2$,
\begin{align*}
\int_M \star |\dif u_q|^q 
&\leq \int_M \star |\dif u_q(t_q)| + C\left((q - 1)t_q^{-d/2} + t_q^{1/4}\right) \|\dif u_q\|_{L^q}^2 \\
