...
\item There exists a unique maximizer $\overline \eta \in \coker \Lambda$ of $-\hat I(-\eta)$.
...
The fact that $\overline \eta \in \coker \Lambda$ follows from the facts that $\overline \eta$ is a solution of $\mathscr P^*$, but any solution of $\mathscr P^*$ must be a member of $\ker \Lambda^* = \coker \Lambda$. 
...
Let $M$ be a closed oriented manifold with fundamental group $\Gamma$ and universal covering $\tilde M \to M$, and let $M_{\rm fun} \subset \tilde M$ be a fundamental domain of $\Gamma$.
...
$$J_p(F) := \frac{1}{p} \int_M \star |F|^p + \int_M \alpha \wedge F$$
...
The uniqueness of $u$ follows from the fact that $\ker \Lambda$ is the space of constant functions on $\tilde M$, and (\ref{strong duality}) follows from (\ref{abstract strong duality}).
\end{proof}

\todo{Solve this by inspection, motivated by DU20, and conclude that $F$ is the unique $p$-tight form in its cohomology class. Establish that the kernel of $\alpha \mapsto \rho$ is trivial, so we obtain an isomorphism between $H_{d - 1}$ and $H^{d - 1}$. Add Karen's example of a genus $2$ surface wrapping around a circle to explain the importance of $\Sph^1 \sim K(\ZZ, 1)$.}
