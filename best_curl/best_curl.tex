...
\begin{lemma}
Let $M$ be a compact manifold, $f_q \in L^q(M)$ positive, $\star f$ a positive Radon measure on $M$, and $\star f_q \to \star f$ in the weak topology of measures as $q \to 1$.
Then 
$$\int_M \star f = \lim_{q \to 1} \int_M \star f_q^q.$$
\end{lemma}
\begin{proof}
By H\"older's inequality, $f_q \in L^1(M)$, and in particulr, 
We first split up 
\begin{align*}
\left|\int_M \star f - \frac{1}{q} \int_M \star f_q^q\right|
&\leq \left|\int_M \star f - f_q\right| + \left|\int_M \star f_q - \frac{1}{q} \int_M \star f_q^q\right| \\
&=: I_1 + I_2.
\end{align*}
By the portmanteau theorem, $I_1 \to 0$ as $q \to 1$.
To bound $I_2$, we define for any $a \geq 0$, 
$$\psi(q) := \frac{a^q}{q} = a \sum_{n=0}^\infty \frac{(q - 1)^n}{n!} (\log a)^n.$$
implying by the mean value theorem that 
$$|\psi(q) - \psi(1)| \leq a (q - 1) (\log a - 1) +.$$
