...
\begin{proposition}
A $2$-form is $p$-light iff it solves the system
...
\end{cases}
\end{equation}
in the distributional sense. Moreover, $J_p$ is uniformly convex on a cohomology class.
\end{proposition}
\begin{proof}
...
By the Cauchy-Schwarz inequality and the fact that $p > 1$, one has 
$$(p - 2) \int_M \star |F|^{p - 4} \langle F, G\rangle^2 + \int_M \star |F|^{p - 2} |G|^2 > -\int_M \star |F|^{p - 2} |G|^2 + \int_M \star |F|^{p - 2} |G|^2 > 0$$
which implies the uniform convexity.
\end{proof}
...
since $F$ is $p$-light.
It follows that all expressions in the next proposition are well-defined.
...
We also compute 
$$\int_M \star |F|^p = \int_M \star |\dif u|^{(q - 1)p} = \int_M \star |\dif u|^q$$
and 
$$\int_M \dif u \wedge F = \int_M \dif u \wedge |\dif u|^{q - 2} \star \dif u = \int_M \star |\dif u|^q.$$
Thus both sides of (\ref{strong duality}) are equal to $\int_M \star |\dif u|^q$, and hence to each other.
\end{proof}
...
Let $\tilde M \to M$ be the universal cover, and let $(u_q)$ be a sequence of $\pi_1(M)$-equivariant functions on $\tilde M$ which converge in $L^1_\loc(\tilde M)$ to a function $u$.
...
By Poincar\'e's inequality, $(u_q)$ is then bounded in $W^{1, 1}_\loc(\tilde M) \cap L^\infty_\loc(\tilde M)$ as $q \to 1$, so it converges weakly along a subsequence to an element $u$ of the double dual $BV_\loc(\tilde M)$ of $W^{1, 1}_\loc(\tilde M)$, and also strongly in $L^r_\loc(\tilde M)$ for any $1 \leq r < \infty$.
In particular, $\dif u_q \to \dif u$ in the weak topology of measures.
