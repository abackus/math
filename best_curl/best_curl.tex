...
Here we are able to follow the strategy of \cite[\S6]{daskalopoulos2020transverse}: we show that the $q$-harmonic approximations to the $1$-harmonic dual of $F$ are exponentially small away from the best comass locus in the limit $p \to \infty$.
...
Since $H^{d - 1}(\Sigma, \RR) = 0$, there exist $A, A'$ such that $\dif A = F$ and $\dif A' = F'$, and such that $A, A'$ are in Coulomb gauge (that is, $\dif^* A = \dif^* A' = 0$).
Then 
$$|\Delta (A - A')| = |\dif^* (F - F')| \leq |\nabla (F - F')|,$$
so by elliptic regularity and Sobolev embedding ($p > d$),
...
We study the set on which a best comass representative of $\rho$ attains its comass, which turns out to contain a measured oriented minimal lamination which only depends on $\rho$.
...
One may associate a measured oriented geodesic lamination $\lambda$ associated to a homotopy class $\rho \in [M, N]$, where $M, N$ are closed hyperbolic surfaces of the same genus; we call $\lambda$ a \dfn{Thurston lamination}.
...
Let $F$ be a best comass representative of $\rho \in H^{d - 1}(M, \RR)$.
...
$$f(x, T) := |T^{\rm as}|_{g(x)}.$$
Working locally, we may write $F = \dif A$ for some $A$, which we may assume to be in Coulomb gauge and therefore $C^2$.
