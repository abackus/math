...
Let $M$ have the homotopy type of a torus.
Then the stable unit ball of $H_{d - 1}(M, \RR)$ is strictly convex.
\end{corollary}
\begin{proof}
...
\todo{What are other examples of manifolds such that the wedge product
$$H^1 \times H^1 \to H^2$$
is injective?}

\todo{If $\alpha \cdot \beta = 0$, can we find a metric on $M$ in which they're in the same flat?}
Probably yes. Make the bottlenecks for $\alpha, \beta$ really small compared to the rest of the manifold and show that the bottleneck for $\alpha + \beta$ is just the sum of the bottlenecks for $\alpha, \beta$.
Then if $\rho$ is dual to $\alpha + \beta$, it's also dual to $\alpha, \beta$.

What about the set of $\beta$ with $\alpha \cdot \beta = 0$? Can we find a flat that fits all of them at once?
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Rational points in homology}
\begin{lemma}
Suppose that $u$ is a function of locally least gradient.
Then on any generalized level set of $u$ which is not a component of $\partial \{u > y\}$ for some $y \in \RR$, $u$ attains a local minimum or maximum.
\end{lemma}

\todo{This should really be in paper 2 and worded more carefully for that setting.}

\begin{proof}
Let $N$ be a generalized level set on which $u$ does not attain a local minimum or maximum.
We may work in flow box coordinates, so $N = \{k_*\} \times J$ where $u(k, z) = f(k)$ for every $k \in K$ and $z \in J$ and some $f: K \to \RR$.
If $u$ does not attain an extremum along $N$, then neither does $f$ at $k$, hence there are $k_n^\pm \in K$ with $\pm f(k_n^\pm) > \pm f(k_*)$ and $k_n^\pm \to k_*$.
Level sets are dense in generalized level sets, so we may choose the $k_n^\pm$ so that $\{k_n^\pm\} \times J$ is a component of $\partial \{u > f(k_n^\pm)\}$.

We claim that $N$ is a component of $\partial \{u > f(k)\}$.
If not, then without loss of generality we may assume that $u \geq f(k_*)$ in a neighborhood of $N$, so there exists $\varepsilon > 0$ such that if $k \in (k_* - \varepsilon, k_* + \varepsilon)$, $f(k) \geq f(k_*)$.
But if $n$ is large enough then $k_n^- \in (k_* - \varepsilon, k_* + \varepsilon)$, so $f(k_n^-) > f(k_*) \geq f(k_n^-)$, a contradiction.
\end{proof}

\begin{proposition}
Let $M$ be a closed manifold and let $u: M \to \Sph^1$ be a topologically nontrivial least gradient map.
Then every leaf of $\lambda_u$ is a closed hypersurface.
\end{proposition}
\begin{proof}
Let $\alpha: \pi_1(M) \to \ZZ$ be the induced map on homotopy.
Then $\ker \alpha$ fits into a short exact sequence 
$$0 \to \ker \alpha \to \pi_1(M) \to \ZZ \to 0,$$
so by the Galois correspondence, there exists a covering space $p: \hat M \to M$ with $\Gal(\hat M, M) \cong \ZZ$
such that the lift of $u$ is topologically trivial:
$$\begin{tikzcd}
\hat M \arrow[r, "\hat u"] \arrow[d, "p"] & \RR \arrow[d] \\
M \arrow[r, "u"] & \Sph^1
\end{tikzcd}$$
In particular, it makes sense to take superlevel sets $\{\hat u > y\}$.
Moreover, there exists a generator $h$ of $\Gal(\hat M, M)$ which sends $\partial \{\hat u > y\}$ to $\partial \{\hat u > y + 1\}$ (so $\hat u$ does not have a global minimum or maximum).

Let $N$ be a (connected) leaf of $\lambda$, and let $\hat N$ be its preimage in $\hat M$.
Then $h$ sends $\hat N$ to itself.
Since $\hat u$ has no global minimum or maximum, and has least gradient, it follows from the maximum principle for least gradient maps that $\hat u$ has no local minimum or maximum \todo{Cite paper 2, formulating this for homological minimizers}.
So every leaf of $\lambda_{\hat u}$ is a level set $\partial \{\hat u > y\}$, and in particular $\hat N$ is a component of some $\partial \{\hat u > y\}$.

So if $\hat N$ meets $\partial \{u > y\}$, then for every $n \in \ZZ$, a component $K_n$ of $\hat N$ is a component of $\partial \{\hat u > y + n\}$.
Since $h$ acts on $\lambda_{\hat u}$ by sending $\partial \{\hat u > y\}$ to $\partial \{\hat u > y + 1\}$, $h$ acts on $\hat N$ by sending $K_n$ to $K_{n + 1}$.
In particular, $\hat N \cong N \times \ZZ$ and $p: \hat N \to N$ is projection onto the first factor. 
So $N \cong K_0$, and $K_0$ is a closed hypersurface in $\hat M$ by the regularity theorem.
\end{proof}
...
Let $\mathcal T_h$ be a quasiuniform triangulation of $M$ at scale $h$.
We iteratively define $\mathcal T_{h'}$ for $h' < h$ using barycentric subdivision.
We identify undirected weighted graphs with directed weighted graphs with edges going both ways, each with the same weight as the undirected edge.
The triangulation gives us a graph $G_h$ where the vertices are the $d$-simplices, and the edges are the $d - 1$-simplices, with weight $\Mass(e)$.
A closed $d - 1$-form $F$ is identified with the function $F_h$ on edges:
$$F_h(e) = \int_e F.$$
This gives a positive function on one of the directed edges and $0$ on the other.
If $\delta$ is a simplex, $h$ is much smaller than the topological scale, and $\partial \delta = \sum_i e_i$, then $\sum_i F_h(e_i) = 0$ by Stokes' theorem.
If $F$ calibrates $e$, then $F_h(e) = \Mass(e)$, which justifies that choice of weight.

Let $\alpha \in H_{d - 1}(M, \ZZ)$ and let $\hat M$ be the covering space which kills $\alpha$, so $\Gal(\hat M, M) \cong \ZZ$.
Then truncate $\hat M$ to get a manifold-with-boundary $\hat M_{\rm cut}$ with slightly more than a single copy of $M$ in it, if we do this generically the boundary components will not be minimal surfaces.
Choose one to be the source and the other to be the sink (since the Galois group is $\ZZ$ there's a source and a sink).
So, at least if we only care about integral classes, wlog $M$ has two boundary components, which are homologous to each other, and $M$ is compact.
Maybe we can approximate irrational classes by rational classes.

Choose $h$ much smaller than the curvature scale or topological scale.
By the Ford-Fulkerson algorithm we can find a max flow $F_h$ and a minimal cut $C_h$.
Use this data to make a simplicial chain $N_h$: the sum of the $d - 1$-simplices in $C_h$, oriented using $F_h$.
Taking $h \to 0$, we want to show that $N_h$ converges to a minimal lamination and $F_h$ converges to a best comass calibration.

\todo{To do this, we should estimate the mass of $N_h$ in terms of the stable norm, and show that if $h$ is smaller than the topological scale, then}
$$\Mass(N_h) \leq \Mass(\alpha) + o(1).$$

Side note: each simplex has mass $\sim h^d$ so there are $h^{-d}$ simplices.
...
\todo{Does lattice MFMC implies manifold MFMC?}
