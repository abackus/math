...
\begin{proposition}
Let $M$ be a closed manifold, let $\alpha \in H_{d - 1}(M, \RR)$ have rational direction, and let $u$ be an $\alpha$-equivariant function of least gradient on $\tilde M$.
...
By Poincar\'e duality and the Hurcewiz theorem, we may identify $\alpha$ with a homomorphism
$$\alpha: \pi_1(M) \to \ZZ,$$
or equivalently (since $\Sph^1$ is a $K(\ZZ, 1)$ space) a homotopy class of maps $M \to \Sph^1$.
Let $D$ be the degree of the homotopy class $\alpha$.
By $\alpha$-equivariance, we may view $u$ as a map\footnote{Note that $u$ is not continuous, so strictly speaking the homotopy class of $u$ is not $\alpha$; but by equivariance, it is profitable to view $u$ in this way.}
$$u: M \to \Sph^1.$$
Since $\ker \alpha$ fits into a short exact sequence 
$$0 \to \ker \alpha \to \pi_1(M) \to \ZZ \to 0,$$
by the Galois correspondence, there exists a covering space $p: \hat M \to M$ with $\Gal(\hat M, M) \cong \ZZ$
...
\todo{Define multiple periods, reduce, view triangulations as graphs...}

\begin{proposition}
Let $\hat M_{\rm cut}$ be a compact manifold-with-boundary with multiple periods and exactly two boundary components $S, T$.
Suppose further that $S, T$ are homologous.
Then, for each scale $0 < h \ll 1$, there exist quasiuniform triangulations $\mathcal T_h$ of $\hat M_{\rm cut}$ at scale $h$ such that the following holds:

Choose a maximal flow $f_h$ and minimal cut $c_h$ relative to $S, T$; let $F_h$ be the closed $L^\infty$ $d - 1$-form on $M$, and $N_h$ the closed minimal hypersurface, associated to $f_h$ and $c_h$ respectively.
Along a subsequence as $h \to 0$, $F_h \to F$ and $N_h \to N$ in \todo{what topologies?}, where $F$ has best comass, $N$ is a homologically minimizing closed minimal hypersurface, and $N$ is homologous to to $S$.
\end{proposition}
\begin{proof}
Let $\alpha$ be the homology class of $S$.
...
Let $N_h'$ be $h^{d - 1}$-minimizing, and let $\mathcal S_h$ be a quasiuniform triangulation of $N_h$ at scale $h$.
\todo{By using the $PL$ structure of $M$, and curvature bounds on $N_h$ to preserve the uniformity} we may choose a quasiuniform triangulation $\mathcal T_h$ of $M$ at scale $h$.

We claim that $N_h'$ arises from a cut $c_h'$ of $\mathcal T_h$ relative to $S, T$.
In fact, the chain $N_h' - S$ is exact, and so bounds an open set $U_h$.
Let $V_h := M \setminus \overline{U_h}$, so $\partial V_h = T - N_h'$.
Since $N_h'$ is covered by $d - 1$-simplices, $N_h'$ does not intersect the interior of any $d$-simplex $\Delta$ of $\mathcal T_h$, but $N_h'$ separates $U_h$ and $V_h$, so $\Delta$ cannot intersect both $U_h$ and $V_h$.
Therefore $c_h'$ is a cut.

By the (discrete) max flow/min cut theorem, there exist a maximal flow $f_h$ and minimal cut $c_h$ relative to $S, T$.
Since $N_h'$ is $h^{d - 1}$-minimizing,
$$\Mass(c_h) \leq \Mass(c_h') = \Mass(N_h') \leq (1 + h^{d - 1}) \Mass(\alpha).$$
\todo{Prove more stuff...}

Finally we claim that $N$ is a minimal hypersurface and $F$ has best comass.
\end{proof}
