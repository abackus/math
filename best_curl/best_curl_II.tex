...
We now discuss measured oriented laminations, which are laminations whose leaves are the ``level sets'' of a $1$-current, called the \dfn{Ruelle-Sullivan current}.
Since a current can be approximated by smooth $1$-forms in the weakstar topology, every current has a cohomology class $[T] \in H^1(M, \RR)$.
