...
\todo{Explain that the stable norm is the Thurston norm somewhere}
...
Given a measured lamination $\lambda$, there exists an open cover $\mathcal U := (U_a)_{a \in A}$ of $M$, which we call a \dfn{transverse cover}, such that for each $a \in A$, $\partial U_a$ is smooth and transverse to $\lambda$, and $\lambda_a := \lambda \cap \overline{U_a}$ is a parallel family of compact leaves.
For each $\alpha \subseteq A$, $\lambda_\alpha := \bigcap_{a \in \alpha} \lambda_a$ is a parallel family of compact leaves.
We let $w_\alpha$ be the measure of any transverse curve to $\lambda_\alpha$, and $N_\alpha$ a typical leaf of $\lambda_\alpha$.
...
and does not depend on the choice of transverse cover $\mathcal U$ \cite[Theorem I.5.3]{Morgan88}.
...
\end{lemma}
\begin{proof}
By the Thom transversality theorem, we may assume that $N$ meets $\partial B$ transversely. \todo{Make it rigorous}
