...
Let $M$ be a closed Riemannian manifold of dimension $d \geq 2$.
\dfn{Federer's stable norm} $\Mass(\sigma)$ of a homology class $\sigma \in H_{d - 1}(M, \RR)$ is the infimum of the mass $\Mass(N)$ over all cycles $N$ representing $\sigma$ \cite{gromov2007metric}.
Aside from its intrinsic interest, the stable norm has applications in computational geometry \cite{sullivan1990crystalline}, and is intimately related to the Thurston norm if $M$ is a closed hyperbolic $3$-fold \cite{Brock2017}.

We call the dual norm to $\Mass$ the \dfn{costable norm}, defined for $\rho \in H^{d - 1}(M, \RR)$ as \cite{Federer1974}
\begin{equation}\label{costable norm}
\Comass(\rho) := \inf_{[F] = \rho} \|F\|_{L^\infty}.
\end{equation}
We say that a representative of $\rho$ which attains the mimimum in (\ref{costable norm}) has \dfn{best comass}.
A convex duality argument (see \S\ref{fundamental theorem}) shows that if $F$ is a calibration on $M$ in the sense of Harvey and Lawson \cite{Harvey82}, and $F$ calibrates some hypersurface, then $F$ has best comass and the costable norm of $[F]$ is $1$.
...
In Thurston's work, $L$ is the Lipschitz constant of a given best Lipschitz map $f$ (which plays the role of $F$) and $K$ is the maximal amount that a geodesic lamination may be stretched by $f$.
In fact, every maximally stretched lamination is contained in a geodesic lamination called the \dfn{Thurston canonical lamination} \cite{Thurston98,Gu_ritaud_2017}.
...
To clarify the statement of Theorem \ref{existence of calibrated lam}, we stress that the hypersurfaces $N$ are not assumed closed, so it is possible that there exist best comass calibrations $F, G$, which are cohomologous, but such that $N$ is $F$-calibrated and not $G$-calibrated. \todo{Do we give an example?}
...
\subsection{Fundamental theorem of calibrated geometry}\label{fundamental theorem}
...
A closed $d - 1$-current $T$ is $F$-\dfn{calibrated} if 
$$\Mass(T) = \int_M T \wedge F.$$
A lamination $\lambda$ is \dfn{$F$-calibrated} if every leaf of $\lambda$ is $F$-calibrated.
\end{definition}

The fundamental theorem of calibrated geometry \cite{Harvey82} asserts that a calibrated current is homologically minimizing relative to its boundary (and hence minimal).
The argument is short and frequently useful to us, so we recall it.
Let $F$ be a calibration, $T$ an $F$-calibrated current, and $S$ a current homologous to $T$.
Then by Stokes' theorem,
\begin{equation}\label{calibrated geometry}
\Mass(T) = \int_M T \wedge F = \int_M S \wedge F \leq \Mass(S).
\end{equation}
In particular, every calibrated lamination $\lambda$ is minimal, and if $\lambda$ is measured oriented (so the homology class $[\lambda]$ is well-defined), $\lambda$ is homologically minimizing.
A dual argument to (\ref{calibrated geometry}) shows that most calibrations are ``ersatz'' in that they do not calibrate any currents at all:

\begin{proposition}
Let $F$ be a calibration.
If there exists an $F$-calibrated current, then $F$ has best comass.
\end{proposition}
\begin{proof}
Let $G$ be cohomologous to $F$, and let $T$ be an $F$-calibrated current.
Then by Stokes' theorem,
\begin{align*}
\Comass(F) &= 1 = \frac{1}{\Mass(T)} \int_M T \wedge F = \frac{1}{\Mass(T)} \int_M T \wedge G \leq \Comass(G). \qedhere
\end{align*}
\end{proof}

Henceforth we will mainly be interested in calibrations of best comass, and hence will mainly be interested in cohomology classes whose costable norm is $1$.
...
%%%%%%%%%%%%%%%%%%%
% \subsection{A Gauss-Bonnet formula}
% We now give a formula for the Euler characteristic of a measured stretch lamination.

% We first recall the definition of Euler characteristic of a measured lamination \cite[\S1.5]{Morgan88}.
% A \dfn{parallel family of compact leaves} a closed set $F$ which can be expressed as the union of compact hypersurfaces $N$, possibly with boundary, which are all graphs over each other.
% Therefore the hypersurfaces $N$ are all homeomorphic.
% Given a measured lamination $\lambda$, there exists an open cover $(U_a)_{a \in A}$ of $M$, which we call a \dfn{transverse cover}, such that for each $a \in A$, $\partial U_a$ is smooth and transverse to $\lambda$, and $\lambda_a := \lambda \cap \overline{U_a}$ is a parallel family of compact leaves.
% For each $\alpha \subseteq A$, $\lambda_\alpha := \bigcap_{a \in \alpha} \lambda_a$ is a parallel family of compact leaves.

% \begin{definition}
% Let $(U_a)_{a \in A}$ be a transverse cover of a measured lamination $\lambda$.
% For $\alpha \subseteq A$, let $w_\alpha$ be the measure of any transverse curve to $\lambda_\alpha$, and let $N_\alpha$ be a typical leaf of $\lambda_\alpha$.
% The \dfn{Euler characteristic} of $\lambda$ is defined by the inclusion-exclusion formula
% $$\Euler(\lambda) := \sum_{\alpha \subseteq A} (-1)^{\card \alpha - 1} w_\alpha \Euler(N_\alpha).$$
% \end{definition}

% The Euler characteristic $\Euler(\lambda)$ does not depend on the choice of transverse cover \cite[Theorem I.5.3]{Morgan88}.

% \todo{Redo this for $d = 5$, $d = 7$}

% \todo{Need a lemma which estimates the number of fundamental domains in an annulus of inner radius $R - o(R)$ and outer radius $R + o(R)$}

% \begin{proposition}[Gauss-Bonnet for laminations]
% Let $\kappa_u$ be the measured stretch lamination arising from a function $u$ of least gradient on $\tilde M$.
% Then for any $x \in \tilde M$,
% $$\Euler(\kappa_u) = \lim_{R \to \infty} \frac{\vol(M)}{2\pi \vol(B(x, R))} \int_{-\infty}^\infty \int_{\partial \{u > y\} \cap B(x, R)} K \dif \mathcal H^2 \dif y.$$
% \end{proposition}
% \begin{proof}
% If $\pi_1(M)$ is finite, then there are no measured stretch laminations on $M$; moreover, $\pi_1(M)$ is countable.
% So it is possible to tile $\tilde M$ by an infinite sequence $(F_n)$ of fundamental domains of $M$; we set $\tilde M_n := \bigcup_{m \leq n} F_m$.
% \todo{By the Thom transversality theorem, the fundamental domain can be chosen to be transverse to the universal covering lamination $\tilde \lambda$, and so that the generic leaf of $\tilde \lambda$ misses the singularities of the boundary of the fundamental domain.}
% We order the infinite sequence so that $x \in F_1$, and for $n \gg 1$, there exists $R > 0$ such that 
% $$B(x, R - o(1)) \subseteq \tilde M_n \subseteq B(x, R + o(1)).$$

% We cover $M$ by flow box charts $(U_a, K_a)_{a \in A}$, denote by $\mu_a$ the transverse measure on $K_a$, and denote by $\sigma_{ak}$ the leaf of $\lambda$ in $U_a$ with index $k \in K_a$.
% For $\alpha \subseteq A$ we similarly define $\mu_\alpha, K_\alpha, \sigma_{\alpha k}$, and let $\sigma_\alpha$ denote a typical leaf of $\lambda_\alpha$.
% Then, by the Gauss-Bonnet formula, 
% \begin{align*}
% \Euler(\lambda)
% &= \sum_{\alpha \subseteq A} (-1)^{\card \alpha - 1} \Euler(\lambda_\alpha) \\
% &= \sum_{\alpha \subseteq A} (-1)^{\card \alpha - 1} \mu_\alpha(K_\alpha) \Euler(\sigma_\alpha) \\
% &= \frac{1}{2\pi} \sum_{\alpha \subseteq A} (-1)^{\card \alpha - 1} \int_{K_\alpha} \left[\int_{\sigma_{\alpha k}} K \dif \mathcal H^2 + \int_{\partial \sigma_{\alpha k}} k \dif \mathcal H^1\right] \dif \mu_\alpha(k).
% \end{align*}
% If $S \subseteq M$, we write $S^m$ for the preimage of $S$ in the fundamental domain $F_m$.
% Then
% $$\partial \sigma_{\alpha k}^m = (\partial \sigma_{\alpha k})^m \cup (\sigma_{\alpha k}^m \cap \partial F_m)$$
% and, if $F_m$ is interior to $\partial \tilde M_n$, the integral along $\partial F_m$ will cancel with an integral along $\partial F_\ell$ for some other $\ell \leq n$.
% Meanwhile, the fact that the Euler characteristic is given by an inclusion-exclusion formula means that the integral along $(\partial \sigma_{\alpha k})^m$ will cancel with a similar such term.
% Similarly, all of the integrals along $\sigma_{\alpha k}$ cancel with each other, except for an integral over the entire leaf.
% In particular, for $n \geq 1$,
% \begin{align*}
% \Euler(\lambda)
% &= \frac{1}{2\pi n} \sum_{\alpha \subseteq A} (-1)^{\card \alpha - 1} \int_{K_\alpha} \left[\int_{\sigma_{\alpha k}^m} K \dif \mathcal H^2 + \int_{(\partial \sigma_{\alpha k})^m} k \dif \mathcal H^1\right] \dif \mu_\alpha(k) \\
% &= \frac{1}{2\pi n} \int_{-\infty}^\infty \left[\int_{\partial \{u > y\} \cap \tilde M_n} K \dif \mathcal H^2 + \int_{\partial \{u > y\} \cap \partial \tilde M_n} k \dif \mathcal H^1\right] \dif y.
% \end{align*}
% where the intersections $\partial \{u > y\} \cap \partial \tilde M_n$ are curves because the $F_m$ were chosen transverse to $\tilde \lambda$.

% To control the error terms, we first observe that \todo{for all but $o(1)$ tiles $F_m \subseteq \tilde M_n$, $\partial F_m$ does not meet $\partial \tilde M_n$}.
% Thus 
% \begin{align*}
% \frac{1}{n} \left|\int_{\partial \{u > y\} \cap \partial \tilde M_n} k \dif \mathcal H^1\right|
% & \leq \frac{4}{n} \card \{m: \partial F_m \cap \partial \tilde M_n \neq \emptyset\} \left|\int_{\partial \{u > y\} \cap \partial F_1} k \dif \mathcal H^1\right|
% \ll 1.
% \end{align*}
% \todo{This isn't quite right: we need to show that the geodesic curvature are bounded independently of $y$. This should follow from Gauss-Codazzi since $\partial \{u > y\}$ has bounded curvature and generically misses the singular points of $\partial F_1$.}
% \end{proof}
...
\todo{Look at Thurston's Figure 4 for ideas. Alternatively, find a $3$-manifold which admits a $2$-form which is positive on a distribution which is not everywhere integrable. If it doesn't work just state this as a conjecture.}
...
%%%%%%%%%%%%%%%%%
\subsection{Failure of global absolute minimality}
\todo{Call out post for Crandall}

\begin{lemma}\label{theta is infinity harmonic}
Let $M = (-h, h)_\varphi \times \Sph^1_\theta$ with a warped product metric 
\begin{equation}\label{warped product}
g := \dif \varphi^2 + f(\varphi) \dif \theta^2.
\end{equation}
Then $\theta: M \to \Sph^1$ is an $\infty$-harmonic map. 
\end{lemma}
\begin{proof}
Since the metric $g$ is warped, one can show that every Christoffel symbol of $g$ vanishes except for
\begin{equation}\label{Christoffel symbols of warped metric}
{\Gamma^\theta}_{\varphi \theta} = {\Gamma^\theta}_{\theta \varphi} = \frac{f'(\varphi)}{2 f(\varphi)}, \qquad {\Gamma^\varphi}_{\theta \theta} = -\frac{1}{2f'(\varphi)}.
\end{equation}
Using (\ref{Christoffel symbols of warped metric}), one can show that the Hessian tensor $\nabla (\dif \theta)$ is off-diagonal, and in particular its contraction $\Delta_\infty \theta$ with $\dif \theta \otimes \dif \theta$ vanishes, so
$$\theta : M \to \Sph^1$$
is an $\infty$-harmonic map.
\end{proof}
...
Liu also showed that if $d \geq 8$ and $c = 1$, then there exist smooth homologically minimizing hypersurfaces with no smooth calibration; the regularity of best comass calibrations remains open if $d \leq 7$ \todo{Look for more literature on this}.
