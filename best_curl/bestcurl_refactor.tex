...
Indeed, if every homology class could be represented by a smooth homologically minimizing cycle, then for any continuous best comass calibration $F$ (which could be viewed as a ``maximal flow''), it would follow from (\ref{Federer duality}) that $F$ would attain its $L^\infty$ norm on some minimal hypersurface $N$ (a ``minimal cut''), which would then ``bottleneck'' $F$ in the sense that 
$$\Mass(N) = \int_N F.$$
We refer to \cite[Figure 5]{Freedman_2016} for an enlightening visualization of this bottlenecking phenomenon.

Of course, in reality, not every homology class has a smooth homologically minimizing cycle.
In light of this fact, Bangert and Cui proved the following result which replaces the hypersurface by a lamination (see \S\ref{prevResults} for the definitions):
