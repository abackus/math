...
We can formulate these results in the language of differential forms, which is \emph{diffeomorphism-invariant}, so the proofs go through unchanged if $\Ball^d$ is replaced by a deformed ball; therefore Anzellotti theory holds for any deformed ball.\footnote{Strictly speaking, the $L^p$ and $\Mass$ norms depend on the metric, but every property used about these norms in \cite{Anzellotti1983} also holds in the presence of a Riemannian metric, with the same proof.}
...
So, by applying a partition of unity, Anzellotti theory holds for \emph{any} Riemannian manifold, since it holds for deformed balls.
...
If $U$ is a set of locally finite perimeter (thus $1_U \in BV_\loc$), we recall the \dfn{measure-theoretic boundary}, the set $\partial U$ of $x \in M$ such that for all sufficiently small $\varepsilon > 0$,
