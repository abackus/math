...
The \dfn{mass} $\Mass$ is the dual norm on the $1$-currents; notice that if a $1$-current $T$ represents a submanifold $\Sigma$ of codimension $1$, then $\Mass(T) = \vol(\Sigma)$.
A \dfn{calibration} is a closed $d - 1$-form $F$ such that $\Comass(F) = 1$, and a $1$-current $T$ is \dfn{$F$-calibrated} if 
$$\int_M T \wedge F = \Mass(T).$$
In that case, we think of $F$ as a ``certificate'' that $T$ minimizes its mass, for by Stokes' theorem, if $S$ is a $0$-current, then $\int_M \dif S \wedge F = 0$ and so
$$\Mass(T) = \int_M T \wedge F = \int_M (T + \dif S) \wedge F \leq \Mass(T + \dif S).$$
...
\begin{theorem}
Let $M$ be a closed oriented Riemannian manifold of dimension $2 \leq d \leq 7$, and let $\rho \in H^{d - 1}(M, \RR)$ be nonzero.
Then there is a tight $d - 1$-form $F$ which minimizes its comass in $\rho$, and a locally defined nonconstant function of least gradient $u$ such that 
$$\dif u \wedge F = \Comass(F) \star |\dif u|.$$
The level sets of $u$ are the leaves of a measured oriented lamination $\kappa$ whose leaves are calibrated by $F/\Comass(F)$.
Furthermore, if $\lambda$ ranges over measured oriented laminations, 
$$\sup_\lambda \frac{\langle \rho, [\lambda]\rangle}{\Mass(\lambda)} = \frac{\langle \rho, [\kappa]\rangle}{\Mass(\kappa)} = \Comass(F).$$
\end{theorem}

In a companion paper \cite{BackusBest2}, we turn to the analogue of Thurston's canonical maximally stretched lamination.
We shall associate every $\rho \in H^{d - 1}(M, \RR)$ of unit norm a lamination $\lambda_\rho$, such that every lamination calibrated by forms in $\rho$ embeds in $\lambda_\rho$.
...
Other definitions of $\infty$-harmonic maps are available \cite{Sheffield12}.
...
(Caveat lector: There are various definitions of ``$\infty$-harmonic map'' in the literature, for example \cite{Ou12}, which have nothing to do with the optimal Lipschitz extension problem.)
...
The \dfn{mass} $\Mass$ is the dual norm on the $d - k$-currents.
If $u$ is a function of bounded variation, then $\dif u$ is naturally a $1$-current defined by 
$$\int_M \dif u \wedge \varphi = -\int_M u \dif \varphi$$
for every smooth compactly supported $d - 1$-form $\varphi$.
As the indices are rather annoying to keep track of, we refer the reader to Appendix \ref{homological integration} for our conventions on currents.
...
Recall that the comass and mass induce norms on cohomology and homology.
...
In recent work \label{BackusCML}, I interpreted functions of least gradient in terms of their level sets, which form a lamination.
Though that paper was entirely local, \cite[Theorem B]{BackusCML} immediately implies:
...
Unless noted otherwise, $M$ shall denote a closed oriented Riemannian manifold, $\tilde M \to M$ shall be the universal covering, $M_{\rm fun} \subseteq \tilde M$ shalll be a fundamental domain of $M$, $\Gamma$ shall be $\pi_1(M)$.


%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Equivariant \texorpdfstring{$q$-harmonic}{q-harmonic} functions}
Let $\alpha \in \Hom(\Gamma, \RR)$ and $1 < q < \infty$.
As in Appendix \ref{equivariance}, we identify $\alpha$ with a class in $H^1(M, \RR)$.
...
\end{lemma}
\begin{proof}
Let $u_\infty$ be an $\alpha$-equivariant function which minimizes its Lipschitz constant $\|\dif u_\infty\|_{L^\infty}$ among all $\alpha$-equivariant functions. (For example, we can take $u_\infty$ to be the $\infty$-harmonic map constructed by Daskalopoulos and Uhlenbeck \cite[\S2]{daskalopoulos2020transverse}.)
...
$$\begin{cases}\dif F = 0 \\
	\dif^* (|F|^{p - 2} F) = 0.
\end{cases}$$
In Appendix \ref{duality derivation}, we motivate study of the PDE (\ref{p tight}) by deriving it from the Fenchel--Rockafellar duality theorem for convex optimization.
...
Recall that in the Bangert--Cui theorem, Theorem \ref{Bangert MFMC}, one needed to assume the existence of a continuous best comass calibration to construct a dual measured oriented calibrated lamination.
In general, it seems unlikely that a cohomology class of costable norm $1$ necessarily contains a continuous calibration, so we now consider laminations which are calibrated by measurable calibrations.

We work somewhat generally: we shall not need to assume that $M$ is closed or $d \leq 7$.
Let $F \in L^\infty(M, \Omega^{d - 1}_{\rm cl})$ be a closed form.
...
\item A closed $1$-current $T$ is \dfn{$F$-calibrated} if
$$\int_M T \wedge F = \Mass(T).$$
\end{enumerate}
\end{definition}
...
We first recall some terminology from the $L^\infty$ calculus of variations, reworded to be diffeomorphism-invariant; see \cite[\S5]{Barron08} for a motivation of the definition of absolute minimality.
...
Example \ref{integrability needed} shows that the simplest example of a contact manifold already yields a countereaxample.
...
If $\Sigma$ is a submanifold of codimension $\ell$, then we have a current $T_\Sigma$ defined by $\int_M T_\Sigma \wedge \varphi := \int_\Sigma \varphi$.
...
Suppose that $M$ is closed oriented, so we have the Poincar\'e duality map 
...
%%%%%%%%%%%%%%%%%%%%
\subsection{Equivariant functions}\label{equivariance}
Let $M$ be a closed oriented manifold with fundamental group $\Gamma$, and let $\alpha \in \Hom(\Gamma, \RR)$.
...
Then there is an $\alpha$-equivariant $q$-harmonic function $u$ on the universal cover $\tilde M$, which is unique modulo constants.
