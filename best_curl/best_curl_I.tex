...
We also explore the limiting dual problem as a model problem for the optimal Lipschitz extension problem.
...
As a byproduct, we collect some technical facts about $L^\infty$ calibrations of laminations which may be of independent interest.
...
This metric is intimately related to the structure of the geodesic laminations on $M$.
...
\item $\dif v$ is a Ruelle-Sullivan current\footnote{We define Ruelle-Sullivan currents in \S\ref{lamination intro sec}.} for $\lambda$.
\end{enumerate}
\end{theorem}

In Theorem \ref{Daskalopolous MFMC}, after normalizing $\Lip(u) = 1$, one can think of $\dif u$ as a calibration which calibrates the leaves of $\lambda$.\footnote{We review calibrated geometry in \S\ref{calibration intro sec}.}
...
Finally we recall the theorem of Bangert and Cui, which itself is a consequence of the theory of measured laminations developed by Auer and Bangert \cite{Auer01} to study the stable norm on $H_{d - 1}$.
The \dfn{costable norm}\footnote{Caveat lector: some authors, such as Massart \cite{Massart1997StableNO}, call the costable norm ``stable.'' We call it ``costable'' to emphasize that it is not the stable norm, but rather its dual.} of a class $\rho \in H^{d - 1}_{\rm dR}(M, \RR)$ satisfies
...
%%%%%%%%%%%%%%%%%%
\subsection{Motivation: Optimal Lipschitz extension problem}
...
The Daskalopolous--Uhlenbeck definition of ``$\infty$-harmonic map'' is a map which can be approximated weakly in $W^{1, r}$ by solutions of a generalized $p$-Laplacian, so they have little pointwise control of the limiting map.
Thus to construct the geodesic lamination, they must appeal to the theorem of Gu\'eritaud and Kassel \cite{Gu_ritaud_2017}.
...
Other definitions of $\infty$-harmonic (also known as tight) maps are available \cite{Sheffield12}.\footnote{Caveat lector: There are various definitions of ``$\infty$-harmonic morphism'' in the literature, for example \cite{Ou12}, which have nothing to do with the optimal Lipschitz extension problem.}
...
Preliminary work in this direction includes \cite{Barron2001,Katzourakis2018OnAV}.
...
Let $M$ be a closed oriented Riemannian manifold of dimension $d \geq 2$.
...
Since Theorem \ref{existence of infinity tight forms} implies that every calibration calibrates a function of least gradient, it complements Theorem \ref{Mazon MFMC}.
...
Theorem \ref{Thurston canonical} defines a \dfn{canonical maximally stretched lamination} $\mu(\rho, \sigma)$, which can be used to study the duality of the tangent and cotangent bundles of $\mathscr T_\gamma$ \cite[\S10]{Thurston98}.
...
I would also like to thank Bernd Kawohl for suggesting the references \cite{Kawohl2003, Grieser05}, Anatole Gaudin for suggesting the reference \cite{Costabel2010}, and Taylor Klotz for suggesting the reference \cite{Peralta_Salas_2023}.
...
\subsection{Deriving the dual equations}\label{duality derivation}
Throughout this section, let $M$ be a closed oriented Riemannian manifold with dimension $d \geq 2$, fundamental group $\Gamma$, and universal cover $\tilde M \to M$.

We begin by deriving the equation for $p$-tight forms, following \cite[Chapter IV]{Ekeland99}.
This can essentially be skipped by the hurried reader, but it motivates the proofs of the main results.
...
In this setting, we have the following form of the convex duality theorem, which is a far-reaching generalization of the max flow min cut theorem \cite{umesh2006algorithms}.
...
If $u$ is a $\Gamma$-equivariant $BV$ function on $\tilde M$, $\dif u$ drops to a current on $M$, which then has a homology class in $H_{d - 1}(M, \RR)$.
We write $[\dif u] = \alpha$ if that homology class is $\alpha$.
We identify $\alpha$ with the harmonic $1$-form on $M$ which represents its Poincar\'e dual.

\begin{proposition}[convex duality for the $q$-Laplacian]\label{mfmc qLaplacian}
Let $\alpha \in H_{d - 1}(M, \RR)$.
Then there is a solution of the $q$-Laplacian
$$\begin{cases}
\dif^*(|\dif u|^{q - 2} \dif u) = 0, \\
[\dif u] = \alpha,
\end{cases}$$
...
Let $u$ be a solution of the $q$-Laplacian with $[\dif u] = \alpha$, and let 
...
\end{cases}
\end{equation}
Moreover, $F$ satisfies (\ref{strong duality}) (since $\dif u$ is cohomologous to $\alpha$) so $F$ is the unique minimizer of $J_{p, \alpha}$.
Thus we have proven:

\begin{corollary}
Let $u, F$ be as in Proposition \ref{mfmc qLaplacian}.
Then $F$ is $p$-tight, and we have the pointwise duality relation (\ref{dual solution}).
\end{corollary}

Finally, let us observe that when $d = 2$, we can locally write $F = \dif v$ for a function $v$.
Then (\ref{dual solution}) is the equation for the conjugate $q$-harmonic that was studied by Daskalopolous and Uhlenbeck \cite[\S3]{daskalopoulos2020transverse}.
If, in addition, $p = q = 2$, then (\ref{dual solution}) is exactly the Cauchy-Riemann equation.
So it is reasonable to view (\ref{dual solution}) as a higher-dimensional and nonlinear generalization of the Cauchy-Riemann equation.


%%%%%%%%%%%%%%%%%
\subsection{Properties of \texorpdfstring{$p$-tight}{p-tight} forms}
We now begin the proof of Theorem \ref{existence of infinity tight forms} in earnest, by studying properties of $p$-tight forms.
As before, $M$ is a closed oriented Riemannian manifold; let $M_{\rm fun} \subseteq \tilde M$ denote a fundamental domain of $M$.
...
Motivated by the derivation in \S\ref{duality derivation}, we study the $q$-harmonic conjugate of a $p$-tight form, which is defined by ``dualizing'' the pointwise duality relation (\ref{dual solution}).
...
Then $u$ is called the \dfn{$q$-harmonic conjugate} of the $p$-tight form $F$, where $1/p + 1/q = 1$.
\end{definition}

Let $u$ be the $q$-harmonic conjugate of the $p$-tight form $F$.
Then $\dif u$ is well-defined as a closed $1$-form on $M$, so it is cohomologous to a harmonic $1$-form $\alpha$.
As usual we identify $\alpha$ with the homology class of its Poincar\'e dual, and we say that $u$ is \dfn{$\alpha$-equivariant}.
We record that 
...
Then $u$ is $q$-harmonic, $F$ satisfies (\ref{dual solution}), and we have (\ref{strong duality}).
\end{lemma}
\begin{proof}
...
We now estimate the energy of a $q$-harmonic function $u$ in terms of the stable norm of $[\dif u]$ and the geometry of $M$.
...
\todo{Write out the calibation argument that $u$ has least gradient}
\end{proof}
...
We call a Riemannian manifold $M$ a \dfn{deformed euclidean ball} if there is a bi-Lipschitz diffeomorphism $M \cong \Ball^d$.
We shall appeal to results of Anzellotti \cite{Anzellotti1983} which are carried out on $\Ball^d$.
We can formulate these results in the language of differential forms, which is diffeomorphism-invariant, so the proofs go through unchanged if $\Ball^d$ is replaced by a deformed euclidean ball.\footnote{Strictly speaking, the $L^p$ and $\Mass$ norms depend on the metric, but every property used about these norms in \cite{Anzellotti1983} also holds in the presence of a Riemannian metric, with the same proof.}
...
So, by covering a Riemannian manifold $M$ by deformed euclidean balls and applying a partition of unity, we see that Anzellotti theory holds for $M$.
...
Suppose that $U$ is a deformed euclidean ball, and let $F \in L^\infty(U, \Omega^\ell_{\rm cl})$.
