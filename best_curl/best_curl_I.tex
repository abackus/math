...
This approach cannot work if $N$ is a complete Riemannian manifold of dimension $\geq 2$.
As intimated by the use of viscosity solutions, the study of the $\infty$-Laplacian is based on the maximum principle, which is useless if $N$ is not $\RR$.
Daskalopolous and Uhlenbeck proposed to study ``$\infty$-harmonic maps'', which are weak limits of solutions of a certain generalized $p$-Laplacian as $p \to \infty$ \cite{daskalopoulos2022}.
One has little pointwise control of the $\infty$-harmonic map, so at this time it is not even known if $\infty$-harmonic maps locally minimize their Lipschitz constants.
...
These definitions emphasize local minimality of the Lipschitz constant, but it is not known if such maps even exist, nor what their relation with the Daskalopolous--Uhlenbeck $\infty$-harmonic maps is.
...
Motivated by Daskalopolous--Uhlenbeck $\infty$-harmonic maps, we study closed $d - 1$-forms which are weak limits of solutions of the generalized $p$-Laplacian 
...
For one, the problem of absolutely minimizing the $L^\infty$ norm is associated to the PDE
...
\end{cases}
\end{equation}
We derive (\ref{tight Einstein}) as a formal limit of (\ref{p tight}), not unlike the original derivation of the $\infty$-Laplacian \cite{Aronsson67}.
We then have:
...
If $F$ solves (\ref{tight Einstein}), then for every open $U$ with $H_{d - 1}(U, \RR) = 0$, $F|_U$ minimizes its $C^0$ norm among all closed forms of the same trace.
\end{enumerate}
\end{mainthm}

One can study nonsmooth tight forms by passing to the convex dual problem, which is the celebrated \dfn{least gradient problem}: the problem of minimizing the total variation $\int_M \star |\dif v|$ of a function $v$ \cite{gorny2024leastgradient}.
The convex dual problem for $\infty$-harmonic maps is a more complicated $1$-Laplacian system that has not been studied much \cite{daskalopoulos2022}.
...
Moreover, there exists a nonconstant function of least gradient $u$ on the universal cover $\tilde M$, such that $\dif u$ drops to $M$, the product of distributions $\dif u \wedge F$ is well-defined, and
\begin{equation}\label{max flow min cut}
\dif u \wedge F = \|F\|_{L^\infty} \star |\dif u|.
\end{equation}
\end{mainthm}

Since least gradient functions have a geometric interpretation as area-minimizing measured laminations \cite{BackusCML}, tight forms have an appealing interpretation as particularly well-behaved measurable calibrations.

The present paper suggests that, in the absence of a suitable generalization of viscosity solutions, the natural formulation of weak solution to an $L^\infty$ variational system should be based on its convex dual problem.
Conversely, weak formulations of $BV$ variational problems are often based on their dual problems, which are $L^\infty$ variational systems \cite{Mazon14,górny2022dualitybased}.
However, we identify two hurdles to overcome.
First, for the Dirichlet problem for tight forms, there are obstructions to the existence of a conjugate least gradient function.
Second, the PDE for smooth tight forms does not enjoy a suitable uniqueness theorem, and a construction motivated by contact geometry yields solutions which have nothing to do with the minimization problem whatsoever.
...
In our situation, while there may be many measured stretch laminations associated to $\rho$, we will embed them all in a \dfn{canonical calibrated lamination} $\lambda(\rho)$, as I will show in an upcoming companion paper \cite{BackusBest2}.
...
Finally, we record an easy corollary of Theorem \ref{lams are calibrated} and \cite[Theorem B]{BackusCML}:

\begin{corollary}
Let $M$ be a closed oriented Riemannian manifold of dimension $\leq 7$, and let $F$ be a calibration $d - 1$-form on $M$.
Let $\lambda$ be a measured oriented $F$-calibrated lamination on $M$.
...
%%%%%%%%%%%%%%%
\subsection{Homological integration}\label{homological integration}
...
A function $u$ has \dfn{bounded variation} if $\dif u$ is a $1$-current (of locally finite mass).
...
Then, $\dif u$ drops to a closed $1$-form on $M$, which we also call $\dif u$, and $[\dif u] = \alpha$.
...
Since $u$ is equivariant, $\tilde \lambda_u$ drops to a lamination $\lambda_u$ on $M$ such that $\Mass(\lambda_u) = \Mass(\dif u)$.
...
If we now let $v_\varepsilon$ be $\alpha$-equivariant with $\|\dif v_\varepsilon\|_{L^1} \leq \Mass(\PD(\alpha)) + \varepsilon$, we have by Stokes' theorem
\begin{align*}
...
Clearly $\lambda$ contains the left edge of $\partial M$, and intersects no other edge of $M$. \todo{This isn't actually so clear. Write it out carefully.}
...
We first recall some terminology from the $L^\infty$ calculus of variations, reworded to be diffeomorphism-invariant; see \cite[\S5]{Barron2001} for a motivation of the definition of absolute minimality.
...
We say that a section $u: M \to E$ \dfn{absolutely minimizes} $f$ in a class of open sets $\mathscr U$, if for every $U \in \mathscr U$, and every $v \in W^{1, \infty}(U, E)$ with zero trace,
$$\esssup_{x \in U} f(x, u(x), \nabla u(x)) \leq \esssup_{x \in U} f(x, u(x) + v(x), \nabla u(x) + \nabla v(x)).$$
We say that $u$ \dfn{absolutely minimizes} $f$, if $u$ absolutely minimizes $f$ in the class of all open subsets of $M$.
\end{definition}
...
\end{proposition}
\begin{proof}
Let $U$ be an open set with $H_{d - 1}(U, \RR) = 0$.
...
So it suffices to show that, if $x \in U$, then $|F(x)| \leq \|F\|_{C^0(\partial U)}$.
...
Example \ref{integrability needed} shows that the simplest example of a contact manifold already yields a counterexample.
...
%%%%%%%%%%%%%%%
\subsection{Sundries}
Here we record some miscellaneous results about Radon measures.
...
\[\begin{tikzcd}
	...
	\arrow["{\mathrm d}", from=1-1, to=1-3]
\end{tikzcd}\]
