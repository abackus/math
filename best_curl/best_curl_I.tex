...
For now, the reader can think of a \dfn{calibration} for a current $T$ as a differential form which is a ``certificate'' that $T$ minimizes its mass $\Mass(T)$.
...
$$\int_M \star |\dif u| = \int_M \dif u \wedge F.$$
\end{theorem}

If $M$ is a closed manifold, then the comass induces norms on the cohomology and homology of $M$.
The \dfn{costable norm} $\Comass(\rho)$ of $\rho \in H^k(M, \RR)$ is the infimum of the comass $\Comass(F)$ of any form $F$ representing $F$.
Its dual norm is the \dfn{stable norm} $\Mass$ on $H_k(M, \RR)$: the stable norm $\Mass(\psi)$ of $\psi \in H_k(M, \RR)$ is the infimum of mass $\Mass(T)$ of any current $T$ representing $\psi$.
These norms were introduced by Federer \cite{Federer1974} but the stable norm was named by Gromov \cite[Chapter 4]{gromov2007metric}.

Work of Auer and Bangert \cite{Auer01} and Massart \cite{Massart1997StableNO} shows that that the stable norm has a particularly rich duality theory in codimension $1$.
Auer and Bangert developed a theory of measured laminations in codimension $1$ to study the stable norm; that theory was then used by Bangert and Cui to prove:
...
Let $\Comass(F)$ be the comass of a form $F$, and dually let $\Mass(T)$ be the mass of a current $T$.
...
By Proposition \ref{integration is welldefined}, for every hypersurface $N \subset M$, $\int_N F$ is well-defined, and the proof of the fundamental theorem of calibrated geometry (Theorem \ref{calibrated theorem}) goes through without any changes.
...
By the fundamental theorem of calibrated geometry, Theorem \ref{calibrated theorem}, every calibrated current is homologically minimizing.
...
Thus, by the fundamental theorem of calibrated geometry (Theorem \ref{calibrated theorem}) and the contractibility of $U$, $N$ is absolutely area-minimizing in $U$, and hence must intersect $\partial U$ at some point $x_N$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Calibrated geometry}\label{calibrated appendix}
For a more detailed treatment, which emphasizes applications to special holonomy manifolds, see the original paper of Harvey and Lawson \cite{Harvey82}.
...
If $F$ is a calibration, then a current $T$ is \dfn{$F$-calibrated} if
$$\int_T F = \Mass(T).$$

\begin{theorem}[fundamental theorem of calibrated geometry]\label{calibrated theorem}
Let $F$ be a calibration $k$-form and $T$ an $F$-calibrated $k$-current. Then:
\begin{enumerate}
\item For every $k + 1$-current $S$, $\Mass(T) \leq \Mass(T + \partial S)$.
\item If $\partial T = 0$, then for every $k - 1$-form $A$, $\Comass(F) \leq \Comass(F + \dif A)$.
\end{enumerate}
\end{theorem}
\begin{proof}
By Stokes' theorem, $\int_{\partial S} F = 0$, so
$$\Mass(T) = \int_T F = \int_{T + \partial S} F \leq \Mass(T + \partial S).$$
The proof that $F$ minimizes its comass is similar.
\end{proof}
