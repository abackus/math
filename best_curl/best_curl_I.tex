...
For one, can study nonsmooth tight forms by passing to the convex dual problem, which is the celebrated \dfn{least gradient problem}: the problem of minimizing the total variation $\int_M \star |\dif v|$ of a function $v$ \cite{gorny2024leastgradient}.
...
Second, the PDE (\ref{tight Einstein}) does not enjoy a suitable uniqueness theorem, and a construction motivated by contact geometry yields solutions which have nothing to do with the minimization problem whatsoever, so that the involutivity hypothesis in Theorem \ref{tight are absolute minimizers} cannot be avoided.
...
As a sample application of Theorem \ref{lams are calibrated}, we prove:
...
In a companion paper \cite{BackusBest2}, I will make the analogy between Theorem \ref{lams are calibrated} and Thurston's approach to Teichm\"uller theory even stronger.
...
The superlevel sets of the first eigenfunction are \dfn{Cheeger sets} -- sets $U$ which maximize the isoperimetric ratio $\vol_{d - 1}(\partial U)/\vol_d(U)$ \cite{Kawohl2003}.
The dual $d - 1$-forms $F$ are not closed, but instead satisfy $\dif F \geq 0$ \cite{Grieser05}.
It would be interesting to study the $L^\infty$ one-sided variational problem that $F$ solves in this setting, and the $L^\infty$ dual problems to $BV$ eigenvalue problems more generally.
...
I would like to thank Georgios Daskalopoulos and Karen Uhlenbeck for helpful discussions and for providing me with a draft copy of \cite{daskalopoulos2023}, Taylor Klotz for helpful discussions and suggesting the reference \cite{Peralta_Salas_2023}, Trent Lucas for helpful discussions, Bernd Kawohl for suggesting the references \cite{Kawohl2003,Grieser05}, and Anatole Gaudin for suggesting the reference \cite{Costabel2010}.
...
The computations in the above construction were facilitated by the failure of $M$ to be strictly convex.
The astute reader may object that the least gradient problem is ill-behaved on domains which are not strictly convex \cite{Sternberg1992}, but in the above example, after constructing $F$, we may replace $M$ by the ball of unit diameter centered on $(1/2, 1/2)$, and observe the same phenomenon: the key point is that $|F|$ does not attain its maximum on the interior of $M$.
...
We first show that the PDE (\ref{infty Max}) has a simple geometric interpretation, which generalizes the interpretation of the $\infty$-Laplace equation as asserting that the gradient curves of an $\infty$-harmonic function are lines.
