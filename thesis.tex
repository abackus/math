...
\end{definition}
We will need two facts about the indicator function of a function of completely regular growth, and we omit their proof. They can be found in \cite[Chapter III]{levin1964distribution}.
\begin{theorem}
...
\end{definition}
When we refer to the indicator function $h$, the counting function $N$, and the auxiliary function $s$ that relates $h$ and $N$, we will always do so in the context of the associated entire function $F$ (so, e.g., $N$ counts the zeroes of $F$ in a sector).
