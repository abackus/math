...
\begin{lemma}
  \label{order of vhat}
Let $\rho$ be the order of $\hat V$. Then $\rho \geq 1$, and $\rho = 1$ if and only if $V$ has compact support.
\end{lemma}
\begin{proof}
Suppose that $\rho < 1$, and let $\varepsilon > 0$. By assumption, $\hat V(z) = o(\exp(\varepsilon |z|)$. Let $z = \alpha + i\beta$; then we have 
$$|\hat V(z)| \leq \int_{-\infty}^\infty |V(x)|e^{x\beta} ~dx.$$
...
\begin{definition}
Let $R > 0$ and let $\theta,\varphi$ be angles. The \dfn{angular counting function} $N(R, \theta, \varphi)$ is the number of resonances $\lambda$ of $V$ such that $\arg \lambda \in (\theta, \varphi)$ and $|\lambda| < R$.
\end{definition}
\begin{conjecture}[Froese's conjecture]
The angular counting function satisfies
$$N(R, \theta, \varphi) = \frac{s(\theta, \varphi)}{2\pi \rho} R^\rho + o(R^\rho).$$
Moreover, the asymptotic distribution of resonances is identical to the asymptotic distribution of zeroes of $\lambda \mapsto \hat V(2\lambda)\hat V(-2\lambda) + 1$.
\end{conjecture}
Froese was able to prove his conjecture \cite[Theorem 1.3]{froese1997asymptotic} for potentials which meet the following hypothesis.
\begin{definition}
Suppose that $\hat V$ has order $\rho > 1$ and is of completely regular growth. Suppose that... TODO Then we say that $V$ satisfies \dfn{Froese's criterion}.
\end{definition}
...
If $V$ is not compactly supported and satisfies Froese's conjecture, then the Breit-Wigner series $S_V$ is not absolutely convergent.
\end{theorem}
...
Let $\rho$ be the order of $\hat V$. By the Paley-Weiner theorem,
\begin{lemma}
Suppose that $V$ meets the hypotheses of Theorem \ref{conditional convergence of breit wigner}.
\end{lemma}
\begin{proof}
Suppose that there are infinitely many real resonances of $V$. Then there are infinitely many real solutions $\xi$ to the equation
$$\hat V(2\xi) \hat V(-2\xi) + 1 = 0,$$
by Froese's conjecture. These solutions cannot cluster, since $\lambda \mapsto \hat V(2\lambda)\hat V(-2\lambda) + 1$ is a nonzero entire function (since $V$ is super-exponentially decreasing and nonzero). Without loss of generality, we may assume that there are infinitely many positive solutions $\xi$; so we can find a sequence $\xi_j \to \infty$ of solutions. In the weakstar topology, we have
$$-1 = \lim_{j \to \infty} \int_{-\infty}^\infty V(x) e^{-2i\xi_jx} ~dx \int_{-\infty}^\infty V(x)e^{2i\xi_jx} ~dx = 0$$
by the Riemann-Lebesgue lemma. (NOTE: One of these goes to $1$, the other goes to $0$. I should be clear which is which.) This is a contradiction.
\end{proof}
