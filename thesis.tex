...
We are interested in generalizing the Breit-Wigner formula to non-compactly supported potentials. Formally, we write
$$\frac{1}{2\pi i}\tr S'(\lambda)S(\lambda)^* = \infty - \frac{1}{\pi} \sum_{\kappa \in P} \frac{\Im \kappa}{|\kappa - \lambda|^2}$$
which would imply that the sum over resonances diverges. Of course, our definition of the scattering matrix, or even of the set of resonances $P$, relied on the notion of an outgoing wave, which makes no sense if $\supp V$ is not compact.
...
Let $R_0(\lambda) = (D_x^2 - \lambda^2)^{-1}$ be the free resolvent, as in the case of compactly supported potentials.
\begin{definition}
The \dfn{potential-weighted resolvent} $W_V(\lambda)$ is defined by
$$W_V(\lambda) = \sqrt VR_0(\lambda)|\sqrt V|.$$
\end{definition}

Let $B^1(L^2(\RR))$ denote the space of trace-class operators on $L^2(\RR)$.
\begin{lemma}
Suppose that $V$ is super-exponentially decreasing. Then $W_V$ extends to a meromorphic family of trace-class operators on $\CC$, with only a single pole at $0$.
\end{lemma}
\begin{proof}
TODO
\end{proof}
TODO: Use Froese's construction of the resonant set $P$.
