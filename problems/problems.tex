...
This is probably too hard, but at least try to formula the idea that this holds under an ``energy evacuation'' hypothesis.
\end{problem}
...
A more general question, which is probably easier, is to show that under mild hypotheses there is a a large class of calibrations on $M$, even though these calibrations are for submanifolds of codimension $4 > 1$ (so we cannot use the $\infty$-Laplacian, since comass does not equal $L^\infty$).
\end{problem}
