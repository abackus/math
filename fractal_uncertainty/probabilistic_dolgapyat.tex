...
% \begin{lemma}
% Let $\xi \in (0, 1)$. Then if $L \gg 1$ then
% $$\Pr(p_a \gtrsim L^{-\delta}) \Pr(q_b \gtrsim L^{-\delta'}) \geq 1 - \xi.$$
% \end{lemma}

% This lemma is believable because it basically says that for $L$ large, it shouldn't be possible for $X \cap I$ to be spread out evenly among $\gg L^\delta$ many children $I_a$.
% I don't know how to prove it though.

% We assume that
% $$\Expect ||F_J||_{C_\theta(I_a)}^2 \geq (1 - \varepsilon_1)R$$
% for some $\varepsilon_1 > 0$ depending on $L^\delta, L^{\delta'}$ such that
% $$\Pr(p_a \geq \sqrt{\varepsilon_1}) \Pr(q_b \geq \sqrt{\varepsilon_1}) \geq 1 - \xi.$$
