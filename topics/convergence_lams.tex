...
\subsection{Compactness}
\begin{definition}
A set $\Lambda$ of laminations is \dfn{tight} if for every sequence $(\lambda_i)$ in $\Lambda$ and $x_i \in \supp \lambda_i$, there is a subsequence of $(x_i)$ which converges in $M$.
\end{definition}
...
Then if we set $u_{ij} := f_{i,j+1} - f_{ij}$, then $u_{ij} > 0$ by the maximum principle, and we get a second-order operator $P_{ij}(\varepsilon)$ such that
$$-\Delta u_{ij} = P_{ij}(\varepsilon) u_{ij}$$
(TODO: Use \cite[\S1.1, \S7.1]{colding2011course} and Gilbarg--Trudinger to derive this)
where $P_{ij}(\varepsilon)$ is perturbative as $\varepsilon \to 0$, in the sense that for $0 < \delta < 1/2$,
$$\sup_{B_{\delta r}} u_{ij} \leq e^{c_1(\varepsilon) \delta^{c_2(\varepsilon)}} \inf_{B_{\delta r}} u_{ij}$$
where $c_i(\varepsilon) \to c_i > 0$, given by the Harnack inequality for $-\Delta$.
In paticular, we now choose $\varepsilon$ small enough that for $\delta < 1$,
\begin{equation}\label{Harnack bound}
	\sup_{B_{\delta r}} u_{ij} \leq e^{2 c_1\delta^{c_2}} \inf_{B_{\delta r}} u_{ij}.
\end{equation}
...
$$\left|\left|\dif f_{ij} + \frac{\eta_i - \eta_{ij}}{\eta_{i,j+1} - \eta_{ij}} \dif u_{ij}\right|\right|_{C^0(B_r)} \leq 3\sup_{j' \in \{1, \dots, k_i\}} ||\dif f_{ij'}||_{C^0(B_r)} \lesssim r.$$
...
\end{theorem}
\begin{proof}
For each $i$, let $(\lambda_{ij})_j$ be finite sublaminations of $\lambda_i$ converging to $\lambda_i$ on the level of flow boxes.
...
Indeed, as geodesic laminations, they have bounded curvature, and it is clear from construction that they are tight.
