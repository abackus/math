...
\begin{proposition}\label{regularity of limits}
Suppose that $M$ has constant sectional curvature and $2 \leq d \leq 7$.
Let $(\lambda_i)$ be a sequence of minimal laminations such that $\lambda_i \to \lambda$ in Thurston's geometric topology.
Then $\lambda$ is minimal.
\end{proposition}

Here the hypothesis of constant sectional curvature is technical, and made so that we may appeal to the theory of the prequel paper \cite{BackusFLG}.
...
We shall later show that this definition is equivalent for minimal laminations to the following extension of Thurston's definition:
...
\begin{proposition}\label{equivalence of flow box convergence}
A sequence $(\lambda_i)$ of minimal laminations flow-box converges in $X$ to $\lambda$, witnessed by laminar atlases $(F_\alpha^i, K_\alpha^i)$ and $(F_\alpha, K_\alpha)$, iff the sets of leaves $K_\alpha^i$ converge in Hausdorff distance to $K_\alpha$.
\end{proposition}
...
We shall later show that the convergence of Ruelle-Sullivan currents, which is very convenient to work with analytically, is equivalent to a definition of measure convergence that may be more familiar to topologists:

\begin{proposition}\label{equivalence of measure convergence}
A sequence of measured laminations $(\lambda_i, \mu_i)$ converges to $(\lambda, \mu)$ iff:
\begin{enumerate}
\item Every curve $\gamma$ which is transverse to $\lambda$ is eventually transverse to $\lambda_i$ for every sufficiently large $i$ depending on $\gamma$, and
\item For every curve $\gamma$ which is transverse to $\lambda$, $\mu_i|_\gamma \to \mu|_\gamma$ in the weak topology of measures.
\end{enumerate}
\end{proposition}
