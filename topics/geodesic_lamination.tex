...
\begin{proposition}[Hilbert's 19th problem]\label{regularity of reduced boundary}
Let $U$ be a set of least perimeter with conormal $1$-form $\normal$.
If $\normal$ is continuous, then $\partial U$ is as smooth as possible.
\end{proposition}
\begin{proof}
According to Proposition \ref{locality of Caccioppoli}, there exists a local $C^1$ parametrization $f: \RR^{d - 1} \to M$ of $\partial U$, so that $f$ is a minimizer of the Lagrangian $\mathscr L$ defined by (\ref{Lagrangian density}).
Since $\mathscr L$ is a regular Lagrangian in the sense of Hilbert's 19th problem, $f$ is analytic \cite{morrey2009multiple}.
\end{proof}
...
\subsection{Estimates on approximate Laplacians}
...
$$P = -\Div A \nabla + b \cdot \nabla + c$$
such that $A(0) = 1$, $A$ is symmetric, and $b(0) = c(0) = 0$.
\end{assumption}

By the spectral theorem, the assumption $A(0) = 1$ can always be achieved at the cost of a diffeomorphism.
...
To control the error term, we observe that $du$ solves a PDE $Qdu = 0$ which is a perturbation of the Laplace equation (since if $v$ is harmonic then so is $dv$). TODO: Compute $Q$ and show that it is uniformly elliptic.
So by Assumption \ref{nice elliptic operator}, if $\rho$ is small, then $Q$ is uniformly elliptic, so by boundary Schauder estimates and elliptic regularity,
$$||du||_{L^2(B_{2.6\rho})}^2 \lesssim ||Tdu||_{L^2(\partial B_{2.6\rho})}^2 \leq ||Tdu||_{L^2(\partial(B_{3\rho} \setminus B_{2.6\rho}))}^2 \lesssim ||u||_{L^2(B_{3\rho} \setminus B_{2.6\rho})}^2.$$
TODO: Fill in the details and add in the right scaling here.

We have shown that TODO: Determine $N$
$$||du||_{L^2(B_\rho)}^2 \leq 2^{-\ell-2}||du||_{L^2(B_{2\rho})}^2 + O(\rho^N) ||du||_{L^2(B_{3\rho} \setminus B_{2\rho})}^2$$
which is sufficient if $\rho$ is chosen so small that $O(\rho^N) < 2^{-\ell-2}$.
\end{proof}
