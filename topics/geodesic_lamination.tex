...
\item $\Lambda(U, x, \rho) = 0$ iff $df = 0$ identically.
\item The map $\tilde f: \DD^{d - 1} \to \DD^{d - 1} \times J$ given by $\tilde f(y) = (y, f(y))$ realizes $N$ as an embedding of $\DD^{d - 1}$.
\end{enumerate}

\begin{lemma}
Assume that $f,N$ are related as above.
There exists $\delta > 0$ and a Lagrangian density $\mathscr L$ on $\DD^{d - 1}$ which is as smooth as possible, such that if $||df||_{L^\infty} < \delta$, then $|N \cap V| = \int_{\DD^{d - 1}} \mathscr L$
and
$$\mathscr L = (1 + O(|y|^2))\sqrt{1 + \sum_{i < d} f_{,i}^2(y)} ~dy_1 \wedge \cdots \wedge dy_{d - 1}$$
where $y$ is the usual coordinate on $\DD^{d - 1}$.
\end{lemma}
\begin{proof}
...
Using the approximately euclidean nature of $(y, z)$ and the fact that the coordinates on $N$ induced by $\tilde f$ are comparable to $(y, z)$ (which follows because $df \in L^\infty$), we see that the coordinates induced by $\tilde f$ are approximately euclidean and hence
$$\tilde g_{ij}(f) = \delta_{ij} + f_{,i}f_{,j} + O(|y|^2(1 + ||df||_{L^\infty} + ||df||_{L^\infty}^2)).$$
We can thus introduce the Lagrangian density
$$\mathscr L = \sqrt{|\det \tilde g(f)_y|} ~dy_1 \wedge \cdots \wedge dy_{d - 1}$$
which satisfies $|N \cap V| = \int_{\DD^{d - 1}} \mathscr L$.

If we set $a_{ij} = \delta_{ij} + f_{,i}f_{,j}$ then $a$ is invertible if $||df||_{L^\infty}$ is small, in which case the formula
$$\det \tilde g(f) = \det (I + O(|y|^2)B^{-1}) \det B$$
induces the Taylor expansion
$$\det \tilde g(f) = (1 + O(|y|^2 \tr B^{-1})) \det B.$$
But $\sqrt{\det B}$ is exactly what the volume form on $N$ would be if $g_{ij} = \delta_{ij}$, thus $\det B = 1 + \sum_i f_{,i}^2$.

To bound the error term, we assume that $||df||_{L^\infty}$ is so small that $\tr B^{-1} \leq 2$, and then Taylor expand $\sqrt\cdot$ about $1$ to get
\begin{align*}
\sqrt{\det \tilde g(f)} &= \sqrt{\det B}(1 + O(|y|^2)). \qedhere
\end{align*}
\end{proof}

We can always impose $||df||_{L^\infty} < \delta$ by shrinking $V$, since $f \in C^1$ has a critical point at the origin.
Linearizing $\mathscr L$ about $||df||_{L^\infty} = y = 0$, then $\mathscr L$ is approximated by the Dirichlet energy
$$\mathscr I = \left(1 + \frac{|df_y|^2}{2}\right) ~dy_1 \wedge \cdots dy_{d - 1}$$
of $f$. Here the norm $|\cdot|$ is meant in the sense of the euclidean norms on $\DD^{d - 1}$ and $J$.
In particular, the Euler-Lagrange equation for $\mathscr L$ is a uniformly elliptic quasilinear equation, and we have:
...
In each such trivialization, $\partial U$ is the graph of a $C^1$ function which solves the Euler-Lagrange equation $Pf = 0$ for $\mathscr L$.
After shrinking the trivialization suitably, the quasilinear operator $P$ becomes uniformly elliptic with coefficients which are as smooth as possible, so de Giorgi--Nash--Moser theory implies that $f$ is as smooth as possible \cite{morrey2009multiple}.
\end{proof}
