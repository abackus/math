...
\begin{align}\label{minimal surface lagrangian}
\mathscr L[f] &= \sqrt{1 + S_{ii} + 2 f_{,i} S_{id} + |df|^2 + |df|^2 S_{dd} + f_{,i} f_{,j} S_{ij} + \varepsilon_6} ~dy, \\
S_{ij} &= -\frac{1}{3} R_{i \mu \nu j} y^\mu y^\nu, \\
\label{error 6}
|\varepsilon_6| &\lesssim |y|^3 + |f(y)|^3 + |df_y|^6.
\end{align}
We now linearize $\mathscr L$ about $df = 0$. Since $S$ depends on $f$ but not $df$, we employ (\ref{error 6}) to compute
\begin{align}\label{linear lagrangian}
\mathscr L[f] &= \left(\sqrt{1 + S_{ii}} + \frac{f_{,i} S_{id} + |df|^2 + |df|^2 S_{dd} + f_{,i} f_{,j} S_{ij}}{2 \sqrt{1 + S_{ii}}} + \varepsilon_7\right) ~dy\\
|\varepsilon_7| &\lesssim |y|^3 + |f(y)|^3 + |df_y|^4.
\end{align}
We call the principal part
$$\mathscr I[f] = \left(\sqrt{1 + S_{ii}} + \frac{f_{,i} S_{id} + |df|^2 + |df|^2 S_{dd} + f_{,i} f_{,j} S_{ij}}{2 \sqrt{1 + S_{ii}}}\right) ~dy$$
the \dfn{modified Dirichlet energy} of $f$. Indeed, if $R_{i \mu \nu j} = 0$, then $\mathscr I[f]$ is minimized exactly by the harmonic functions.
...
Using the Taylor expansion of $\sqrt \cdot$, we can approximate $\mathscr L[f]$ by the \dfn{modified Dirichlet energy}, which schematically of the form
$$\mathscr I[f] = \left(1 + \frac{|df_y|^2}{2} + O(|y|^2 + |f(y)|^2)(1 + |df_y|)\right) ~dy_1 \wedge \cdots \wedge dy_{d - 1}$$
whenever $f$ is small in $W^{1, \infty}$.
Minimizers of $\mathscr I[f]$ solve a semilinear Euler-Lagrange equation which is of the form
\begin{equation}\label{semilinear EL}
\Delta f(y) + O(|f(y)|(1 + |df_y|)) + \Div O(|y|^2 + |f(y)|^2) = 0.
\end{equation}
To top order, (\ref{semilinear EL}) is just the Laplace equation, so (\ref{semilinear EL}) has a good regularity theory.

\begin{lemma}\label{regularity of reduced boundary}
...
In each such trivialization, $\partial U$ is the graph of a $C^1$ function $f$ with a double zero at $0$, which minimizes $\mathscr L[f]$, and we can make $||f||_{W^{1, \infty}}$ as small as we want by shrinking its domain to a small neighborhood of $0$.
So de Giorgi--Nash--Moser theory implies that $f$ is as smooth as possible \cite{morrey2009multiple}.
\end{proof}

We are interested in the case that $\Lambda(U, x, \rho)$ and $\rho$ are small and $f(0) = 0$.
In that case, the lower-order terms in (\ref{semilinear EL}) become vanishingly small, and so solutions to (\ref{semilinear EL}) should be approximated well by solutions to the Laplace equation.
We now use this fact to prove a bootstrapping lemma for minimizers of $\mathscr L$.

\begin{definition}
Let $f \in C^1(B_\rho)$. The \dfn{harmonic approximant} $Hf$ to $f$ is the function which solves the Dirichlet problem
\begin{align}
\label{Hf is harmonic} \Delta Hf &= 0, \\
\label{Hf boundary data} Hf|\partial B_\rho &= f|\partial B_\rho
\end{align}
on $B_\rho$.
\end{definition}
