...
Theorem \ref{main thm} generalizes the maximum principle stated by G\'orny \cite[Proposition 3.4, Corollary 3.5]{górny2017planar}; in addition to Theorems \ref{main lma} and \ref{monotonicity prestate} we will need some standard facts about functions of least gradient that we will review in \S\ref{LeastGradientFunctions} but for now can be viewed as black boxes.    
...
We prove it in \S\ref{GornySec}.

\begin{theorem}[G\'orny decomposition]\label{Gorny regularity}
Let $\overline M$ be an compact convex surface-with-boundary of dimension $\leq 7$ and constant scalar curvature $\leq 0$.
