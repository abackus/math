...
\end{equation}
and for every $\omega \in C^r(\Omega)$, $r \geq 1$, such that $|\dif \omega| \lesssim 1$, the \dfn{Plateau energy}
\begin{equation}\label{Plateau energy}
\Lagrange[\omega, \dif \omega] := e^{O(|x|^2 + |\omega(x)|^2)} \sqrt{1 + |\dif \omega(x)|^2_{(\slashed g^{(\omega(x))})^{-1}(x)}} \dif x^1 \wedge \cdots \wedge \dif x^{d - 1}
\end{equation}
