...
\frac{\dif \varphi}{|\dif \varphi|} \geq 1 - O(\gamma^q) - O(\rho^d).
\end{equation}
\end{enumerate}
\end{proposition}

The second assertion says that the normal vector to level sets $\{\varphi = y\}$, where $y$ is far from $\{0, 1\}$, is approximately constant in the coordinates $(x^\mu)$.
...
though of course in the application to de Giorgi's lemma we will be interested in the case $\rho \to 0$.
