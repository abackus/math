...
Let $2 \leq d \leq 7$ and let $M$ be a hyperbolic $d$-fold.
Then every set of least perimeter in $M$ is bounded by a $C^\infty$ minimal hypersurface.
\end{theorem}
...
Such an identification is given by a choice of universal covering $\Hyp^d \to M$ and a choice of embedding $\Hyp^d \to \RR^{1, d}$ whose image is the space of unit future-pointing timelike vectors.
To change the universal covering or the embedding amounts to an action of the isometry group $\SpOrth_{1, d}^+$ of $\Hyp^d$.
