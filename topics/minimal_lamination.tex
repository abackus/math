...
\end{lemma}
\begin{proof}
% We first construct a $\star |\dif u|$-measurable vector field $X_1$ which is continuous along $\supp \dif u$, such that $(\dif u, X_1) = |\dif u|$.
% To do this, we let $f$ be the section of the cosphere bundle which appears in the Radon-Nikod\'ym decomposition (\ref{RNy formula}) of $\dif u$.
% Then $X_1 := f^\sharp$ has the desired properties, except that it is not clear that $X_1$ is continuous.

% To prove the continuity of $X_1$, we observe that
% $$X_1|\partial \{u > y\} = \normal^\sharp_{\partial \{u > y\}}$$
% is the unit normal vector field to the smooth hypersurface $\partial \{u > y\}$.
% So if $(x_n)$ is a sequence of points in $\supp \dif u$, $x_n \in \partial \{u > y_n\}$, and $x_n \to x \in \partial \{u > y\}$, but 
% $$\lim_{n \to \infty} X_1(x_n) \neq X_1(x),$$
% then infinitely many of the $y_n$ are not $y$, and by taking subsequences we may assume that $\partial \{u > y_n\} \neq \partial \{u > y\}$ for every $n$.
% So $\partial \{u > y\}$ meets $\partial \{u > y_n\}$ transversely at $x$, which contradicts that $\lambda$ is a set-theoretic lamination.

% Thus by the Tietze extension theorem, there exists a continuous vector field $X_2$ on $M$ such that
% $$X_1|\supp \dif u = X_2|\supp \dif u,$$
% and hence, since $\dif u = 0$ away from $\supp \dif u$, $(\dif u, X_2) = |\dif u|$.
% We can then mollify $X_2$ to obtain a smooth vector field $X_3$ which is close to $X_2$ in $C^0_\loc$, thus
% $$(\normal_{\partial \{u > y\}}, X_3) \cdot |\dif u| = (\dif u, X_3) \gtrsim |\dif u|$$
% locally uniformly.
% It follows that the integral curves of $X_3$ are transverse to $\partial \{u > y\}$.
\end{proof}

% \begin{proposition}
% The partition associated to a function of least gradient is actually a (topological) lamination.
% \end{proposition}
% \begin{proof}
% Let $u$ be a function of least gradient, $\lambda$ the associated set-theoretic lamination, and $X$ a smooth vector field whose integral curves are transverse to every leaf of $\lambda$.
% Fix a leaf $N$, and let $(x^i)$ be local coordinates on $N$, say centered on $Q \in N$, which extend to coordinates to every hypersurface transverse to the integral curves of $X$ which pass through the domain of $(x^i)$.
% Let
% $$x^0(P) := \sup \{y: P \in \partial \{u_{\text{cts}} > y\}\}$$
% for $P \in \supp \dif u_{\text{cts}}$, and extend $x^0$ by interpolation to a connected contractible open set containing $Q$ such that $x^0$ is constant on leaves of $\supp \dif u_{\text{jmp}}$ and $Xx^0 > 0$.
% Then it is clear that $(x^\mu)$ define flow box coordinates centered on $Q$ for $\lambda$.
% \end{proof}

% \begin{proposition}
% Let $\overline M$ be a surface-with-boundary such that $\partial M$ is transverse to every geodesic in $M$.
% Then the geodesic lamination associated to a function of least gradient on $M$ extends to a geodesic lamination of $\overline M$.
% \end{proposition}
% \begin{proof}
