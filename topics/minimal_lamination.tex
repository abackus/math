...
copy the old motivation crap...

\subsection{The Plateau energy}
choose coords...
...
Now suppose that $\Omega \subseteq \Hyp^{d - 1}$ is open and equipped with an identification
$$\Omega \cong \{x^0 = 0\};$$
...
$$\Psi^* g = \slashed g + g_{00} \dif \omega \otimes \dif \omega$$
where $\slashed g$ is the metric on $\{x^0 = 0\}$, $\slashed g_{ij} = g_{ij}$.
The only independent component of the volume form of the pullback metric $\Psi^* g$ is
$$\sqrt{\det \Psi^* g} = \sqrt{\det \slashed g} \sqrt{1 + g_{00} |\dif \omega|_{\slashed g^{-1}}^2}$$
...
$$\Lagrange[\dif \omega] = \star \sqrt{1 + g_{00} |\dif \omega|^2},$$
where $|\cdot|$ and $\star$ are with respect to the usual metric $\slashed g$ on $\Hyp^{d - 1}$ and $g_{00}$ is the pullback of the $00$-component of $g$ on $\{x^0 = 0\} \subset M$.
...
In particular, $\omega$ is a variational solution of the Plateau equation iff $N$ bounds a set of least perimeter.

The Plateau equation can be computed by considering a variation $\omega$ with $\phi = -\dot \omega$ compactly supported in $\Hyp^{d - 1}$.
In that case,
\begin{align*}
\frac{\dif}{\dif t} \Lagrange[\dif \omega] &= \int_{\Hyp^{d - 1}} \star \partial_t \sqrt{1 + g_{00} |\nabla \omega|^2} \\
&= -\int_{\Hyp^{d - 1}} \star \frac{g_{00} \slashed g(\nabla \phi, \nabla \omega)}{\sqrt{1 + g_{00}|\nabla \omega|^2}} \\
&= \int_{\Hyp^{d - 1}} \star \phi\left[\frac{g_{00}}{\sqrt{1 + g_{00} |\nabla \omega|^2}} \Delta \omega + \slashed g\left(\nabla \frac{g_{00}}{\sqrt{1 + g_{00} |\nabla \omega|^2}}, \nabla \omega\right)\right].
\end{align*}
Thus the Plateau equation is
\begin{equation}\label{Plateau eqn}
\Delta \omega + \frac{\sqrt{1 + g_{00} |\nabla \omega|^2}}{g_{00}} \slashed g\left(\nabla \frac{g_{00}}{\sqrt{1 + g_{00} |\nabla \omega|^2}}, \nabla \omega\right) = 0
\end{equation}
where $\Delta$ is the hyperbolic Laplace-Beltrami operator. Linearizing about $||\omega||_{C^1} \ll 1$ we arrive at the Laplace-Beltrami equation $\Delta \omega = 0$ on $\Hyp^{d - 1}$.
We therefore launch into a study of harmonic functions on $\Hyp^{d - 1}$.
...
\section{Plateau's equation OLD}
...
L^i := 2(\slashed g^{-1})^{ij} \Psi^*_\omega (g_{0j}).
\end{equation}
The \dfn{Plateau equation} $P\omega = 0$ is the Euler-Lagrange equation for minimizers of $\int \Lagrange[\omega, \dif \omega]$.
\end{definition}
...
\begin{lemma}
The linearization of the Plateau equation $P\omega = 0$ in Killing gauge with
\begin{equation}\label{hypothesis on the linearization}
||\dif \omega||_{L^\infty} + ||g_{00} - 1||_{C^1} + \sum_{i,j = 1}^{d - 1} ||\slashed g_{ij} - \delta_{ij}||_{C^1} \ll 1
\end{equation}
is the euclidean Laplace equation $\Delta \omega = 0$.
\end{lemma}
\begin{proof}
In Killing gauge, the Plateau equation takes the form $\partial_i \partial_{\omega_{,i}} \Lagrange[\dif \omega] = 0$, or in other words
\begin{equation}\label{Plateau equation}
\Div_{\slashed g} \frac{2g_{00} \grad_{\slashed g} \omega + L}{2 \sqrt{1 + g_{00}|\grad_{\slashed g} \omega|_{\slashed g}^2 + L\omega}} = 0.
\end{equation}
Moreover, $L$ is given by (\ref{Plateau error}) and so vanishes as $\slashed g_{ij} - \delta_{ij} \to 0$ and $g_{00} \to 1$ in $C^1$.
So in the linearization of (\ref{Plateau equation}) we may discard terms involving $L$; we may also discard terms involving $|\dif \omega|^2$. We are then left with $\Div \grad \omega = 0$.
\end{proof}

We therefore seek to impose (\ref{hypothesis on the linearization}), which would be true for $||\dif \omega||_{L^\infty} \ll 1$ if $(x^\mu)$ were normal coordinates, but we do not have the freedom to impose both that $(x^\mu)$ are normal and that $g$ is in Killing gauge:
We therefore compromise between these two conditions:

\begin{definition}
Let $(p, v) \in TM$ and $(x^\mu)$ be a coordinate system. We say that $(x^\mu)$ are \dfn{Killing-normal coordinates} based at $(p, v)$ if:
\begin{enumerate}
\item $g_{\mu\nu} = g(\partial_\mu, \partial_\nu)$ is in Killing gauge,
\item $(x^i)$ are $\slashed g$-normal coordinates,
\item $g_{00}(p) = 1$, and
\item $p$ is the origin of $(x^\mu)$ and $\partial_0(p) = v$.
\end{enumerate}
\end{definition}
