...
More precisely, since $\Mink$ restricts to an inner product on $\RR^{d - 1}$,
$$\det(I + \dif \omega \otimes \dif \omega) = 1 + \Mink^{-1}(\dif \omega, \dif \omega) = 1 + |\dif \omega|^2 > 0.$$
Therefore 
\begin{align*}
\det(I + \dif \omega \otimes \dif \omega - \dif T \otimes \dif T) &= (1 - \Mink^{-1}(\dif T, (I + \dif \omega \otimes \dif \omega) \dif T))(1 + |\dif \omega|^2).
\end{align*}
We moreover have 
$$|\dif T|^2 = \frac{r^2 + w^2|\dif w|^2 + rw \partial_r w}{1 + r^2 + w^2}.$$
To finish the proof we just compute 
\begin{align*}
\Mink^{-1}(\dif T, (\dif \omega \otimes \dif \omega) \dif T) &= (\grad T, \dif w)(\grad w, \dif T) = \Mink(\grad w, \grad T)^2\\
&= r^2 (\partial_r w)^2 + 2 rw \partial_r w |\dif w|^2 + w^2 |\dif w|^4. \qedhere
\end{align*}
\end{proof}

\begin{definition}
We define the \dfn{Plateau energy} on $\RR^{d - 1}$,
$$\Lagrange(w, \xi) = \sqrt{1 + |\xi|^2 - \frac{r^2 + w^2 |\xi|^2 + rw (\partial_r, \xi) + r^2 (\partial_r, \xi)^2 + 2 rw (\partial_r, \xi) |\xi|^2 + w^2 |\xi|^4}{1 + r^2 + w^2}},$$
for $\xi$ a $1$-form and $w$ a function.
The Euler-Lagrange equation for minimizers of $\Lagrange(w, \dif w)$ is called the \dfn{Plateau equation}.
A variational solution of the Plateau equation is called a \dfn{minimal graph} in $\Hyp^d$.
\end{definition}

Though we will not write out the Plateau equation explicitly, it is easy to see that is a quasilinear elliptic PDE that has second order and $C^\infty$ coefficients.
Using (\ref{measure of minimal surface}) and an elliptic bootstrapping argument, the next lemma follows:

\begin{lemma}
Every minimal graph in $\Hyp^d$ is a $C^\infty$ minimal hypersurface in $\Hyp^d$.
\end{lemma}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The bootstrap argument}
In order to carry out our bootstrapping argument we will need some small parameters $c_i$.
We begin by fixing $c_0 > 0$, which we will determine later (but the reader could take as $c_0 = 10^{-5}$ if desired).
Henceforth all implied constants will be allowed to depend on $c_0$ (but, crucially, not on $w \in C_1(B_\rho)$ or $\rho$).
