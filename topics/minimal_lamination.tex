...
This equation is invariant under target translations $v \mapsto v + y$ and so Noether's theorem associates a conserved flux\footnote{By a \dfn{conserved flux}, we mean a closed $d-1$-form $\psi$; the equation $\dif \psi = 0$ can be viewed as a conservation law.} $\dif u$ to each $\infty$-harmonic function $v$.
Daskalopolous--Uhlenbeck \cite{daskalopoulos2020transverse,daskalopoulosPrep1} show that if $M$ is a simply connected surface, then $u$ is a $1$-harmonic function, and the level sets of $u$ are geodesics contained in the \dfn{maximum stretch locus} $\{|\dif v| = ||\dif v||_{L^\infty}\}$.
...
We now show that $\Lagrange(w, \dif w) - \Lagrange(w, \avg_\rho \dif w)$ is close to the analogous quantity for the euclidean case if $\rho$ is small, and in particular is close to the excess:
...
\end{align}
and if $\rho \ll_K 1$, then there exists $\rho' = e^{O(c_0)} \rho$ such that
\begin{equation}
\int_{\mathscr B_\rho} \Lagrange(w, \dif w) - \Lagrange(w, \avg_\rho \dif w) = \Exc_{B(P, \rho')} (U, \Psi_w(0)) + O(|K| \rho^{d + 1}). \label{compare Lagrangians 2}
\end{equation}
\end{lemma}
\begin{proof}
...
Now we recall the euclidean version of (\ref{compare Lagrangians 2}), which appears in the proof of \cite[Lemma 6.4]{Giusti77}:
$$\Exc'(B'(P, \rho - O(c_0))) \leq \int_{\mathscr B_\rho} \Japan{\dif w} \dif x - \Japan{\avg_\rho \dif w} \dif x \leq \Exc'(B'(P, \rho + O(c_0)).$$
Here the prime indicates the euclidean object, thus we identify $M$ with $\RR^d$, and this result holds as long as $||\dif w||_{C^0}$ is small depending on $c_0, K$.
Taylor expanding the metric around $P$ and recalling that balls may shrink or grow by $O(K\rho^3)$ when we do so, we bound
\begin{align*}
\Exc_{B(P, e^{-O(c_0)}\rho}(U, P) &\leq \Exc'_{B'(P, e^{-O(c_0))}\rho + O(|K|\rho^3)}(U, P) + O(|K|\rho^{d + 1}) \\
&\leq \int_{\mathscr B_\rho} \Japan{\dif w} \dif x - \Japan{\avg_\rho \dif w} \dif x + O(|K|\rho^{d + 1}) \\
&= \int_{\mathscr B_\rho} \Lagrange(w, \dif w) - \Lagrange(w, \avg_\rho \dif w) + O(|K|\rho^{d + 1})
\end{align*}
(where we assumed $\rho$ small depending on $c_0, K$).
The complementary bound is analogous and gives
\begin{align*}
\Exc_{B(P, e^{-c_0}\rho)}(U, P) - O(|K|\rho^{d + 1}) &\leq \int_{\mathscr B_\rho} \Lagrange(w, \dif w) - \Lagrange(w, \avg_\rho \dif w) \\
&\leq \Exc_{B(P, e^{c_0}\rho)}(U, P) + O(|K|\rho^{d + 1}).
\end{align*}
The conclusion (\ref{compare Lagrangians 2}) now follows from the intermediate value theorem.
\end{proof}
...
By (\ref{Miranda44 normal hyp}) it follows that on $B(O, \rho)$,
$$\star(((\Phi^Q)^* \normal_U) \wedge \psi) \geq O(\rho) + \star(((\Phi^P)^* \normal_U) \wedge \psi) \geq O(\rho) + (1 - o(c_1^2)) |\psi|.$$
Assuming that $c_1$ is small enough depending on the implicit constant in our gauge assumption,
\begin{equation}\label{moving the normal vector is good}
\star(((\Phi^Q)^* \normal_U) \wedge \psi) \geq (1 - o(c_1^2)) |\psi|.
\end{equation}

By (\ref{moving the normal vector is good}) and Lemma \ref{rep as a good graph},
...
The idea behind the proof of Proposition \ref{main mollifier lemma}, as in \cite{Giusti77}, is to cover $\partial^* U$ by small balls and apply the monotonicity formula in each ball, so we begin by proving a lemma, Lemma \ref{mollifier sublemma}, that suitably estimates the behavior of $u$ in a single ball.
