...
\end{equation}
on $W$. Indeed, the statement of Lemma \ref{mollifier sublemma} is independent of the choice of gauge $\Phi^P$, so we can make a gauge transformation, apply Lemma \ref{DoVF lemma}, and recall $O \in W$ (since $\sigma > \varepsilon$) to get
$$|\partial_0 - \partial_0^P| \lesssim \sigma^2$$
...
To estimate $I_3 - I_4$, we apply the monotonicity formula and compatibility again:
\begin{align*}
I_3 - I_4 &\lesssim \sqrt{1 + (d - 1) \log \frac{\sigma}{2\delta\varepsilon}} \sqrt{\sigma^{1 - d} \int_W \star |\dif u|} \sqrt{\int_{2\delta\varepsilon}^\sigma \partial_r \left[e^{Ar^2} r^{1 - d} \int_{B(Q, r)} \star |\dif u|\right] \dif r}\\
&\qquad + O(|K|) \sigma^{3 - d} \int_W \star |\dif u| \\
&=: J_1 J_2 J_3 + J_4.
\end{align*}
Here $0 \leq A \lesssim |K|$ is as in Proposition \ref{Monotone}.
By definition we have $J_1 \lesssim -\log \gamma$, and $J_4 = I_1 \lesssim \gamma^{1/(d - 1)}$.
From Corollary \ref{doubling dimension} we have $J_2 \lesssim 1$.
We then need to bound $J_3$:

\begin{sublemma}
Up to a gauge transformation we have $J_3 \lesssim \gamma^{\min(1/(2(d - 1)), 1/4)}$.
\end{sublemma}
\begin{proof}
We bound
\begin{align*}
...
&=: K_1 + K_2 + K_3 - K_4 + K_5.
\end{align*}
Then $K_1 = I_1 \leq \gamma^{1/2}$ and $K_2 = I_2 \lesssim \gamma^{1/(d - 1)}$.
To bound $K_2$, recall that by (\ref{difference of vector fields}), possibly after applying a gauge transformation, we may assume that on $B_\varepsilon$,
$$|\partial_0^P - \partial_0| \lesssim \varepsilon^2 \lesssim \gamma^8.$$
In particular, $K_2 \lesssim \gamma^7$.
