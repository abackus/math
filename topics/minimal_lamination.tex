...
\title{Regularity of minimal perimeters in manifolds of constant sectional curvature}
\author{Aidan Backus}
\date{October 2022}
...
Functions of least gradient arise naturally in magnetic resonance imaging \cite{Tamasan2019, Joy09} and in the continuum-time limit of certain combinatorial games \cite{Kohn06}.
They have recently become of great interest to both numerical \cite{Loisel20} and geometric \cite{daskalopoulos2020transverse} analysts.

Here we consider the geometric applications of functions of least gradient.
The Euler-Lagrange equation for (\ref{least gradient functional}) is the $1$-Laplace equation
\begin{equation}\label{1Laplacian}
\dif^* \left(\frac{\dif u}{|\dif u|}\right) = 0
\end{equation}
though one has to be careful about what this means \cite{Mazon14}.
...
On the other hand, if $v: M \to \RR$ is an $\infty$-harmonic function on a surface, that is
$$(\nabla^\mu \partial^\nu v) (\partial_\mu v) (\partial_\nu v) = 0$$
in the sense of comparison with cones, then Daskalopolous--Uhlenbeck \cite{daskalopoulos2020transverse,daskalopoulosPrep1} associate the following data to $v$:
\begin{enumerate}
\item A closed $1$-form $\dif u$, the N\"other current arising from translations of the target $\RR$.
\item A geodesic lamination $\lambda$, the \dfn{locus of maximum stretch}, whose support is $\{|\dif v| = ||\dif v||_{L^\infty}\}$.
\end{enumerate}
Here a \dfn{geodesic} (resp. \dfn{minimal}) \dfn{lamination} is a closed set which is foliated by geodesics (resp. minimal hypersurfaces); such objects are foundational in low-dimensional topology \cite{thurston1979geometry}.
The relationship between $\lambda$ and $\dif u$ is given by \cite{daskalopoulos2020transverse}: 

\begin{theorem}[Daskalopoulos--Uhlenbeck duality]
Suppose that $M = \Hyp^2$.
Let $v$ be an $\infty$-harmonic function, $\dif u$ the N\"other current associated to $v$, and $\lambda$ the locus of maximum stretch associated to $v$.
Then $u$ is a $1$-harmonic function and the level sets of $u$ are geodesics in $\lambda$.
\end{theorem}

It is natural to expect that the same ideas hold in higher generality \cite[\S9]{daskalopoulos2020transverse}, and we embark on this project by studying the level sets of a $1$-harmonic function.
Classically, such level sets have least perimeter \cite{BOMBIERI1969}, and this proof works in high generality.
The de Giorgi--Miranda regularity theorem \cite{deGiorgi61, Miranda66}\footnote{De Giorgi's original paper \cite{deGiorgi61} is notoriously difficulty to find a copy of, as noted by \cite{Miranda66, Giusti77}; moreover, \cite{Miranda66} is in Italian. Therefore, in the proof of Theorem \ref{main lma}, we follow the English-language monograph of Giusti \cite[Part 1]{Giusti77}.} shows that for $d \leq 7$ the level sets are then smooth minimal hypersurfaces, as one would formally expect from the $1$-Laplace equation (\ref{1Laplacian}).

The proof of the de Giorgi--Miranda regularity theorem strongly uses the flatness of the domain, and the purpose of this paper is to remove this assumption.
...
We will then apply Theorem \ref{main lma} and the compactness theory to prove:
...
Here a \dfn{Ruelle-Sullivan current} for $\lambda$ is a $d-1$-current of the form $\normal \mu$, where $\mu$ is supported on $\supp \lambda$ and $\normal$ is the conormal $1$-form to the minimal surfaces in $\lambda$.
...
At this point it is not clear how, or if, the $\infty$-Laplacian enters the picture for a function of least gradient on a manifold of dimension $d \geq 3$.
...
As in the euclidean case, one can use the monotonicity formula to show that mollification preserves the excess (Lemma \ref{main mollifier lemma}) which completes the proof.
...
I would also like to thank Karen Uhlenbeck and Christine Breiner for helpful comments, and the NSF TODO.
...
Note carefully: if we have a hyperbolic time $t$ we do \emph{not} sum over it, and do not write $x^0 = t$.
...
which is well-defined as long as $u \in L^1_\loc(U)$.
