...
Functions of least gradient arise naturally in the study of magnetic resonance imaging, the continuum-time limit of certain combinatorial games, and as a model problem for numerical analysis of variational problems in $BV$ \cite{Nachman2009, Joy09, Kohn06, Loisel20}.
The gradient flow for (\ref{least gradient functional}), \dfn{level set flow}, arises in models of surface tension, as well as a method for continuing the mean curvature flow past its singular times \cite{Evans91, Sethian90, Chen89}.
We are interested in functions of least gradient on manifolds because of their potential applications to geometric topology, which was proposed in \cite{daskalopoulos2020transverse, daskalopoulosPrep1} and we recall in \S\ref{AppsSec}.
...
%%%%%%%%%%%%%%%%%%%
\subsection{Applications of the main theorem}\label{AppsSec}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The Lagrangian for minimal surfaces}
If $N \subset \RR^3$ is an embedded minimal surface, then at least locally, we can realize $N$ as the graph of a function $w: \RR^2 \to \RR$ which solves the minimal surface equation\footnote{We remind the reader that we mean by $\Japan \xi$ the Japanese norm $\sqrt{1 + |\xi|^2}$ of $\xi$.}
$$L w := \dif^* \left(\frac{\dif w}{\Japan{\dif w}}\right) = 0.$$
If $N \subset M$ is an embedded minimal hypersurface and $M$ is a Riemannian manifold equipped with a choice of coordinates, however, the minimal surface operator $L$ may be quite complicated \cite[\S7.1]{colding2011course}.
Here, we give a simple Lagrangian for the minimal surface operator in the case that $M$ is a manifold of constant sectional curvature $K \in \RR$, equipped with coordinates $(x, y) \in \RR^{d - 1}_x \times \RR_y$ in which the metric is diagonal and of the form
$$g(\partial_{x^i}, \partial_{x^i}) = g(\partial_y, \partial_y) = \frac{1}{(1 + K(|x|^2 + y^2)/4)^2}.$$
This Lagrangian will be a crucial object of study in both this paper and the sequel \cite{BackusCML}.
...
Define for $w \in C^1(\Omega)$, $\Omega \subseteq \RR^{d - 1}$, the map $\Psi_w: \Omega \to M$ given by
$$\Psi_w(x)^i := x^i, \quad \Psi_w(x)^0 := w(x).$$
Then $(\Psi_w^{-1})^* \Lagrange(w, \dif w)$ is the Riemannian measure on $\Psi_w(\Omega)$.
In particular, if $w$ is a critical point of $\Lagrange(w, \dif w)$, then $\Psi_w(\Omega)$ is a minimal hypersurface.
\end{lemma}
\begin{proof}
...
\end{align*}
By \cite[(24)]{Petersen2008} we have $\det((\delta_{ij} + \partial_i w \partial_j w)_{ij}) = 1 + |\dif w|^2$, so if $\omega$ denotes the pullback of the Riemannian measure on $\Psi_w(\Omega)$ to $\Omega$, then
\begin{align*}
\omega &= \sqrt{\det(\Psi_w^*(g|\Psi_w(\Omega))(\partial_i, \partial_j))} = \frac{\Japan{(w, \dif w)}}{((1 + K(|x|^2 + w^2)/4)^2)^{\frac{d - 1}{2}}}. \qedhere 
\end{align*}
\end{proof}
...
To set up the proof, recall that we may view the derivative of a function $w: \Omega \to \RR$ as a map $\dif w: \Omega \to \RR^{d - 1}$.
...
Let $\Lagrange$ be the Lagrangian derived in Lemma \ref{Plateau setup lemma}, and $\Psi_w: \RR^{d - 1} \to M$ the graph embedding.
...
\todo{Move the $\Two$ computation to the prelims to use in the other paper}
