...
Let $U$ be a set of least perimeter, $P \in \partial U$, and $c, \rho_*$ as in the de Giorgi lemma.
...
As on \cite[pg109]{Giusti77}, we see that by the de Giorgi lemma, $\Exc_\rho(U, P) \leq c\rho^{d - 1}$ for $\rho < \rho_*$ which is locally uniformly bounded from below in $P$ (even if $P \notin \partial^* U$).
So, reasoning identically to \cite[Chapters 8-9]{Giusti77}, we just need to show the following analogue of \cite[Theorem 8.2]{Giusti77}:
\begin{equation}\label{LC Cauchy}
|\normal_U(P, r) - \normal_U(P, s)| \lesssim \sqrt{\frac{r}{\rho}}.
\end{equation}
Moreover, as on \cite[pg100]{Giusti77}, we may assume that $r/2 < s < r = \rho/2^n$ for some $n$.
