...
We construct minimal laminations using the following compactness theorem.
...
\end{proposition}
\begin{proof}
The $d = 3$ case is treated in \cite[Proposition B.1]{ColdingMinicozziIV} \cite[{\S}II.1]{ColdingMinicozziV}.
However, one can check that their proof also works in arbitrary dimension.
\end{proof}
...
In order to apply the compactness theorem, we need an interior curvature estimate.
...
Our first task is to show that a function of least gradient defines a minimal partition of bounded curvature.

\begin{lemma}\label{bounds on Two of least gradient}
Let $u$ be a function of least gradient on $M$, and $y \in \RR$. Then the level set $\partial \{u > y\}$ is the locally finite union of minimal hypersurfaces in $M$.
