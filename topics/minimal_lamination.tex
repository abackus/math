...
\begin{definition}
Let $(\slashed g^{(y)})_{y \in \RR}$ be a family of Riemannian metrics on $\Omega$.
Then the \dfn{Plateau energy} of a function $\omega$ and a $1$-form $\alpha$ on $\Omega$ is the $d - 1$-form on $\Omega$
\begin{equation}\label{Plateau energy}
\Lagrange[\omega, \alpha] := e^{O(|x|^2 + |\omega(x)|^2)} \sqrt{1 + |\alpha(x)|^2_{(\slashed g^{(\omega(x))})^{-1}(x)}} \dif x^1 \wedge \cdots \wedge \dif x^{d - 1}.
\end{equation}
\end{definition}
