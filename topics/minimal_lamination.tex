...
\begin{definition}
A \dfn{minimal lamination} $\lambda$ in $M$ is a partition of a closed subset of $M$ into smooth hypersurfaces, called \dfn{leaves}, with zero mean curvature.
We call $\lambda$ a \dfn{geodesic lamination} if $M$ is a surface, \dfn{discrete} if the moduli space of leaves of $\lambda$ is discrete, and \dfn{analytic} if every leaf of $\lambda$ is analytic.
\end{definition}
...
This result was already known \cite[Proposition 3.4, Corollary 3.5]{górny2017planar} for flat $M$, where it was called the \dfn{least-gradient maximum principle}:
...
The necessity of the condition $d \leq 7$ is the existence of codimension-$8$ singularities in sets of least perimeter \cite[Theorem A]{BOMBIERI1969}.
