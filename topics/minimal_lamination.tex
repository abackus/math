...
In this section we select a gauge which any homogeneous space (and in particular the model spaces $\RR^d$, $\Hyp^d$, and $\Sph^d$), can be put in, so as to remove the dependency on $\omega$ from $\Lagrange$.
...
\item $(x^i)$ are $\slashed g$-normal coordinates, and
