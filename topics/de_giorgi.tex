...
Another proof, based on Allard's $\varepsilon$-regularity theorem, can also be modified \cite{DeLellis18}; see Appendix \ref{Allard}.
...
%%%%%%%%%%
\appendix \section{Regularity by Allard} \label{Allard}
It is well-known that one can use Allard's $\varepsilon$-regularity theorem, which is usually formulated on euclidean space, to prove the regularity of minimal submanifolds of arbitrary Riemannian manifolds.
However, I am not aware of a convenient one-stop reference for the details of this reduction, beyond an exercise in lecture notes of de Lellis \cite[Exercise 1.6]{DeLellis18}.
As I have often needed a reference for this fact, and it nicely complements the proof of Theorem \ref{main thm} by Miranda's method covered elsewhere in this paper, we shall now give a short reduction of Theorem \ref{main thm} to Allard's theorem and Corollary \ref{blowup theorem}.

Recall first that the mean curvature $H_N$ of a rectifiable set $N \subset \RR^d$ satisfies, for every $C^\infty_\cpt(\RR^d)$ vector field $X$,
...
\begin{theorem}[Allard's $\varepsilon$-regularity theorem {\cite[Theorem 3.2]{DeLellis18}}]
...
Let $1 \leq \delta \leq 6$ be an integer, let $M$ be a Riemannian manifold of dimension $\delta + 1$, and let $N \subset M$ be the reduced boundary of a set of least perimeter.
Then by Proposition \ref{locality of Caccioppoli}, $N$ is a $\delta$-rectifiable set.
By the Nash embedding theorem, we may assume that $M$ is embedded in $\RR^d$ for some $d \geq \delta + 2$, thus $N$ is $\delta$-rectifiable in $\RR^d$.

By Corollary \ref{blowup theorem}, $T_x N$ is a well-defined hyperplane in $T_x M$.
...
In particular,
$$\left|\int_N \Div_{TN} X \dif \mu_N\right| \lesssim_{\delta, d} \int_N |\Two_M| |X| \dif \mu_N \leq \|\Two_M\|_{C^0} \|X\|_{L^1(N)}.$$
Therefore 
$$\|H_{N \to \RR^d}\|_{L^\infty} \lesssim_{\delta, d} \|\Two_M\|_{C^0}.$$
So after rescaling to decrease $\Two_M$, we may assume that the embedding $N \to \RR^d$ satisfies (\ref{mean curvature excess}).
Since the blowup in Corollary \ref{blowup theorem} converges weakly, $r^{-\delta} |N \cap B(x, r)|$ is approximated arbitrarily well by
$$r^{-\delta} |T_x N \cap B(x, r)| = r^{-\delta} |\Ball^\delta| r^\delta = |\Ball^\delta|$$
as $r \to 0$, so there exists $r_* > 0$ such that for $0 < r \leq r_*$, (\ref{measure excess}) holds.

So by Allard's theorem, $N \cap B(x, \gamma r)$ is $C^{1 + \alpha}$.
By elliptic bootstrapping, it follows that $N \cap B(x, \gamma r)$ is $C^\infty$.
Since $x$ was arbitrary, the interior of $N$ is $C^\infty$.
\end{proof}
