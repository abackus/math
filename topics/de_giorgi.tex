...
% It is very inconvenient to pass to coordinates so that convolution becomes well-defined, however, and so we use a more intrinsically defined operator.
% The price we pay is that, of course, such an operator does not commute with $X$ or $\dif$, but we estimate the error and obtain the desired result.

% \subsection{Mollification}
% Let $f$ be a $d$-current of locally finite total variation (so that $f \dif V$ is a Radon measure).
% We define its \dfn{mollification} at scale $\varepsilon$
% $$M_\varepsilon f(P) = f_\varepsilon(P) := \frac{d + 1}{\varepsilon^d |\Ball^d|} \int_{B(P, \varepsilon)} f(Q) \left(1 - \frac{\dist(P, Q)}{\varepsilon}\right) \dif V(Q).$$
% If $\gamma_{P, Q}$ denotes the unique geodesic from $P$ to $Q$ in $B(P, \varepsilon)$, then 
% $$\dif_Q \dist(P, Q) = \gamma_{PQ}'(Q)^\flat.$$
% Thus, if $f \in L^\infty$, 
% \begin{align*}
% &|\dif M_\varepsilon f(P) - \dif M_\varepsilon f(Q)| \\
% &\qquad \lesssim \frac{1}{\varepsilon^{d + 1}} \left[\int_{B(P, \varepsilon) \Delta B(Q, \varepsilon)} |f| \dif V + \int_{B(P, \varepsilon) \cap B(Q, \varepsilon)} |f| |\gamma'_{PR}(R) - \gamma'_{QR}(R)| \dif V(R)\right] \\
% &\qquad \lesssim \frac{\|f\|_{L^\infty}}{\varepsilon} \dist(P, Q)
% \end{align*}
% so $M_\varepsilon: L^\infty \to C^1$.
% Moreover, if $(\partial_\mu)$ is a normal coordinate frame based at some point of $B(P, \varepsilon)$, then 
% \begin{align*}
% \partial_{P,\mu} \dist(P, Q) &= \gamma'_{QP}(P)^\mu = -\gamma'_{PQ}(P)^\mu \\
% &= -\gamma'_{PQ}(Q)^\mu + O(\|\Gamma\|_{C^0(B(P, \varepsilon))} \dist(P, Q)) \\
% &= -\partial_{Q,\mu} \dist(P, Q) + O(\varepsilon^2).
% \end{align*}
% The divergence of $\partial_\mu$ is $O(\varepsilon)$, so integrating by parts 
% \begin{align*}
% \partial_\mu M_\varepsilon f(P)
% &= \frac{C}{\varepsilon^d} \int_{B(P, \varepsilon)} f(Q) \partial_{P, \mu} \left(1 - \frac{\dist(P, Q)}{\varepsilon}\right) \dif V(Q) \\
% &= -\frac{C}{\varepsilon^d} \int_{B(P, \varepsilon)} f(Q) \partial_{Q, \mu} \left(1 - \frac{\dist(P, Q)}{\varepsilon}\right) + O(\varepsilon |f(Q)|) \dif V(Q) \\
% &= \frac{C}{\varepsilon^d} \int_{B(P, \varepsilon)} \partial_\mu f(Q) \left(1 - \frac{\dist(P, Q)}{\varepsilon}\right) + O(\varepsilon |f(Q)|) \dif V(Q) \\
% &= M_\varepsilon \partial_\mu f(P) + O(\varepsilon \|f\|_{L^\infty})
% \end{align*}
% and in particular we have the commutator estimate
% \begin{equation}\label{commutator}
% \|[\partial_\mu, M_\varepsilon]\|_{L^\infty \to C^0} \lesssim \varepsilon.
% \end{equation}
% Reasoning completely analogously to \cite[Lemma 7.1]{Giusti77} we also have:

% \begin{lemma}
% Let $U \subseteq M$ be a Borel set and $\varphi := (1_U)_\varepsilon$.
% For every $P \in M$, if $\rho > 0$ satisfies $\varphi(P) \in (O(\rho^2), 1 - O(\rho^2))$, then 
% \begin{equation}\label{far from boundary mollified}
% \dist(P, \partial^* U) \leq (1 - \rho) \varepsilon.
% \end{equation}
% \end{lemma}

% Mimicking the proof of \cite[(7.2)]{Giusti77}, we bound 
% \begin{equation}
% \int_{B(P, \rho)} 
