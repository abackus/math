...
\item We obtain estimates which only depend on the curvatures of the leaves and $M$, and on the injectivity radius $i$; they do not depend on the regularity of a given $C^0$ atlas.
\item In fact, we do not even assume the existence of a $C^0$ atlas.
\end{enumerate}
...
This circle of ideas has been developed by the Thurston school (see \cite{papadopoulos:hal-00129729, Gu_ritaud_2017} among others) but has recently also made contact with geometric PDE through the work of Daskalopoulos--Uhlenbeck \cite{daskalopoulos2020transverse,daskalopoulosPrep1}.
...
The $\infty$-Laplace equation is invariant under addition of constants $v \mapsto v + y$, so by Noether's theorem, it has a conserved flux $\dif u$.
...
In \S\ref{CompactnessSec} we prove Theorems \ref{implication theorem} (the implication theorem), \ref{compactness theorem} (the compactness theorem), and \ref{main thm} (the equivalence of $1$-harmonic functions and measured minimal laminations).
...
We use the Hausdorff notions of limit and limit inferior of a sequence in sets \cite[Chapter IV]{nadler2017continuum}:

\begin{definition}
Let $X$ be a topological space, and $(Y_n)$ a sequence of sets in $X$.
\begin{enumerate}
...
Diagonalizing against the compact exhaustion $(X_m)$ and using Theorem \ref{compactness theorem}, we obtain a lamination $\lambda = \lim_m \lambda_m$ on $M$.
