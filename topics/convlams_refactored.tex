...
Thus results are compatible with the topology literature.

Recall that the space $G(\Hyp^2)$ of complete geodesics in $\Hyp^2$ is homeomorphic to the open M\"obius strip equipped with a Lorentzian metric, which has the following properties:
\begin{enumerate}
\item If $\gamma_n \to \gamma$ in $G(\Hyp^2)$, then $\gamma_n \to \gamma$ in $C^\infty$.
\item Any two geodesics are spacelike-separated iff they intersect.
\end{enumerate}
A geodesic lamination $\lambda$ is thus identified with a closed set $\hat \lambda \subset G(\Hyp^2)$ twhich does not contain any two spacelike-separated points \cite[\S8.5]{thurston1979geometry}.
...
As we have already seen, transverse measures in the sense of Definition \ref{transverse measure definition} can be characterized invariantly by their Ruelle-Sullivan currents.\footnote{Since we can always twist all currents and forms in the below discussion by $\ZZ/2$ if $\lambda$ is not orientable, it is no loss for us to assume that $\lambda$ is orientable.}
So if $(\lambda, \mu)$ is a measured geodesic lamination, we can define $\hat \mu$ to be the Radon measure which disintegrates $T_\mu$ into the leaves of $\lambda$.
To be more precise, we define $\hat \mu$ to be the Radon measure furnished by the below lemma:
...
\begin{proof}[Proof of Lemma \ref{existence of boundary measure}]
...
Then $T_{\mu_n} \to T_\mu$ iff $\hat \mu_n \to \hat \mu$.
\end{proposition}
\begin{proof}
...
If $T_{\mu_n} \to T_\mu$, then $\lambda_n \to \lambda$ in the tangentially $C^1$ flow box topology by Proposition \ref{convergence of traansverse measures means flow box convergence}, so we may assume $\varphi_n \to \varphi$.
...
\end{align*}
Conversely, if $\hat \mu_n \to \hat \mu$, then we fix a $C^\cpt_0$ $1$-form $\varphi$ on $\Hyp^2$ and define $G(\gamma) := \int_\gamma \varphi$.
...
&= \lim_{n \to \infty} \int_{G(\Hyp^2)} G \dif \mu_n
= \int_{G(\Hyp^2)} G \dif \mu 
