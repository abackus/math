...
In general, we work locally, replacing $M$ with small open sets in which the Ruelle-Sullivan current is well-defined.
...
\begin{lemma}\label{surface to graph}
Let $0 < \varepsilon \leq 1$, $s > 0$, and $N \subseteq \RR^d$ a hypersurface with $\|\Two_N\|_{C^0} \lesssim \varepsilon/s$.
Suppose that $x \in N$ with $\dist_N(x, \partial N) \geq s$.
Then the intrinsic ball $B^N_s(x)$ is the graph of a function $u: T_x N \to \RR$ with $\|\nabla u\|_{C^0} \leq \varepsilon$ and $\|\nabla^2 u\|_{C^0} \lesssim \varepsilon/s$.
\end{lemma}
\begin{proof}
After rescaling we may assume $\varepsilon = 1$.
Then this follows from a straightforward modification of \cite[Lemma 2.4(1)]{colding2011course} to remove the hypothesis on dimension (their $2s$ is our $s$).
\end{proof}
