...
Let $M$ be a manifold of constant sectional curvature\footnote{We mainly make this hypothesis so that we may appeal to \cite{BackusFLG}; otherwise it can be significantly weakened.} and dimension $2 \leq d \leq 7$.
...
The notion of flow-box convergence is mainly useful for the following convergence modes $X$:
\begin{enumerate}
	\item $C^{1-} := \bigcap_{0 \leq \theta < 1} C^\theta$, where $C^\theta$ are H\"older spaces, and 
	\item tangential $C^\infty$, that is, $C^0$ convergence for every function $\partial_{\xi^1} \cdots \partial_{\xi^n} F_\alpha$, where $\partial_{\xi^i}$ are tangent to every leaf.
\end{enumerate}
...
\end{lemma}
\begin{proof}
The portmanteau theorem (via Lemma 2.4 in the other guy \todo{probably just make this a general statement about converging normal vectors}) implies that $(\normal_{N_n}(x_n))$ is a Cauchy sequence whenever $(x_n) \subset \prod_n N_n$ is a Cauchy sequence.
...
In such coordinates $N_n$ is the graph of some function $u_n: \RR^{d - 1}_x \to \RR_y$ which is bounded in $C^2$ and solves the minimal surface equation.
By Schauder estimates and elliptic bootstrapping, $(u_n)$ is bounded and hence compact in $C^\infty$.
...
\begin{definition}
Let $X$ be a topological space, and $(Y_n)$ a sequence of sets in $Y$.
\begin{enumerate}
...
\item If $\liminf_{n \to \infty} Y_n$ equals the set of all $x \in X$ such that for every open neighborhood $U \ni x$, $U \cap Y_n$ is nonempty for infinitely many $n$, then we call it the \dfn{limit} $\lim_{n \to \infty} Y_n := \liminf_{n \to \infty} Y_n$.
\end{enumerate}
\end{definition}
