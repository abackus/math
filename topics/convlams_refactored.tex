...
Let $d \leq 7$ and $i, A > 0$.
Let $g$ be a Riemannian metric which is bounded in $C^\infty$.
Then there exists $\delta_* > 0$ which only depends on the $C^\infty$ seminorms of $g$ such that the following holds:
...
\liminf_{n \to \infty} \lambda_1^{(n)} = \lambda_1.
\end{equation}
\end{lemma}
\begin{proof}
...
If the proposition fails, then by the compactness of the unit ball in $C^\infty(\Ball^d)$, we can find metrics $g_j$ on $\Ball^d_x$, which satisfy the normal coordinates conditions that $|g_j - I| \lesssim x^2$ uniformly and $B(0, r) = B_{g_j}(0, r)$, and such that the following holds:
\begin{enumerate}
\item There exist $\delta_j \to 0$, such that some oriented $g_j$-minimal hypersurface $N_j \subset B_j := B(0, \delta_j)$ contains $0$ and is not uniquely area-minimizing, but has curvature $\leq A$.
\item There exists a metric $g$ such that $(g_j)_{\mu\nu} \to g_{\mu\nu}$ in $C^\infty$.
\end{enumerate}

By Lemma \ref{existence of absolute minimizers}, there exist absolutely minimizing competitors $N_j'$ to $N_j$ in $B_j$.
Let $L_j, L_j' \subset \Ball^d$ be the rescalings of $N_j, N_j'$ by $\delta_j^{-1}$.
Since $A < \infty$, $(g_j)$ is bounded in $C^2$ (so $\Riem_{g_j}$ is bounded in $C^0$) and $\delta_j \to 0$, $L_j$ is an oriented hypersurface in $\Ball^d$ such that $0 \in L_j$ and $\Two_{L_j} \to 0$ uniformly, where $\Two_{L_j}$ is taken in the flat metric.
...
\todo{Expand this out to estimate $s_j \to 0$ in $W^{1, 2}$}
...
$$|L_j'|_{g_j'} \geq |L_j|_{g_j'} + \langle s_j, Q_j s_j\rangle_{L^2(L_j, g_j')} - O(\|s_j\|_{W^{1, 2}}^3)$$
...
Indeed, for every $k \geq 3$, there exists $r \in \NN$ such that the $k$th variation of $L_j|_{g_j'}$ is controlled by the $C^r$ norm of $g_j'$ and $f_j$, times $\|s_j\|_{W^{1, 2}}^k$.
\todo{Just bound the modulus of continuity of the bilinear form on $Q$}

The stability operators satisfy 
$$Q_j = -\Delta_{g_j'} - |\Two_{g_j', L_j}|_{g_j'}^2 - \Ric_{g_j'}(\normal_{g_j', L_j}, \normal_{g_j', L_j}).$$
So if $Q = -\Delta$ is the stability operator of $L$, then the nondivergence form coefficients of $Q_j$ converge in $C^0$ to coefficients of $Q$, so by (\ref{spectral gap}), if $j$ is large enough, then $Q_j$ has spectral gap $\geq \lambda_1/2$.
