...
We say that they belong to the same \dfn{laminar atlas} if the transition map $\psi_{\alpha \beta}$ between $F_\alpha$ and $F_\beta$ is \dfn{laminar} if it maps each leaf $\{k\} \times \RR^{d - 1}$, $k \in K_\alpha$, to a leaf $\{\psi_{\alpha \beta}(k)\} \times \RR^{d - 1}$, so that $\psi_{\alpha \beta}$ is a homeomorphism $K_\alpha \to K_\beta$.
...
A related notion, which we shall find useful, is that a laminaiton $\lambda$ is \dfn{tangentially} $C^r$, in the sense that the flow boxes $F_\alpha$ restrict to $C^r$ embeddings along each leaf $\{k\} \times \RR^{d - 1}$, $k \in K_\alpha$.
...
\begin{enumerate}
\item If $\psi \mapsto \int_M T \wedge \dif \psi$ extends from $C^1_c(M, \Omega^{\ell - 1})$ to an $\ell - 1$-current, we define the \dfn{exterior derivative}
...
The fact that $\lambda$ is tangentially $C^1$ justifies the various pullbacks that appear in the proof.
...
$$\int_U \star |T_\mu| = \sup_{\|\varphi\|_{C^0} \leq 1} \int_{K_\alpha} \int_{\RR^{d - 1} \times \{k\}} \varphi \dif \mu_\alpha(k)$$
...
$$\int_U \star |T_\mu| = \int_{K_\alpha} \int_{\RR^{d - 1} \times \{k\}} (F_\alpha^{-1})^*(1_U \star \normal_\lambda) \dif \mu_\alpha(k).$$
...
\item There exist constants $L = L(g, A) > 0$ and $r = r(g, A) > 0$, and a Lipschitz, tangentially $C^\infty$ laminar atlas $(F_\alpha)$ for $\lambda$, such that for every $\alpha$,
...
It is clear that $F$ is tangentially $C^\infty$, since $u_k$ and $v_{\ell k}$ are $C^\infty$.
...
The facts that every cocycle of labels defines a complete hypersurface, and that $K_\alpha$ is compact for every $\alpha$ and nonempty for some $\alpha$, ensure that $\lambda$ is actually a lamination.
...
Thus $\{k_\alpha\} \times \RR^{d - 1}$ can be written as the set of limits of sequences $(k_\alpha^n, x)_n \in \prod_n K_\alpha^n \times \RR^{d - 1}$, and so any leaf $N$ of $\lambda$ can be written 
...
A $1$-harmonic function on an interval is monotone, and since $(k, x)$ are oriented laminar coordinates, $u(\cdot, x)$ is nondecreasing.
So $u(\beta, x) \geq u(\alpha, x)$, so $\mu$ is a positive measure.
