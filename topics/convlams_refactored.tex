...
\item Every leaf of $\lambda$ is the Hausdorff limit of connected components of level sets $\partial \{u > y\}$.
...
%%%%%%%%%%%%%%%%%%
\subsection{Level sets of \texorpdfstring{$BV$ functions}{functions of bounded variation}}
Suppose that $u \in BV_\loc(M)$; then $u$ is only defined almost everywhere, and we cannot discuss its values pointwise.
Instead, we work with the superlevel sets $\{u > y\}$, $y \in \RR$, which are defined by the property that $x \in \{u > y\}$ iff, for some (and hence every) everywhere-defined representative $u^*$ of $u$,
$$\lim_{\varepsilon \to 0} \frac{|\{u^* > y\} \cap B(x, \varepsilon)|}{|B(x, \varepsilon)|} = 1.$$
For almost every $y \in \RR$, it follows from the coarea formula that $\{u > y\}$ is a set of locally finite perimeter, and hence we can interpret $\partial \{u > y\}$ to be (the support of) the exterior derivative of the current $1_{\{u > y\}}$.
This defines $\partial \{u > y\}$ in a way which does not depend on the choice of representative $u^*$.
Since it is a support, $\partial \{u > y\}$ is a closed set, which is the closure of the reduced boundary studied in \cite[Chapter 4]{Giusti77}.
In particular, $x \in \partial \{u > y\}$ iff for every $\varepsilon > 0$,
$$0 < \frac{|\{u > y\} \cap B(x, \varepsilon)|}{|B(x, \varepsilon)|} < 1.$$
...
Let $u \in BV(M)$ have least gradient in $M$. Then $1_{\{u > y\}}$ has least gradient in $M$.
...
By (\ref{level sets define support}), $\bigcup_{y \in \RR} \partial \{u > y\}$ is dense in $\supp \dif u$. \todo{Rewrite this}
