...
% We begin with an estimate on the curvature of a stable minimal surface which is due to Schoen \cite{Schoen2016} and Colding and Minicozzi \cite{ColdingMinicozziParametric} in dimension $d = 3$ and Chodosh and Li \cite{Chodosh2021} if $d = 4$.
% If one proved this estimate for $d \in \{5, 6, 7\}$, then Theorem \ref{main thm} would follow for such dimensions.
...
% 	Then there exists $A \geq 0$ such that for every compact stable minimal immersion $N \to M$ of codimension $1$ and trivial normal bundle,
% 	$$|\Two_N(P)| \leq \frac{A}{1 + \dist(P, \partial M)}.$$
% \end{theorem}
% \begin{proof}
% If $d = 2$, this is trivial.
% If $d = 3$, this follows from \cite[Corollary 2.11]{colding2011course}. 
% If $d = 4$, we may glue $M$ along its boundary to a region in $\RR^4$ to embed $M$ in a complete $4$-manifold of bounded geometry, and then apply \cite[Corollary 2.5]{chodosh2022complete}.
% \end{proof}

% \begin{lemma} \label{LLG implies LG}
% Let $u$ be a $1$-harmonic function, $x \in M$, and suppose that $r > 0$ is at most the injectivity radius of $M$ at $x$.
% Then $u|_{B(x, r)}$ has least gradient.
% \end{lemma}
% \begin{proof}
% By Theorem \ref{main thm of old paper}, $u$ has locally least gradient, so there exists an open cover $(U_\alpha)$ of $B(x, r)$ by sets in which $u$ has least gradient.
% Let $(u_t)$ be a variation of $u$ for the Dirichlet problem for the total variation energy $J$.
% Since $\overline{B(x, r)}$ is a complete contractible manifold-with-boundary, one can show that $J$ is convex (see for example \cite[Lemma 1]{Loisel20}).
% By taking a partition of unity, we may write $u_t = \sum_\alpha u_t^\alpha$ where $u_t^\alpha$ is a variation of $u$ for the Dirichlet problem on $U_\alpha$ such that $\partial_t u_t^\alpha = 0$ on $U_\alpha \setminus \overline{B(x, r)}$.
% Then at $t = 0$,
