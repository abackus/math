...
\date{\today}
\keywords{laminations, minimal hypersurfaces, functions of least gradient, Lipschitz regularity}
\subjclass[2020]{primary: 49Q20; secondary: 49Q05, 51F30}
...
Write $BV_\loc(M), BV(M), BV_\cpt(M)$ for the spaces of functions of locally bounded variation, bounded variation, and bounded variation and compact support, respectively.
...
%%%%%%%%%%%%%%%%%%%
\subsection{Hausdorff limits of closed sets}
\begin{definition}[{\cite[Chapter IV]{nadler2017continuum}}]
...
\item If $\liminf_{n \to \infty} Y_n = \limsup_{n \to \infty} Y_n$, we call that set the \dfn{Hausdorff limit} $\lim_{n \to \infty} Y_n$.
\end{enumerate}
\end{definition}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Laminations in \texorpdfstring{$\Hyp^2$}{the hyperbolic plane}}
...
View $\Sph^1$ as the circle at infinity of $\Hyp^2$.
If $\Delta_{\Sph^1}$ is the diagonal, then 
$$\mathrm{Geo}(\Hyp^2) := \frac{(\Sph^1 \times \Sph^1) \setminus \Delta_{\Sph^1}}{\ZZ/2},$$
where $\ZZ/2$ acts by permutation, is homeomorphic to a M\"obius band.
The map
$$\gamma \mapsto [(\gamma(-\infty), \gamma(+\infty))]$$
that sends a geodesic $\gamma$ to (the orbit under $\ZZ/2$ of) the endpoints of $\gamma$ allows us to view $\mathrm{Geo}(\Hyp^2)$ as the space of geodesics in $\Hyp^2$.

A geodesic lamination $\lambda$ is thus identified with a closed set $\hat \lambda \subset \mathrm{Geo}(\Hyp^2)$ \cite[\S8.5]{thurston1979geometry}.
Let $\mu$ be a transverse measure to $\lambda$, which we can view as a Radon measure on $\Hyp^2$ by (\ref{transverse measure of an open set}).
Given $f \in C^0_\cpt(\mathrm{Geo}(\Hyp^2))$, we can always find $F \in C^0_\cpt(\Hyp^2)$ such that for every geodesic $\gamma$,
\begin{equation}\label{reconstruct function from boundary}
f(\gamma) = \int_\gamma F \dif \mathcal H^1.
\end{equation}
We then set
\begin{equation}\label{definition of boundary measure}
\int_{\mathrm{Geo}(\Hyp^2)} f \dif \hat \mu := \int_{\Hyp^2} F \dif \mu.
\end{equation}

\begin{lemma}
Given a measured geodesic lamination $(\lambda, \mu)$ in $\Hyp^2$, $\hat \mu$ is a well-defined Radon measure on $\mathrm{Geo}(\Hyp^2)$.
\end{lemma}
\begin{proof}
We must show that for every $f$, there exists an $F$ satisfying (\ref{reconstruct function from boundary}); that the choice of $F$ is immaterial in (\ref{definition of boundary measure}); and that $\hat \mu$ is honestly a Radon measure.

To construct $F$, observe that since $f$ is compactly supported, there are balls $B' \Subset B \Subset \Hyp^2$ such that every geodesic in $X := \supp f$ passes through $B'$.
Using Urysohn's lemma, we can find a weight $w \in C^0_\cpt(B, \RR_+)$ such that for each $\gamma \in X$, $\int_\gamma w \dif \mathcal H^1 = 1$.
For any $x \in \gamma$, define $F(x) := f(\gamma) w(x)$.
Then $F$ is continuous since $f$ and $w$ are, and compactly supported in $B$ since $w$ is.
Moreover, it is clear that (\ref{reconstruct function from boundary}) holds.

If $G$ also satisfies (\ref{reconstruct function from boundary}), then the invariance of the locally defined transverse measures, combined with the definition (\ref{transverse measure of an open set}) of $\mu$, implies that
$$\int_{\Hyp^2} F \dif \mu = \int_{\Hyp^2} G \dif \mu.$$
Finally, the map $f \mapsto \int f \hat \mu$ is linear and bounded on $C^0$ because the map $f \mapsto F$ can be chosen to be linear and bounded $C^0 \to C^0$.
\end{proof}

Thurston's definition of a transverse measure to $\lambda$, essentially, is the Radon measure $\hat \mu$ on $\hat \lambda$.
Thus, his definition is that a sequence of measured geodesic laminations $(\lambda_n, \mu_n)$ converge to $(\lambda, \mu)$ if $\hat \mu_n \to \hat mu$ in the weak topology of measures on $\mathrm{Geo}(\Hyp^2)$ \cite[\S8.10]{thurston1979geometry}.

\begin{lemma} 
Let $(\lambda, \mu)$ be a measured geodesic laminations in $\Hyp^2$.
Then for every compactly supported $1$-form $\varphi$ on $\Hyp^2$,
$$\int_{\Hyp^2} T_\mu \wedge \varphi = \int_{\mathrm{Geo}(\Hyp^2)} \int_\gamma \varphi \dif \hat \mu(\gamma).$$
\end{lemma}
\begin{proof}
We compute using (\ref{polar ruelle sullivan}) and (\ref{definition of boundary operator}):
\begin{align*}
\int_{\Hyp^2} T_{\mu} \wedge \varphi 
&= \int_{\Hyp^2} \langle \varphi, \normal_{\lambda} \rangle \dif \mu \\
&= \int_{\mathrm{Geo}(\Hyp^2)} \int_\gamma \langle \varphi, \normal_\gamma \rangle \dif \mathcal H^1 \dif \hat \mu(\gamma) \\
&= \int_{\mathrm{Geo}(\Hyp^2)} \int_\gamma \varphi \dif \hat \mu(\gamma). \qedhere
\end{align*}
\end{proof}
...
The following are equivalent:
\begin{enumerate}
\item $(\lambda_n, \mu_n) \to (\lambda, \mu)$.
\item $\hat \mu_n \to \hat \mu$ in the weak topology of measures on $\mathrm{Geo}(\Hyp^2)$.
\end{enumerate}
\end{proposition}
\begin{proof}
Suppose first that $(\lambda_n, \mu_n) \to (\lambda, \mu)$.
Given $f \in C^0_\cpt(\mathrm{Geo}(\Hyp^2))$, choose $F$ satisfying (\ref{reconstruct function from boundary}).
Then
$$\int_{\mathrm{Geo}(\Hyp^2)} f \dif \hat \mu = \int_{\Hyp^2} F \dif \mu = \lim_{n \to \infty} \int_{\Hyp^2} F_n \dif \mu = \lim_{n \to \infty} \int_{\mathrm{Geo}(\Hyp^2)} f \dif \hat \mu_n,$$
so $\hat \mu_n \to \hat \mu$.

Conversely, if $\hat \mu_n \to \hat \mu$, we must show that we have convergence of Ruelle-Sullivan currents: $T_{\mu_n} \to T_\mu$.
Here, we recall \S\ref{RS prelims} that by twisting the cotangent bundle by a suitable line bundle $L$, these currents are well-defined even if $\lambda_n, \lambda$ do not admit an orientation.
To ease the notation we suppress the line bundle $L$ and just work with sections of the cotangent bundle.

Let $\varphi$ be a compactly supported $1$-form on $\Hyp^2$, \todo{use the previous lemma}
\end{proof}
...
%%%%%%%%%%%%%%%%%%%%%%
\subsection{Boundaries}\label{boundary conventions}
...
$$(\normal_E^\flat)_i(x) = \lim_{\varepsilon \to 0} \frac{1}{\int_{B(x, \varepsilon)} \star |\dif 1_E|} \int_{B(x, \varepsilon)} (\normal_E^\flat)_i \star |\dif 1_E|.$$
