...
%%%%%%%%%%%%%%%%%%
\subsection{Level sets of \texorpdfstring{$BV$ functions}{functions of bounded variation} as currents}
The \dfn{reduced boundary} $\partial^* U$ of a set $U \subset M$ of locally finite perimeter is the set of Lebesgue points of the $L^\infty$ $1$-form $\normal_U^\flat := \dif 1_U/|\dif 1_U|$ \cite[Chapter 4]{Giusti77}.\footnote{The set of Lebesgue points of a section of a tensor bundle is well-defined \cite[\S2]{BackusFLG}.}
Moreover, since $\partial^* U$ carries a natural measure $\star |\dif 1_U|$, we may view the reduced boundary as a closed $d - 1$-current: it is the current $-\dif 1_U$.\footnote{Note carefully that a set, $\partial^* U$ need not be $\supp -\dif 1_U$, since $\partial^* U$ need not be closed \cite[Theorem 4.4]{Giusti77}.}

We apply the above to a function $u \in BV_\loc(M)$.
...
For almost every $y \in \RR$, it follows from the coarea formula that $\{u > y\}$ is a set of locally finite perimeter, and hence we can interpret $\partial \{u > y\}$ to be the current $-\dif 1_{\{u > y\}}$.
This defines $\partial \{u > y\}$ in a way which does not depend on the choice of representative $u^*$, and which identifies $\partial \{u > y\}$ with $\partial^* \{u > y\}$.
