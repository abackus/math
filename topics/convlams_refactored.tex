...
% Assume that $(\lambda_i)$ is a sequence of minimal laminations, and $\lambda$ a minimal lamination, such that for every leaf $N$ of $\lambda$ and every large $i$, there is a leaf $N_i$ of $\lambda_i$ such that $N_i \to N$ in $\Hypspace_\loc(M)$.
% Furthermore, suppose that $(\lambda_i)$ has locally bounded curvature.
...
% Possibly after taking a subsequence of $(\lambda_i)$, we may find $\varepsilon > 0$ such that no matter what sequence $(P_i)$ we choose,
...
% Since $\Two_i$ is uniformly bounded in $i$, so is $\Two_i'$.

% Combining this fact with Lemma \ref{existence of tubes} (possibly after rescaling to get the $\log(5/4)$ bound) and (\ref{thurston angle defect}), we conclude that there exists $\delta > 0$ independent of $i$ such that $N_i$ \dfn{avoids cones} in the sense that for every $v \in \RR^d$ such that $0 < |v| < \delta$ and $|v \cdot \partial_y| < \varepsilon/2$, $P_i + v \notin N_i$.
...
% $$|x(P) - x(P_i)| \geq |P - P_i| - |y(P_i)| \geq \varepsilon - 2\varepsilon^2.$$
% Consider the triangle $\Delta$ whose vertices are $P_i, Q$, and the point $R_i$, where $x(R_i) = x(Q)$ and $y(R_i) = y(P_i)$.
% Then $\Delta$ is a right triangle, and its smallest angle $\theta_\Delta$ satisfies
% $$|\sin \theta_\Delta| = \frac{|y(P_i)|}{|x(Q) - x(P_i)|} < \frac{\varepsilon^2}{\varepsilon - 2\varepsilon^2} < \frac{\varepsilon}{4}.$$
% Thus for every large $i$, $Q$ is contained in the cone avoided by $N_i$, which contradicts the fact that $N_i \to N$ in $\Hypspace_\loc(M)$.
% \end{proof}
