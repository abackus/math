...
Note carefully that unlike in \cite{daskalopoulos2020transverse}, we assume that every transverse measure has full support, $\supp \mu_\alpha = K_\alpha$.
Thus not every lamination $\lambda$ admits a transverse measure, since $\lambda$ may have have an isolated leaf $N$ that meets a compact set infinitely many times; it can be shown that $N$ misses the support of every transverse measure.
...
Let $U \subseteq M$ be a precompact open set.
...
A function $u \in BV_\loc(M)$ has \dfn{locally least gradient} if there exists an open cover $\mathcal U$ of $M$ by sets such that for any $U \in \mathcal U$, $u|_U$ has least gradient.
\end{definition}
...
The reader should carefully note that not every function of locally least gradient has globally least gradient, essentially because a minimal hypersurface need not be absolutely area-minimizing.
Let us give three examples of this phenomenon:
\begin{enumerate}
...
Then $u$ competes with the indicator function $v$ of the region bounded by $D_2 - D_1$, and $\int_M \star |\dif v| = 2A$, so $u$ does not have least gradient.\footnote{For the case that $C$ is a critical catenoid, thus $|C| = 2A$, see \cite[Example 4.3]{Gorny20}.}
...
But there are no functions of least gradient on $\Sph^2$ except constants, since $\Sph^2$ is a closed manifold and so any function must compete with the constant functions.\footnote{One can consider functions of homologically least gradient as in \cite{daskalopoulos2020transverse}, but since $H^1(\Sph^2, \RR) = 0$, the only functions of homologically least gradient on $\Sph^2$ are constants.}
\item Let $M = \RR$ and $u$ be an unbounded increasing function. Then $u$ does not have bounded variation, so $u$ does not have least gradient, but it is clear that for any bounded interval $U$, $u|_U$ has least gradient.\footnote{I thank Wojciech G\'orny for pointing out this example.}
\end{enumerate}
...
\item The leaves of $\lambda$ are exactly the connected components of $\partial \{u > y\}$, $y \in \RR$.
...
In \S\ref{1harmonic apps} we discuss some an consequence of Theorem \ref{main thm}, including a version of the G\'orny decomposition of a function of least gradient \cite[Theorem 1.2]{górny2017planar}.
...
\item $0$ minimizes $\int_M \star |\dif (u + v)|$ among all $v \in BV(M)$ with $v$ compactly supported in $M$.
...
Since $u|_{B(p, R(p))} \in L^\infty$ \cite[Theorem 4.3]{Gorny20}, $(y_n)$ must be a bounded sequence.
...
By construction, $u|_E$ does not have least gradient, so by Theorem \ref{relaxed formulation}, there exists a perturbation $v$ which satisfies
\begin{equation}\label{not least gradient compact support}
\int_E \star |\dif u| > \int_E \star |\dif u + \dif v|
\end{equation}
and has compact support in the interior of $E$.
...
\subsection{Weak topology of measures}\label{portmanteau appendix}
