...
\begin{definition}\label{transverse measure definition}
...
A similar result to Theorem \ref{main thm}, proven with somewhat different methods, was announced but never published by Auer and Bangert \cite{Auer01, Auer12}, who claimed to establish that a locally minimal $1$-current is Ruelle-Sullivan for a lamination in a weaker sense than ours.
...
Then the Ruelle-Sullivan current $T_\mu$ is a well-defined closed $1$-current, and we may lift $T_\mu$ to the universal cover $\tilde M$, where it is exact \cite[\S8]{daskalopoulos2020transverse}.\footnote{The reference deals with geodesic laminations in a closed hyperbolic surface, but the arguments work for any tangentially $C^1$ measured oriented lamination.}
Moreover, $T_\mu$ has an intrinsic definition as the unique $1$-current with a certain polar decomposition.
...
\begin{proof}
	...
	If $A$ is uncountable, then by pigeonholing and the countability of $\NN$, there exists $n \in \NN$ such that for infinitely many $\varepsilon \in A$, $\mu(\partial B(x, \varepsilon)) > 1/n$. 
	Choose $A' \subset A$ such that $\card(A') = \aleph_0$ and for every $\varepsilon \in A'$, $\mu(\partial B(x, \varepsilon)) > 1/n$.
	The sets $\partial B(x, \varepsilon)$ are disjoint, so
	$$\mu(X) \geq \sum_{\varepsilon \in A'} \mu(\partial B(x, \varepsilon)) = \infty,$$
	contradicting that $X$ is compact and $\mu$ is Radon.
\end{proof}

Now let $X = M$ be a manifold, and consider instead the space $C_\cpt(M, \Omega^{d - \ell})$ of compactly supported continuous $d - \ell$-forms.
An $\ell$-\dfn{current of locally finite mass} (which we will simply call an \dfn{$\ell$-current}) is an element of the dual space $C_\cpt(M, \Omega^{d - \ell})'$ \cite{simon1983GMT}.
Notice that our convention is the algebraic-geometric convention: an $\ell$-current generalizes an $\ell$-form or an $\ell$-dimensional submanifold.
This is not the geometric-measure-theoretic convention.

We denote the pairing of a $d - \ell$-current $T$ and an $\ell$-form $\varphi$ by $\int_M T \wedge \varphi$; this defines the weak topology of measures on the space of currents.
To any $d - \ell$-current $T$ we may associate a positive Radon measure, its \dfn{mass measure} $\star |T|$, which satisfies for any function $f$,
...
% We shall be interested in certain second-order differential operators on the euclidean unit ball $\Ball^d$, which we shall write in the form 
% $$Q = \partial_i a^{ij} \partial_j + b^i \partial_i + c.$$
% For $k \geq 0$, we introduce the norm 
% $$\|Q\|_k := \|a\|_{W^{k + 1, \infty}} + \|b\|_{W^{k, \infty}} + \|c\|_{W^{k, \infty}},$$
% and by the \dfn{ellipticity} $\Ell(Q)$ we mean the infimum over $x \in \Ball^d$ of the smallest eigenfunction of $a(x)$.
% If $Q$ is elliptic, we will be interested in the spectrum of $Q$ in $W^{1, 2}_0$.
% We recall some standard elliptic estimates:

% \begin{lemma}\label{estimates on the first eigenfunction}
% Let $Q$ be a second order linear elliptic operator on $\Ball^d$, $k \geq 0$, and $\lambda$ the first Dirichlet eigenvalue of $Q$.
% Suppose that $Qu = \lambda u$, $u|_{\partial \Ball^d} = 0$, and $\|u\|_{L^2} = 1$.
% Then $\lambda$ and $\|u\|_{k + 2, 2}$ are bounded by constants which only depend on $d$, $\|Q\|_k$, $\Ell(Q)$.
% \end{lemma}
% \begin{proof}
% It is clear that $\lambda$ is bounded if $\|Q\|_k$ and $\Ell(Q)$ are.
% We then apply \cite[Theorem 8.12]{gilbarg2015elliptic} to bound
% $$\|u\|_{W^{2, 2}} \lesssim_{d, \|Q\|_k, \Ell(Q)} \|u\|_{L^2} + \|\lambda u\|_{L^2} \lesssim_\lambda 1.$$
% The result then follows from induction on \cite[Theorem 8.13]{gilbarg2015elliptic}.
% \end{proof}

% \begin{lemma}
% Let $Q_n, Q$ be second order linear elliptic operators on $\Ball^d$ such that $\|Q_n - Q\|_1 \to 0$.
...
% \end{lemma}
% \begin{proof}
% The coefficients $a^{ij}_{(n)}$ converge in $W^{1, \infty} \subset C^0$ to $a^{ij}$, which implies that $\Ell(Q_n) \to \Ell(Q)$.
% So by Lemma \ref{estimates on the first eigenfunction}, $\|u_n\|_{W^{3, 2}}$ is bounded and $u_n$ converges strongly in $W^{2, 2}$ to a function $u$ along a subsequence.
