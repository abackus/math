...
\item Connected components of the level sets $\partial \{u > y\}$ form a dense set of leaves.
...
We shall frequently work with functions $u$ of locally bounded variation as though they were defined pointwise.
To accomplish this, we write
$$u(x) = \lim_{\varepsilon \to 0} \frac{1}{|B(x, \varepsilon)|} \int_{B(x, \varepsilon)} \star u,$$
which is defined almost everywhere by the Lebesgue differentiation theorem; by \cite[Proposition 2.1]{BackusFLG} the value of $u(x)$, or the exceptional set on which it fails to be defined, does not depend on a choice of Riemannian metric, and if $u$ is a section of some vector bundle $E$, does not depend on the choice of trivialization of $E$.
For $y \in \RR$, the superlevel set $\{u > y\}$ is well-defined as a set of locally finite perimeter.
We call its measure-theoretic boundary $\partial \{u > y\}$ a \dfn{level set} of $u$.
...
Let $i, A > 0$, and let $g$ be a Riemannian metric on $M$ with $\|g\|_{C^{2 + \alpha}} < \infty$ and injectivity radius $\geq i$.
...
\end{equation}
In fact, the complement of $\supp \dif u$ is the set of $x \in M$ such that for some open neighborhood $V \ni x$, $u|_V$ is constant.
If $\partial \{u > y\}$ intersects $V$, then there exist $x_\pm \in V$ such that, $u(x_-) \leq y$ and $u(x_+) > y$, contradicting the definition of $V$.
Therefore $\partial \{u > y\} \subseteq \supp \dif u$.
Conversely, if $x \in \supp \dif u$, then there exist $x_n$ such that $\dif u(x_n) \neq 0$ and $x_n \to x$.
Thus, if $y_n := u(x_n)$ and $\varepsilon > 0$, there exists $x_{n, \varepsilon, \pm} \in B(x_n, \varepsilon)$ such that $u(x_{n, \varepsilon, -}) \leq y_n$ and $u(x_{n, \varepsilon, +}) > y_n$.
Taking $\varepsilon \to 0$, we conclude that
$$x_n \in \partial \{u > y_n\} \subseteq \bigcup_{y \in \RR} \partial \{u > y\}$$
and hence $x \in \overline{\bigcup_{y \in \RR} \partial \{u > y\}}$, proving the claim (\ref{level sets define support}).

We next address the structure of the closure.
Suppose that we have a sequence of $x_n \in \partial \{u > y_n\}$ converging to some $x \notin \bigcup_{y \in \RR} \partial \{u > y\}$.
By passing to a subsequence, we may assume that $(y_n)$ is a monotone sequence.
By (\ref{curvature bound on level sets}), possibly after passing to a smaller ball and rescaling, the minimal hypersurfaces $N_n := \partial \{u > y_n\}$ all have small curvature in $C^0$.
...
Moreover, since $(y_n)$ is a monotone sequence and the $N_n$ are disjoint, $(f_n)$ is also a monotone sequence; without loss of generality it is an increasing sequence.
Thus $f \geq f_n$ for every $n$, and $f_n(0) < f(0) = 0$, since $x = (0, 0)$ is contained in $N$ but not any of the $N_n$.
So by the maximum principle, $N$ is disjoint from all the $N_n$.
...
We call $N$ a \dfn{generalized level set} of $u$.

It is not possible to produce two generalized level sets $N, N'$ which intersect.
If $x \in N \cap N'$, then as above, we assume that $N, N'$ are graphs $f, f'$.
By the maximum principle, if $N \neq N'$, there exist $z_\pm$ such that $f'(z_+) > f(z_+)$ but $f'(z_-) < f(z_-)$.
We can approximate $f, f'$ in $C^0$ by graphs $f_n, f_n'$ corresponding to level sets $N_n, N_n'$; then for $n$ large, $f_n'(z_+) > f_n(z_+)$ but $f_n'(z_-) < f_n(z_-)$, contradicting the disjointness of the level sets.

Summing up, we have partitioned the closed set $\supp \dif u$ into (possibly generalized) level sets, which satisfy a curvature bound (\ref{curvature bound on level sets}).
...
We briefly give an example of a $1$-harmonic function which has a leaf which is not the boundary of a superlevel set.
Let $u(x, y) := |x|$ on $\Ball^2$; then $u$ is $1$-harmonic, since its level sets are lines.
