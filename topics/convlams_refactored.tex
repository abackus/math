...
To be more precise, if $\lambda$ is a nonorientable lamination, then we can view $\normal_\lambda$ as a section of a (necessarily twisted) line bundle $L$ over $M$.
...
If $(\lambda_n, \mu_n) \to (\lambda, \mu)$ and $x_n \in \supp \lambda_n$ converges to $x \in \supp \lambda$, then $\normal_n(x_n) \to \normal(x)$ pointwise.
\end{lemma}
\begin{proof}
...
By the measure-preserving condition in the definition of transverse measure, $T_\mu|_K$ extends uniquely to a Ruelle-Sullivan current $T_\mu$ on all of $M$.
