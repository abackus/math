...
\todo{Need a hypothesis that the measure of $E$ doesn't go to $0$}
...
\item In \S\ref{CompactnessSec}, we prove the compactness theorem, Theorem \ref{compactness theorem}, and explore the consequences for how the different modes of convergence are related to each other. This section applies both \S\ref{Regularity} and \S\ref{Prelims} for the proof of Theorem \ref{compactness theorem}, but the consequences of it also apply \S\ref{1harmonic sec}.
...
Then $(y_n)$ must a bounded sequence, since by the maximum principle (\ref{least gradient maximum principle}) $u$ is bounded on $B(p, R(p))$, so after passing to a subsequence, we may assume that $(y_n)$ converges monotonically to some $y \in \RR$.

After passing to a smaller ball and rescaling, we may use (\ref{curvature bound on level sets}), we may assume that the the minimal hypersurfaces $N_n := \partial \{u > y_n\}$ all have small curvature in $C^0$.
...
\begin{definition}
We call a hypersurface $N$ a \dfn{generalized level set} of $u$ if it is a component of $\partial \{u > y\}$, or if it is a limit of a sequence of such boundaries as above.
\end{definition}

\begin{lemma}
Let $N, N'$ be distinct generalized level sets of $u$.
Then $N \cap N'$ is empty.
\end{lemma}
\begin{proof}
As above, we assume that $N, N'$ are graphs of functions $f, f'$, which are approximated in $C^2$ by sequences $f_n, f_n'$ whose graphs are level sets.
