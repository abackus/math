...
\item The set of connected components of level sets of $u$ is dense in the set of leaves of $\lambda$.
\item There is a measured oriented structure on $\lambda$ whose Ruelle-Sullivan current is $\dif u$.
\end{enumerate}
\item Conversely, if $\lambda$ is a minimal measured oriented minimal lamination of bounded curvature and Ruelle-Sullivan current $T$, and $H^1(M, \RR) = 0$, then there exists a function $u$ of locally least gradient such that $T = \dif u$.
\end{enumerate}
\end{mainthm}
...
\todo{Replace this with just boundary of currents}
...
\item In \S\ref{1harmonic sec}, we prove the equivalence of $1$-harmonic functions and measured oriented minimal laminations, Theorem \ref{main thm}, and apply it to study $1$-harmonic functions. This section relies on \S\ref{Regularity}, \S\ref{Prelims}, and Appendix \ref{locally minimizing appendix}.
\item In \S\ref{CompactnessSec}, we prove the compactness theorem, Theorem \ref{compactness theorem}, and explore the consequences for how the different modes of convergence are related to each other. This section applies both \S\ref{Regularity} and \S\ref{Prelims}, but is independent of \S\ref{1harmonic sec} and Appendix \ref{locally minimizing appendix}.
...
Then $1_U$ has locally least gradient.
\end{lemma}
\begin{proof}
Since $\partial U$ is minimal, it is locally area-minimizing by Proposition \ref{minimal implies locally minimizing}.
If $\partial U$ is area-minimizing in an open set $V$, then $1_U|_V$ has least gradient, so $1_U$ has locally least gradient.
\end{proof}
...
\todo{Be careful here, don't accidentally invoke a maximum principle}
...
We briefly give an example of a funcion of locally least gradient which has a leaf which is not the boundary of a superlevel set.
...
for $i$ large.
So by Proposition \ref{MirandaStability}, there exists a function $u$ of least gradient such that along a subsequence, $\dif u_i \to \dif u$ in the weak topology of measures. \todo{Seems sketchy?}
...
So along a subsequence, by Proposition \ref{MirandaStability}, the current defined by $L_j'$ converges in the weak topology of measures to the current of an absolutely minimizing competitor $L'$ of $L$.
