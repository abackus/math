...
%     Let $M$ be a contractible Riemannian manifold.
%     Let $U \subseteq M$ be a set of $C^1$ boundary,alet $X$ be a vector field such that $(\normal_U, X) > 0$, and let $T > 0$.
%     If $x: [-T, T] \to M$ is an integral curve of $X$ such that $x(0) \in \partial U$, then for every $t \in (0, T)$, $x(t) \in U$ and $x(-t) \in U$.
% \end{lemma}
% \begin{proof}
%     The assertions of the theorem are local and diffeomorphism-invariant, so we may assume that $M = \RR^d$, $X$ is a standard coordinate vector field, and $x$ is a line segment.
%     By taking a compact exhaustion of $M$ and applying the fact that $\partial U$ is $C^1$, we may also assume that $(\normal_U, X)$ is uniformly positive.
...
We now compare the Plateau energy to the Dirichlet energy, and apply potential theory to get a multiplicative gain when we pass from a scale $n$ to its successor $n + 1$.
