...
\begin{proposition}
Let $D$ be a small ball.
Let $u, v$ be solutions of the $p$-Maxwell equation on $D$ such that $u - v \in C^0(\overline D)$.
Then
$$\|u - v\|_{C^0(\overline D)} \leq \|u - v\|_{C^0(\partial D)}.$$
\end{proposition}
\begin{proof}
Let $\varepsilon > 0$ and suppose that
$$\|u - v\|_{C^0(\overline D)} \geq \|u - v\|_{C^0(\partial D)} + \varepsilon.$$
Consider normal coordinates based at the center of $D$, and the metric components $g_{ij}$ arising from them.
Then for $\varepsilon' := \varepsilon/C$, where $C > 0$ depends on $g_{ij}$, there exists $i$ such that for $M_i := \|u_i - v_i\|_{C^0(\partial D)}$,
$$\|u_i - v_i\|_{C^0(\overline D)} \geq M_i + \varepsilon'.$$
Therefore (possibly after reversing the order of $u, v$) we may assume that for some $i$,
$$u_i \geq v_i + M_i + \varepsilon'$$
and hence 
$$V := \bigcup_{i=1}^3 \{u_i \geq v_i + M_i + \varepsilon'\}$$
is nonempty.
By continuity of $u, v$, and the fact that $u_i \leq v_i + M_i$ on $\partial D$, $V \Subset D$.

We now introduce the $1$-form $w$, which in components is 
$$w_i := \max(v_i - u_i + M_i + \varepsilon', 0).$$
