...
\begin{definition}
Let $M$ be a Riemannian manifold and $\mathscr U$ a class of precompact open subsets of $M$ with smooth boundary.
A Lipschitz map $u: M \to \RR^d$ is \dfn{absolutely minimizing Lipschitz} (or \dfn{AML}) on $\mathscr U$ if for every $U \in \mathscr U$,
$$\Lip(u, \overline U) = \Lip(u, \partial U).$$
If $\mathscr U$ is the topology of $M$, we simply say that $u$ is \dfn{AML}.
\end{definition}
...
In this paper we prove that spectral $\infty$-harmonic maps from surfaces to euclidean spaces are AML on simply connected sets.
...
Then $u$ is absolutely minimizing Lipschitz on sets $U$ with $H^1(U, \RR) = 0$.
\end{mainthm}
...
%%%%%%%%%%%%%%%%%
\subsection{The local Lipschitz constant}
Let $X$ be a metric space.
For a Lipschitz map $u: X \to \RR^d$, $Y \subseteq X$, and $x \in X$, we write $\Lip(u, Y)$ for the Lipschitz constant of $u|_Y$, and
...
Therefore $x \mapsto \Lip(u, x)$ is upper semicontinuous \cite[Lemma 4.2(a)]{Crandall2008}.

The metric space $X$ is a \dfn{geodesic space} if for every $x, y \in X$ there is a \dfn{geodesic} from $x$ to $y$ -- that is, a rectifiable curve $\gamma$ of arc length $\dist(x, y)$ and endpoints $x, y$.
Thus every convex Riemannian domain is a geodesic space.

\begin{lemma}\label{lipschitz constant on geodesic spaces}
If $X$ is a geodesic space, then 
$$\Lip(u, X) = \sup_{x \in X} \Lip(u, x).$$
\end{lemma}
\begin{proof}
It is clear that $\Lip(u, X) \geq \Lip(u, x)$ for any $x \in X$.
Conversely, if $x, y \in X$, $T := \dist(x, y)$, and $\gamma: [0, T] \to X$ is a geodesic from $x$ to $y$, then we may assume that $\gamma$ is parametrized by arc length.
Pulling back along $\gamma$ and applying \cite[Lemma 4.2(d)]{Crandall2008}, we see that
\begin{align*}
|u(x) - u(y)| &\leq \sup_{t \in [0, T]} \Lip(u, \gamma(t)) \dist(x, y) \leq \sup_{z \in X} \Lip(u, z) \dist(x, y). \qedhere
\end{align*}
\end{proof}
...
\begin{definition}
Let $X$ be a metric space.
The \dfn{stretch set} of a Lipschitz map $u: X \to \RR^d$ is the set of maxima of the function $x \mapsto \Lip(u, x)$ on $X$.
\end{definition}
...
From (\ref{normalization condition}) and (\ref{convergence of normalizations}),
\begin{equation}\label{normalized energies bounded}
\int_U |F_p|_p^p \star 1 \leq \kappa_p^p \int_U |\dif u_p|^p = \kappa_p \lesssim 1.
\end{equation}
...
\end{align*}
Therefore
\begin{equation}\label{cut off average is 0}
...
\end{equation}
The inequality (\ref{convexity of Schatten norms}) is vacuous on $G_p$.
However, on $M \setminus G_p$, $-(\chi \star H_p) \geq 0$, which allows us to sandwich its integral using (\ref{normalized energies bounded}):
\begin{align*}
0 \leq -\int_{M \setminus G_p} \chi H_p \leq \frac{1}{p} \int_U \chi |F_p|_p^p \star 1 \lesssim \frac{1}{p}.
\end{align*}
Since $\chi \star H_p$ is unsigned on $M \setminus G_p$, it follows that $1_{M \setminus G_p} \chi H_p \to 0$ in $L^1$.
From this and (\ref{cut off average is 0}) we see that $\chi H_p \to 0$ in $L^1$.
\end{proof}

\begin{lemma}
Let $0 < \varepsilon < 1$ and
$$\chi \in C^1_\cpt(\{x \in U: \Lip(u, x) \leq 1 - \varepsilon\}, [0, 1]).$$
Then
...
Combining this with the support property of $\chi$ and \cite[Lemma 4.2]{Crandall2008}, we deduce that almost everywhere,
...
We apply (\ref{source of decay}) to bound
\begin{align*}
...
&=: \mathbf I + \mathbf{II}.
\end{align*}
To prove (\ref{decay on positive}), it suffices to show that on every sequence $p_j \to \infty$, there is a subsequence on which (\ref{decay on positive}) holds.
So we fix a sequence, which we also denote $p \to \infty$.
After passing to a subsequence, we may assume that either there exists $c > 0$ such that for every $p \gg 1$,
...
In particular, regardless of whether (\ref{one dominates}) or (\ref{two dominates}) holds, (\ref{decay on positive}) holds.
\end{proof}
...
Combining this fact with (\ref{decay on positive}), we obtain (\ref{cut off decay}).
\end{proof}

\begin{proof}[Proof of Proposition \ref{main estimate}]
Since $\Lip(u, \cdot)$ is upper semicontinuous, there is an exhaustion $(W_\varepsilon)_{\varepsilon > 0}$ of $U \setminus \lambda$ by open sets such that
$$\sup_{x \in W_\varepsilon} \Lip(u, x) \leq 1 - \varepsilon$$
and $W_\varepsilon \Subset U \setminus \lambda$.
Let $\eta > 0$.
Since Lebesgue measure is continuous from below, there exists $\varepsilon > 0$ such that $|U \setminus (\lambda \cup W_\varepsilon)| < \eta$.
We choose $\chi_\varepsilon \in C^1_\cpt(W_\varepsilon, [0, 1])$ such that $\|\chi_\varepsilon - 1_{W_\varepsilon}\|_{L^1} < \eta$.
Then by (\ref{cut off decay}) and (\ref{normalized energies bounded}),
\begin{align*}
\lim_{p \to \infty} \kappa_p^p \int_{U \setminus \lambda} |\dif u_p|_p^p \star 1
&= \lim_{p \to \infty} \int_{U \setminus \lambda} |F_p|_p^p \star 1 \\
&\leq \lim_{p \to \infty} \int_M \chi_\varepsilon |F_p|_p^p \star 1 + 2\eta \lim_{p \to \infty} \int_U |F_p|_p^p \star 1 \\
&\lesssim \eta.
\end{align*}
The result follows by taking $\eta \to 0$.
\end{proof}

%%%%%%%%%%%%%%%%
\subsection{The Kirszbraun--Valentine theorem}
\begin{proposition}[Kirszbraun--Valentine]\label{KV}
Suppose that $M$ has nonnegative sectional curvature, and $S \subseteq M$.
Then for every Lipschitz map $f: S \to \RR^d$ there exists $F: M \to \RR^d$ with $F|_S = f$ and $\Lip(F, M) = \Lip(f, S)$.
\end{proposition}

\todo{Prove it with Topoganov}

\begin{proposition}\label{localized AML implies AML}
Suppose that $M$ has nonnegative sectional curvature, $U \subseteq M$ is open, and there is an isometric embedding $\iota: \overline U \to X$ into a geodesic space.
Then for any Lipschitz map $u: M \to \RR^d$, if
\begin{equation}\label{localized AML}
\sup_{x \in U} \Lip(u, x) \leq \Lip(u, \partial U),
\end{equation}
then $\Lip(u, \overline U) = \Lip(u, \partial U)$.
\end{proposition}
\todo{The RHS of localized AML isn't quite what we want}
\begin{proof}
By the Kirszbraun--Valentine theorem, Proposition \ref{KV}, there is a Lipschitz map $v: M \to \RR^d$ such that $v|_{\partial U} = u|_{\partial U}$ and
$$\Lip(u, U) \leq \Lip(v, M) = \Lip(u, \partial U).$$
Since $U$ is open, we obtain from (\ref{localized AML}) that
$$\sup_{x \in U} \Lip(u, x) \leq \sup_{x \in U} \Lip(v, x) \leq \Lip(v, U) \leq \Lip(u, \partial U).$$
If $x, y \in \overline U$, and $T := \dist(x, y)$, let $\gamma: [0, T] \to X$ be a geodesic from $\iota(x)$ to $\iota(y)$ parametrized by arc length.
We define $f: [0, T] \to \RR^d$ by $f(t) := u(z)$ whenever $\gamma(t) = \iota(z)$ for some $z \in \overline U$, and extension by linear interpolation on $\gamma^{-1}(X \setminus \iota(\overline U))$.
Since $f$ is extended by linear interpolation, and the extrinsic and intrinsic distances on $\iota(\overline U)$ agree,
$$|u(y) - u(x)| = |f(T) - f(0)| \leq T\sup_{z \in U} \Lip(u, z) \leq T \Lip(u, \partial U).$$
Therefore $\Lip(u, \overline U) \leq \Lip(u, \partial U)$.
...
By Proposition \ref{localized AML implies AML}, it suffices to show that for each precompact open set $U$ with smooth boundary and $H^1(U, \RR) = 0$, which embeds in a geodesic space, (\ref{localized AML}) holds.
If it does not, then by Lemma \ref{stretch set is compact}, the stretch set of $u|_{\overline U}$ does not intersect $\partial U$, a contradiction to Proposition \ref{main prop}.
\end{proof}
...
\section{Different notions of tight}
We say that $u$ is \dfn{tighter} than $v$, or $u <_T v$, if 
$$\sup \{\Lip(u, x): \Lip(u, x) > \Lip(v, x)\} < \sup \{\Lip(v, x): \Lip(v, x) > \Lip(u, x)\}.$$
We say that $u$ is \dfn{measurably tighter} than $v$, or $u <_{MT} v$, if there exists $s > 0$ such that
$$|\{|\dif u|_\infty \geq s\}| < |\{|\dif v|_\infty \geq s\}|$$
and for every $t \geq s$,
$$|\{|\dif u|_\infty \geq t\}| \leq |\{|\dif v|_\infty \geq t\}|.$$
If $u$ has no tighter (respectively, measurably tighter) competitor we say that $u$ is \dfn{tight} (respectively, \dfn{measurably tight}).

\begin{lemma}[Crandall's notes]
Comparison with cones implies AML \emph{if the target is $\RR$}.
\end{lemma}

\begin{lemma}[DU20]
Infinity-harmonic implies comparison with cones \emph{if the target is $\RR$}.
\end{lemma}

\begin{lemma}[Sheffield and Smart]
($C^2$ and) spectral infinity-harmonic implies tight \emph{if lines escape every subdomain}.
\end{lemma}

\begin{lemma}[main thm of this paper]
(Dimension $2$ and) spectral infinity-harmonic implies AML \emph{on nonannuli}.
\end{lemma}

\begin{lemma}
Tight implies AML \emph{on convex sets}.
\end{lemma}
\begin{proof}
Let $V \subseteq U$ be convex and let $v \in W^{1, \infty}(V)$ be a competitor.
Then $v|_{\partial V} = u|_{\partial V}$ so we can extend $v$ to all of $U$ by setting $v(x) := u(x)$, $x \notin V$.
Let $x$ realize the maximum of $\Lip(u, y)$, $y \in \overline V$.
Since $\overline V$ is convex,
$$\Lip(u, \overline V) = \Lip(u, x).$$
If $\Lip(u, x) \leq \Lip(v, x)$, then 
$$\Lip(u, x) \leq \Lip(v, x) \leq \Lip(v, \overline V).$$
Otherwise, $\Lip(u, x) > \Lip(v, x)$, so by tightness,
\begin{align*}
\Lip(u, x) 
&\leq \sup \{\Lip(u, y): \Lip(u, y) > \Lip(v, y)\} \\
&< \sup \{\Lip(v, y): \Lip(v, y) > \Lip(u, y)\} \\
&\leq \sup_{y \in V} \Lip(v, y) 
\leq \Lip(v, \overline V). \qedhere 
\end{align*}
\end{proof}

\begin{lemma}
Measurably tight implies tight (with no assumptions?)
\end{lemma}
...
\begin{example}
A spectral $\infty$-harmonic map can be Lipschitz but not $C^1$. Take for example $M = \Ball^2$ and
$$u(x, y) = \left(x, \frac{|y|}{2}\right).$$
Then $u$ is clearly an $\infty$-harmonic fan.
\end{example}

\begin{conjecture}
Every $\infty$-harmonic fan is a variational solution of the spectral $\infty$-Laplacian, and vice versa.
\end{conjecture}

\begin{example}[{\cite[Example 3.1]{Sheffield2010VectorvaluedOL}}]
AML does not imply that the stretch set is a geodesic lamination.
If $u(z) := z^2$ on the unit disk of $\CC$, then 
$$\Lip(u, z) = 2|z|.$$
If $V$ is an open set with smooth boundary, take $z_0 \in \partial V$ to maximize $|z_0|$, then the stretch set of $u|_V$ passes through $z_0$.
But the stretch set is a circle.
\end{example}
...
\begin{definition}
The \dfn{nonconformality} of a spectral $\infty$-harmonic map $u$ on an open set $U$ is 
$$\ncf(u, U) := \essinf_{x \in U} \sigma_1(\dif u)(x) - \sigma_2(\dif u)(x).$$
We set 
$$\ncf(u, x) := \liminf_{\delta \to 0} \ncf(u, B(x, \delta)).$$
If $\ncf(u, x) > 0$ everywhere then we say that $u$ is \dfn{nonconformal}.
\end{definition}

Since the liminf in the definition is actually a sup (since we are taking essinf's over smaller and smaller sets), $\ncf(u, \cdot)$ is lower semicontinuous.
...
\todo{We can probably use the Kirszbraun-Valentine theorem here as in Gueritaud--Kassel. We're going to use Topoganov maybe?}

\section{Examples of least gradient maps}
\subsection{Rank-one zero-homogeneous maps}
...
Suppose that $v: \Ball^2 \to \RR^2$ is conformal. Then we can write 
$$\dif v = \lambda R$$
for a field of orthogonal matrices $R$ and a scalar field $\lambda$.
We have $\dif^* R = 0$ and $\dif(\lambda R) = 0$, and we have 
$$R = \begin{bmatrix}\cos \theta & -\sin \theta \\ \sin \theta & \cos \theta\end{bmatrix}$$
for a scalar field $\theta$.
Then 
$$0 = \dif^* R = -\begin{bmatrix} \sin \theta \frac{\partial \theta}{\partial x} + \cos \theta \frac{\partial \theta}{\partial y} \\ \sin \theta \frac{\partial \theta}{\partial y} - \cos \theta \frac{\partial \theta}{\partial x}\end{bmatrix}$$
which is a differential system that can be shown to only admit constant solutions $\theta = \theta_0$.
Thus we may rotate our coordinate system so that $R$ is the identity matrix,
$$R = \begin{bmatrix}\dif x \\ \dif y\end{bmatrix}.$$
We then turn to the Bianchi identity 
$$0 = \dif(\lambda R) = \dif \lambda \wedge R = (\star \dif \lambda)^\sharp \otimes (\dif x \wedge \dif y)$$
implying that $\dif \lambda = 0$.
Therefore $\dif v = 0$, hence $v$ is constant.  

\subsection{Rotationally invariant maps}
Suppose that $v: M \to \CC$ is rotationally invariant, where $M$ is an annulus.
Then we can write 
$$\dif v = \lambda e^{i\theta} \dif r$$
where $e^{i\theta}$ is viewed as a point in $\CC$, and $\lambda, \theta$ are scalar fields that only depend on $r$.
We have 
\begin{align*}
0 &= \dif^*(e^{i\theta} \dif r) = \nabla \cdot(e^{i\theta} \partial_r) = r^{-1} \frac{\partial}{\partial r}(e^{i\theta(r)} r) \\
&= ie^{i\theta(r)} \theta'(r) + r^{-1} e^{i\theta(r)}.
\end{align*}
This differential equation has solutions 
$$\theta = \theta_0 + i\log r$$
but $\theta$ is a real scalar field, so this is a contradiction unless $\lambda = 0$.
This seems like a mistake though -- why can't we have $\theta$ be some other constant?
