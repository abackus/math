...
Let $U$ be a Riemannian domain in a surface.
If $A$ is a matrix, let $v_1(A)$ be a principal singular vector of $A$.

\begin{definition}
We say that a Lipschitz map $u: U \to \RR^c$ is a \dfn{variational solution} of the \dfn{spectral $\infty$-Laplacian}
$$
\langle \nabla^2 u, v_1(\nabla u) \otimes v_1(\nabla u)\rangle = 0,
$$
if there exists a sequence of $p \to \infty$ along which there exist \dfn{$p$-regularizations} $u_p$ of $u$, that is, Schatten von Neumann $p$-harmonic maps $u_p \in W^{1, p}(U, \RR^c)$, such that for any $s \in [1, \infty)$, $u_p \weakto u$ in $W^{1, s}$ as $p \to \infty$.
\end{definition}

\begin{definition}
Suppose that $u$ is a spectral $\infty$-harmonic map.
The \dfn{stretch set} is the set of points $x$ such that $\Lip(u, x) = \Lip(u)$.
\end{definition}

\begin{proposition}[Caccioppoli's inequality]
Let $V \Subset U$, and suppose that $v$ is Schatten-von Neumann $q$-harmonic on $U$.
Then, for $\rho := \dist(\partial U, \partial V)$,
$$\int_V |\dif v|_{sv^q}^q \leq \left(\frac{q}{\rho}\right)^q \int_{U \setminus V} |v|^q.$$
\end{proposition}
\begin{proof}
Choose $\chi \in C^\infty_\cpt(U \to [0, 1])$ such that $\chi = 1$ on $V$ and
$$\|\dif \chi\|_{C^0} \leq \frac{1 + \varepsilon}{\rho}.$$
Multiplying the $q$-Laplacian by $\psi := \chi^q \dif v$ and integrating by parts,
$$0 = \int_U \tr(\dif \psi^\dagger Q(\dif v)^{p - 2} \dif v) = \int_U q\chi^{q - 1} \tr((v \otimes \dif \chi)^\dagger Q(\dif v)^{q - 2} \dif u) + \int_U \chi^q |\dif u|_{sv^q}^q.$$
By H\"older's inequality,
\begin{align*}
\int_U \chi^q |\dif v|_{sv^q}^q 
&\leq q \int_U \chi^{q - 1} |v \otimes \dif \chi|_{sv^\infty} |\dif v|_{sv^q}^{q - 1}
\leq q \left(\int_U \chi^q |\dif v|_{sv^q}^q\right)^{1/p} \left(\int_U |\dif \chi|^q |v|^q\right)^{1/q}.
\end{align*}
After rearranging terms we get 
$$\int_U \chi^q |\dif v|_{sv^q}^q \leq q^q \int_U |\dif \chi|^q |v|^q$$
so we get 
$$\int_V |\dif v|_{sv^q}^q \leq \int_U \chi^q |\dif u|_{sv^q}^q \leq q^q \int_U |\dif \chi|^q |v|^q \leq \left(\frac{(1 + \varepsilon)q}{\rho}\right)^q \int_{U \setminus V} |v|^q$$
